...
async function run() {
  ...
  await test('resolve() returns the real engine, loading lazily if needed', async () => {
    // globalName updated to 'AudioManager' (AA-004 resolution, M387.5b) —
    // matches core/bridge/engine-bridge-bootstrap.js's real registration;
    // this test's `target` is a fake object local to this test, so the
    // rename has no functional effect here, only keeps this test honest
    // about what the real bootstrap actually registers.
    EngineBridge.register('audio', { modulePath: '../engines/audio/audio-manager.js', globalName: 'AudioManager', expectedManifestName: 'audio-manager' });
    const target = {};
    const engine = await EngineBridge.resolve('audio', { target });
    assert.ok(engine);
    assert.ok(target.CozyOS.AudioManager === engine);
  });
