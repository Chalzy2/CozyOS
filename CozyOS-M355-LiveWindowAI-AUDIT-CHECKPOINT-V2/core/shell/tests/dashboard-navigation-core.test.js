...
(async () => {
    ...
    // -----------------------------------------------------------------
    // Level 1 Universal Intelligence slice (User Dashboard: Live Video,
    // Documents, Calculations, Request an Application, Profile) extends
    // the original five surfaces to ten. Community-immediately-after-Home
    // remains the one hard invariant carried over unchanged.
    test('navigation: exactly ten surfaces exist', () => {
    ...
    test('navigation: Live Video exists', () => assert.ok(mods.Core.getSurfaceOrder().includes('live-video')));
    test('navigation: Apps exists', () => assert.ok(mods.Core.getSurfaceOrder().includes('apps')));
    test('navigation: Documents exists', () => assert.ok(mods.Core.getSurfaceOrder().includes('documents')));
    test('navigation: Calculations exists', () => assert.ok(mods.Core.getSurfaceOrder().includes('calculations')));
    test('navigation: Request an Application exists', () => assert.ok(mods.Core.getSurfaceOrder().includes('requests')));
    test('navigation: Profile exists', () => assert.ok(mods.Core.getSurfaceOrder().includes('profile')));
    ...
    test('navigation: exact required order, Community immediately after Home', () => {
        const order = mods.Core.getSurfaceOrder();
        assert.deepStrictEqual(order, ['home', 'community', 'ai', 'live-video', 'apps', 'documents', 'calculations', 'requests', 'profile', 'settings']);
    ...
    test('navigation: fails the test if Community is moved away from immediately after Home', () => {
        const order = mods.Core.getSurfaceOrder();
        const wrongOrder = ['home', 'apps', 'community', 'ai', 'live-video', 'documents', 'calculations', 'requests', 'profile', 'settings'];
    ...
    test('navigation: Home -> Community -> AI -> Live Video -> Apps -> Documents -> Calculations -> Requests -> Profile -> Settings -> Home', () => {
        mods = loadModules();
        const path_ = ['community', 'ai', 'live-video', 'apps', 'documents', 'calculations', 'requests', 'profile', 'settings', 'home'];
    ...
    await asyncTest('AI: context is a structured object, not free-text/DOM scraping', async () => {
        ...
        assert.deepStrictEqual(ctx.availableDashboardSurfaces, ['home', 'community', 'ai', 'live-video', 'apps', 'documents', 'calculations', 'requests', 'profile', 'settings']);
