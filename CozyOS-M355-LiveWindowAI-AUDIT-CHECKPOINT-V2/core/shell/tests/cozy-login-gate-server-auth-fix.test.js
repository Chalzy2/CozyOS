...
async function runHandler({ container, showError, proceed, loginWithServerPassword, loginWithCredentials, offerBiometricEnrollmentIfEligible, showMfaModal }) {
    ...
    const sandbox = {
        ...
        showMfaModal: showMfaModal || (() => { throw new Error('showMfaModal must not be called by this test'); }),
        e: { preventDefault() {} },
    };
...
test('an MFA-required response opens the real MFA modal (passkey/OTP) instead of silently stalling or dead-ending', async () => {
    let mfaArgs = null;
    let proceeded = null;
    await runHandler({
        container: buildContainer({ username: 'Chalzcozy', password: 'pw', remember: true }),
        showError: () => { throw new Error('must not show a dead-end error — MFA must be completable, not just disclosed'); },
        proceed: (id) => { proceeded = id; },
        loginWithServerPassword: async () => ({ available: true, requiresOtp: true, pendingId: 'abc' }),
        loginWithCredentials: poisoned('loginWithCredentials'),
        showMfaModal: (args, onSuccess) => { mfaArgs = args; onSuccess({ email: 'Chalzcozy' }); },
    });

    assert.equal(mfaArgs.pendingId, 'abc');
    assert.equal(mfaArgs.rememberMe, true);
    assert.equal(mfaArgs.email, 'Chalzcozy');
    // onSuccess -> offerBiometricEnrollmentIfEligible's default stub -> proceed
    assert.equal(proceeded, 'Chalzcozy');
});
