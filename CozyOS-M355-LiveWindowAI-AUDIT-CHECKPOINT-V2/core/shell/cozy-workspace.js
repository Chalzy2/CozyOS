...
(function () {
    ...
    class CozyOSWorkspaceShell {
        ...
        #appPurposeExpanded = new Set(); // Application Center "Purpose & Human Impact" dependency - same Set-based accordion pattern as #appManageExpanded, a sibling section, not a second mechanism.
        ...
        // Theme switcher dependency — same real localStorage persistence
        // pattern as #sidebarCollapsed above, no new settings system.
        #currentTheme = (() => { try { const v = window.localStorage.getItem("cozy.workspace.theme"); return ["dark", "green", "blue", "red"].includes(v) ? v : "dark"; } catch (_err) { return "dark"; } })();
        ...
        #renderCenter(centerId) {
            ...
            switch (centerId) {
                ...
                case "applicationKnowledge": return this.#renderApplicationKnowledge();
                ...
                case "security": return this.#renderSecuritySettingsGrid();
        ...
        #renderDashboard() {
            ...
            // Real admin display name — identity.getUser() already exposes
            // username (see identity-engine.js getUser()); never a
            // fabricated "Super Administrator" placeholder. Falls back to
            // the generic label only when no identity record is resolvable.
            let adminDisplayName = "Administrator";
            try {
                if (identity && this.#currentUserId && typeof identity.getUser === "function") {
                    const rec = identity.getUser(this.#currentUserId);
                    if (rec && rec.username) adminDisplayName = rec.username;
                }
            } catch (_err) { /* stays "Administrator" */ }

            const heroHtml = `
                <div class="cozy-hero">
                    <div>
                        <h1>Welcome back, ${this.#escapeHtml(adminDisplayName)}</h1>
                        <p>CozyOS Enterprise Control Center</p>
                    </div>
                    <span class="cozy-hero-status">System Online</span>
                </div>`;

            // Top-level metric row (M-DASHBOARD-VISUAL-TARGET). Total
            // Users and Active Apps reuse the same real values already
            // computed above for the stat grid — not a second counter.
            // Security Score, Pending Requests, and System Uptime have no
            // real platform-wide source anywhere in CozyOS today (checked:
            // authentication-analytics.js only scores per-user, never
            // aggregates one platform number; no module tracks admin
            // approval queues or process uptime) — shown honestly as "—"
            // rather than a fabricated percentage, count, or duration.
            const topMetricsHtml = `
                <div class="cozy-stat-grid">
                    <div class="cozy-stat-card"><div class="cozy-card-label">Total Users</div><div class="cozy-card-value">${usersValue}</div></div>
                    <div class="cozy-stat-card"><div class="cozy-card-label">Active Apps</div><div class="cozy-card-value">${appsValue}</div></div>
                    <div class="cozy-stat-card"><div class="cozy-card-label">Security Score</div><div class="cozy-card-value" style="font-size:14px;color:var(--text-secondary);">Not tracked platform-wide</div></div>
                    <div class="cozy-stat-card"><div class="cozy-card-label">Pending Requests</div><div class="cozy-card-value">—</div></div>
                    <div class="cozy-stat-card"><div class="cozy-card-label">System Uptime</div><div class="cozy-card-value" style="font-size:14px;color:var(--text-secondary);">Not tracked</div></div>
                </div>`;
            ...
            // Settings grid (M-DASHBOARD-VISUAL-TARGET) — each card links,
            // via the existing data-center click mechanism, to a real
            // section that already exists in NAV_SECTIONS above. Where no
            // such section exists anywhere in this workspace (Fingerprint,
            // Face Recognition, Two-Factor Auth, Password Policy, Session
            // Management, Login History, Device Management), the card is
            // rendered non-interactive with an honest "Not part of this
            // workspace yet" note instead of linking to the wrong place or
            // inventing a page. "Security Center" and "Automation" do
            // exist as real nav items, but are themselves still
            // "Integrations (awaiting coordinators)" placeholders — that
            // is disclosed by those sections' own existing render code,
            // unchanged here.
            const settingsCards = [
                { label: "Fingerprint", sub: "Manage fingerprint verification", center: null },
                { label: "Face Recognition", sub: "Manage face recognition", center: null },
                { label: "Two-Factor Auth", sub: "Configure 2FA settings", center: null },
                { label: "Password Policy", sub: "Set password requirements", center: null },
                { label: "Session Management", sub: "Manage user sessions", center: null },
                { label: "Login History", sub: "View login attempts", center: null },
                { label: "Device Management", sub: "Manage trusted devices", center: null },
                { label: "Security Alerts", sub: "Configure alert preferences", center: "notifications" },
                { label: "User Management", sub: "Manage platform users", center: "users" },
                { label: "Logs & Audit", sub: "Review platform audit log", center: "audit" },
                { label: "Security Center", sub: "System security", center: "security" },
                { label: "Automation", sub: "Automation Center", center: "automation" }
            ];
            const settingsGridHtml = `
                <h3 style="margin:20px 0 10px;">Settings</h3>
                <p class="cozy-disclosure-note">Cards below link to real, existing sections of this workspace where one exists. Cards with no linked section are honestly disclosed as not yet part of this workspace, rather than pointing anywhere fabricated.</p>
                <div class="cozy-quick-grid">
                    ${settingsCards.map(c => c.center
                        ? `<div class="cozy-quick-card" data-center="${this.#escapeHtml(c.center)}"><div class="cozy-card-label">${this.#escapeHtml(c.label)}</div><div style="color:var(--text-secondary);font-size:0.85rem;">${this.#escapeHtml(c.sub)}</div></div>`
                        : `<div class="cozy-quick-card" style="cursor:default;opacity:0.6;" title="Not part of this workspace yet"><div class="cozy-card-label">${this.#escapeHtml(c.label)}</div><div style="color:var(--text-secondary);font-size:0.85rem;">Not part of this workspace yet</div></div>`
                    ).join("")}
                </div>`;

            // Fingerprint Verification panel (M-DASHBOARD-VISUAL-TARGET) —
            // real data only. FingerprintProvider (core/security/
            // fingerprint-provider.js) has no real backend registered
            // anywhere in this codebase (confirmed: no call to
            // FingerprintProvider.registerBackend() exists) and
            // AuthEnrollmentStore has no "fingerprint" factor enrollments
            // recorded by any real flow — so every figure below is
            // honestly zero/unavailable, never the fabricated numbers a
            // mockup might show. This intentionally does not match a
            // visual reference that assumes a working fingerprint
            // backend CozyOS does not have.
            let fpEnrolledCount = 0, fpEnabledCount = 0, fpHasRealBackend = false;
            try {
                const fpProvider = window.CozyOS && window.CozyOS.FingerprintProvider;
                fpHasRealBackend = !!(fpProvider && typeof fpProvider.hasRealBackend === "function" && fpProvider.hasRealBackend());
                const store = window.CozyOS && window.CozyOS.AuthEnrollmentStore;
                if (store && typeof store.listEnrolledUsers === "function" && typeof store.listEnrollments === "function") {
                    for (const uid of store.listEnrolledUsers()) {
                        const rec = store.listEnrollments(uid).find(r => r.factorName === "fingerprint");
                        if (rec) { fpEnrolledCount++; if (rec.enabled) fpEnabledCount++; }
                    }
                }
            } catch (_err) { /* stays 0 */ }
            const fingerprintPanelHtml = `
                <h3 style="margin:20px 0 10px;">Fingerprint Verification</h3>
                ${!fpHasRealBackend ? this.#renderNotConnected("No real fingerprint verification backend is registered (FingerprintProvider.hasRealBackend() is false). The figures below reflect real enrollment records only — they will honestly stay at zero until a genuine backend is wired via registerBackend().") : ""}
                <div class="cozy-stat-grid">
                    <div class="cozy-stat-card"><div class="cozy-card-label">Registered Users</div><div class="cozy-card-value">${fpEnrolledCount}</div></div>
                    <div class="cozy-stat-card"><div class="cozy-card-label">Active Fingerprints</div><div class="cozy-card-value">${fpEnabledCount}</div></div>
                    <div class="cozy-stat-card"><div class="cozy-card-label">Success Rate</div><div class="cozy-card-value" style="font-size:14px;color:var(--text-secondary);">Not available</div></div>
                    <div class="cozy-stat-card"><div class="cozy-card-label">Last Updated</div><div class="cozy-card-value" style="font-size:14px;color:var(--text-secondary);">Not tracked</div></div>
                </div>
                <p class="cozy-disclosure-note">${fpEnrolledCount === 0 ? "No registered fingerprints — table intentionally omitted rather than shown with fabricated rows." : `${fpEnrolledCount} real enrollment record(s) found; a full per-user table is not yet built here.`}</p>
                <!-- Enterprise Control Center Fingerprint/Face -> WebAuthn
                     passkey connection dependency. This does NOT add a
                     fingerprint/face MATCHING backend (none exists, and
                     none is fabricated here). It connects this panel to
                     the real, already-working, server-authoritative
                     WebAuthn passkey ceremony (AuthCoordinator.
                     registerServerPasskey(), core/modules/identity/
                     auth-coordinator.js) — on real hardware with a
                     platform authenticator, that ceremony IS a
                     fingerprint/Face ID/Windows Hello prompt, handled
                     entirely by the device/OS. CozyOS never receives or
                     processes the biometric itself, only the resulting
                     cryptographic assertion. Honestly explained inline,
                     never presented as this panel's own biometric
                     backend suddenly working. -->
                <p class="cozy-disclosure-note">Real passwordless sign-in using your device's own fingerprint or face verification is already available through Passkeys — your device or browser performs the actual biometric check; CozyOS only receives a verified cryptographic result, never the biometric itself. This is separate from the (currently unconnected) fingerprint backend described above.</p>
                <button type="button" class="cozy-btn" data-action="setup-passkey">Set up a Passkey (uses your device's fingerprint/Face ID)</button>
                <div id="cozy-passkey-setup-result"></div>`;
            ...
            return `${heroHtml}${topMetricsHtml}
                ${settingsGridHtml}
                ${fingerprintPanelHtml}
                ${statsHtml}
                ${summaryCardsHtml}
        ...
        /**
         * #renderApplicationKnowledge()
         *   Application Knowledge dependency - a NEW Level-2 destination
         *   inside the existing master menu (Overview section), NOT a
         *   new dashboard/framework. Composes exclusively existing,
         *   authoritative sources:
         *     - getApplicationCenterData() (already reuses ServiceRegistry
         *       + the existing role/permission filter - no second
         *       application list, no bypass of that filter)
         *     - window.CozyOS.CozyKnowledge.getApplicationFact() (technical)
         *     - window.CozyOS.CozyKnowledge.getApplicationHumanPurposeFact() (human purpose)
         *     - window.CozyOS.CozyKnowledge.getCapabilityHumanPurposeFact()
         *       for the platform capabilities already registered there
         *   Read-only: no input, no button that mutates anything. Never
         *   invents an application or a purpose - an application with no
         *   registered human-purpose record shows the same honest empty
         *   state the Application Center's own "Purpose & Human Impact"
         *   section already uses.
         */
        #renderApplicationKnowledge() {
            const knowledge = window.CozyOS && window.CozyOS.CozyKnowledge;
            const centerData = this.getApplicationCenterData();
            if (!centerData.connected) return `<h2>Application Knowledge</h2>${this.#renderNotConnected(centerData.message)}`;

            const appSectionsHtml = centerData.applications.length === 0
                ? this.#renderNotConnected("No applications registered yet.")
                : centerData.applications.map(app => {
                    const fact = knowledge && typeof knowledge.getApplicationFact === "function" ? knowledge.getApplicationFact(app.name) : { evidence: "NOT_FOUND", application: null };
                    const purposeFact = knowledge && typeof knowledge.getApplicationHumanPurposeFact === "function" ? knowledge.getApplicationHumanPurposeFact(app.name) : { evidence: "NOT_FOUND", purpose: null };
                    const overviewHtml = fact.evidence === "VERIFIED" && fact.application
                        ? `<p><b>Category:</b> ${this.#escapeHtml(fact.application.category || "—")} &middot; <b>Enabled:</b> ${fact.application.enabled === false ? "No" : "Yes"} &middot; <b>Version:</b> ${this.#escapeHtml(fact.application.version || "—")}</p>`
                        : this.#renderNotConnected(`No technical fact registered for "${this.#escapeHtml(app.name)}" through getApplicationFact() — showing raw registry fields only.`);
                    const purposeHtml = purposeFact.evidence === "VERIFIED" && purposeFact.purpose ? `
                        <p><b>Human purpose:</b> ${this.#escapeHtml(purposeFact.purpose.humanPurpose)}</p>
                        <p><b>Real-life problems addressed:</b> ${this.#escapeHtml(purposeFact.purpose.realLifeProblems.join(", "))}</p>
                        <p><b>Who benefits:</b> ${this.#escapeHtml(purposeFact.purpose.whoBenefits.join(", "))}</p>
                        <p><b>Human benefits:</b> ${this.#escapeHtml(purposeFact.purpose.humanBenefits.join(", "))}</p>
                        <p><b>CURRENTLY VERIFIED:</b> ${this.#escapeHtml(purposeFact.purpose.currentVerifiedCapabilities.join("; "))}</p>
                        <p><b>VISION / DESTINATION (not yet implemented):</b> ${this.#escapeHtml(purposeFact.purpose.visionCapabilities.join("; "))}</p>
                    ` : this.#renderNotConnected(`No human-purpose information is registered for "${this.#escapeHtml(app.name)}" yet — this page does not invent one.`);
                    return `
                        <div class="cozy-app-card" style="border:1px solid var(--cozy-border,#233827);border-radius:12px;padding:16px;margin-bottom:14px;">
                            <h3 style="margin:0 0 8px;">${this.#escapeHtml(app.name)}</h3>
                            ${overviewHtml}
                            ${purposeHtml}
                        </div>`;
                }).join("");

            // Platform capabilities already registered through the same,
            // existing getCapabilityHumanPurposeFact() fact-getter - the
            // exact known identifiers actually committed there (this
            // page does not invent capability names; it checks the real
            // registered set built across prior dependencies).
            const KNOWN_CAPABILITY_KEYS = ["natural-record-capture", "biometric-login"];
            const capabilitiesHtml = KNOWN_CAPABILITY_KEYS.map(key => {
                const capFact = knowledge && typeof knowledge.getCapabilityHumanPurposeFact === "function" ? knowledge.getCapabilityHumanPurposeFact(key) : { evidence: "NOT_FOUND", purpose: null };
                if (capFact.evidence !== "VERIFIED" || !capFact.purpose) return "";
                return `
                    <div class="cozy-app-card" style="border:1px solid var(--cozy-border,#233827);border-radius:12px;padding:16px;margin-bottom:14px;">
                        <h3 style="margin:0 0 8px;">${this.#escapeHtml(key)}</h3>
                        <p><b>Human purpose:</b> ${this.#escapeHtml(capFact.purpose.humanPurpose)}</p>
                        <p><b>CURRENTLY VERIFIED:</b> ${this.#escapeHtml(capFact.purpose.currentVerifiedCapabilities.join("; "))}</p>
                        <p><b>VISION / DESTINATION (not yet implemented):</b> ${this.#escapeHtml(capFact.purpose.visionCapabilities.join("; "))}</p>
                    </div>`;
            }).join("");

            return `<h2>Application Knowledge</h2>
                <p class="cozy-disclosure-note">This page shows the same authoritative application and capability knowledge Cozy AI itself reads — never a separate, potentially-diverging administrator description. Read-only: nothing here can be edited from this page yet.</p>
                <h3 style="margin:20px 0 10px;">Applications</h3>
                ${appSectionsHtml}
                ${capabilitiesHtml ? `<h3 style="margin:20px 0 10px;">Platform Capabilities</h3>${capabilitiesHtml}` : ""}`;
        }

        /**
         * #renderSecuritySettingsGrid()
         *   Settings-grid dependency — reuses the existing Level-2
         *   "Security Center" destination (previously an unconnected
         *   INTEGRATION_SLOTS placeholder). Only wires cards whose real
         *   capability was confirmed by direct source inspection:
         *     - Fingerprint -> links to the existing Dashboard center,
         *       where the real WebAuthn passkey button already lives.
         *     - Two-Factor Auth -> real AuthCoordinator
         *       beginServerTotpEnrollment()/disableServerTotp().
         *     - Login History -> real AuthCoordinator.getLoginHistory().
         *     - System Preferences -> the existing "configuration"
         *       Level-2 destination.
         *   Every other card has no real destination/capability found
         *   anywhere in the codebase and is shown with an honest
         *   unavailable state — never a fake destination. No new
         *   router/registry: one render method behind one existing
         *   Level-2 id.
         */
        #renderSecuritySettingsGrid() {
            const CARDS = [
                { icon: "🖐️", title: "Fingerprint", desc: "Sign in with your device's fingerprint via Passkeys.", action: "goto-dashboard" },
                { icon: "🙂", title: "Face Recognition", desc: "No real face-recognition backend is connected.", action: null },
                { icon: "🔢", title: "Two-Factor Auth", desc: "Enroll or disable TOTP-based two-factor authentication.", action: "totp" },
                { icon: "🔑", title: "Password Policy", desc: "Not implemented in the current CozyOS security architecture.", action: null },
                { icon: "🖥️", title: "Session Management", desc: "Not implemented — no live session listing/revocation exists yet.", action: null },
                { icon: "📜", title: "Login History", desc: "View your real recent sign-in history.", action: "login-history" },
                { icon: "📱", title: "Device Management", desc: "Not implemented — no trusted-device listing exists yet.", action: null },
                { icon: "🚨", title: "Security Alerts", desc: "Not implemented in the current CozyOS security architecture.", action: null },
                { icon: "✉️", title: "Email Settings", desc: "Not implemented in the current CozyOS security architecture.", action: null },
                { icon: "🔐", title: "API Keys", desc: "Not implemented in the current CozyOS security architecture.", action: null },
                { icon: "💾", title: "Backup & Restore", desc: "Not implemented in the current CozyOS security architecture.", action: null },
                { icon: "⚙️", title: "System Preferences", desc: "Configure platform-wide preferences.", action: "goto-configuration" },
            ];
            const cardsHtml = CARDS.map((c) => `
                <div class="cozy-settings-card${c.action ? "" : " cozy-settings-card-disabled"}" data-settings-action="${c.action || ""}">
                    <div class="cozy-settings-card-icon">${c.icon}</div>
                    <div class="cozy-settings-card-title">${this.#escapeHtml(c.title)}</div>
                    <div class="cozy-settings-card-desc">${this.#escapeHtml(c.desc)}</div>
                    ${c.action ? `<div class="cozy-settings-card-arrow">→</div>` : ""}
                </div>`).join("");
            return `<h2>Security &amp; Sign-in</h2>
                <div class="cozy-settings-grid">${cardsHtml}</div>
                <div id="cozy-settings-result"></div>`;
        }
        ...
        #renderApplicationCenter() {
            ...
            const cards = data.applications.map(app => {
                ...
                // Application Center "Purpose & Human Impact" dependency
                // — read-only, reuses the exact existing
                // getApplicationHumanPurposeFact() fact-getter (Cozy
                // AI's own source, cozy-knowledge-registry.js) so the
                // administrator sees the SAME application truth Cozy AI
                // already understands — never a second/derived
                // description. VERIFIED (currentVerifiedCapabilities)
                // and VISION (visionCapabilities) are rendered in
                // clearly separate, labeled blocks, in that order,
                // exactly mirroring the conversational reply's own
                // ordering discipline (app-importance:known template).
                // No write path: this section has no input/button that
                // mutates anything.
                const purposeExpanded = this.#appPurposeExpanded.has(app.id);
                const knowledge = window.CozyOS && window.CozyOS.CozyKnowledge;
                const purposeFact = knowledge && typeof knowledge.getApplicationHumanPurposeFact === "function"
                    ? knowledge.getApplicationHumanPurposeFact(app.name)
                    : { evidence: "NOT_FOUND", purpose: null };
                const purposePanel = purposeExpanded ? `
                    <div class="cozy-app-purpose-panel" style="margin-top:8px;padding:10px;border-top:1px dashed var(--cozy-border,#233827);font-size:12px;">
                        ${purposeFact.evidence === "VERIFIED" && purposeFact.purpose ? `
                            <p><b>Human purpose:</b> ${this.#escapeHtml(purposeFact.purpose.humanPurpose)}</p>
                            <p><b>Real-life problems addressed:</b> ${this.#escapeHtml(purposeFact.purpose.realLifeProblems.join(", "))}</p>
                            <p><b>Who benefits:</b> ${this.#escapeHtml(purposeFact.purpose.whoBenefits.join(", "))}</p>
                            <p><b>Human benefits:</b> ${this.#escapeHtml(purposeFact.purpose.humanBenefits.join(", "))}</p>
                            <p><b>CURRENTLY VERIFIED:</b> ${this.#escapeHtml(purposeFact.purpose.currentVerifiedCapabilities.join("; "))}</p>
                            <p><b>VISION / DESTINATION (not yet implemented):</b> ${this.#escapeHtml(purposeFact.purpose.visionCapabilities.join("; "))}</p>
                        ` : this.#renderNotConnected(`No human-purpose information is registered for "${this.#escapeHtml(app.name)}" yet — this section does not invent one.`)}
                    </div>` : "";
                return `
                <div class="cozy-app-card" style="border:1px solid var(--cozy-border,#233827);border-radius:12px;padding:16px;margin-bottom:10px;display:flex;flex-direction:column;gap:8px;">
                    ...
                    <div style="display:flex;gap:8px;margin-top:4px;">
                        ...
                        <button type="button" class="cozy-btn" data-app-purpose-toggle="${this.#escapeHtml(app.id)}">${purposeExpanded ? "Hide Purpose & Human Impact" : "Purpose & Human Impact"}</button>
        ...
        #render() {
            ...
            // Applications sidebar grouping — real, dynamic per-app links
            // built from the SAME authoritative, already-authorized
            // getApplicationCenterData() the Application Center itself
            // uses (ServiceRegistry + existing role/permission filter).
            // Never hard-coded app names, never a second application
            // list. Each link reuses the existing per-app detail view
            // (#renderApplicationHealth) via #selectedContext, set by
            // the click handler above.
            let dynamicAppItems = [];
            try {
                const centerData = this.getApplicationCenterData();
                if (centerData.connected) {
                    dynamicAppItems = centerData.applications.map((app) => [`app:${app.id}`, app.name]);
                }
                ...
                <div class="cozy-nav-section${isOpen ? " open" : ""}">
                    ...
                    <div class="cozy-nav-section-items">
                        ${section.items.map(([id, label]) => {
                            const isAppLink = id.startsWith("app:");
                            const realAppId = isAppLink ? id.slice(4) : null;
                            const targetCenter = isAppLink ? "applications" : id;
                            const isActive = isAppLink
                                ? (this.#activeCenter === "applications" && this.#selectedContext && this.#selectedContext.type === "app-health" && this.#selectedContext.id === realAppId)
                                : this.#activeCenter === id;
                            return `<div class="cozy-nav-link${isActive ? " active" : ""}" data-center="${targetCenter}"${realAppId ? ` data-app-id="${this.#escapeHtml(realAppId)}"` : ""} title="${this.#escapeHtml(label)}"><span class="cozy-nav-link-label">${this.#escapeHtml(label)}</span></div>`;
                        }).join("")}
                    </div>
                </div>`;
            }).join("");

            // Live Admin Header dependency — real identity only.
            // identity.getUser() has no email/avatar field (confirmed by
            // direct source inspection) - never fabricated here. Avatar
            // is a deterministic initials badge derived from the real
            // username, not a fake image. Notification count reuses the
            // existing, real getNotificationFeed() - never a fabricated
            // badge. Logout reuses the existing, real
            // AuthCoordinator.logout(). Display-only: this identity
            // block grants nothing - IdentityEngine/server remain the
            // sole authorization authority.
            let headerUsername = "Administrator";
            try {
                const identity = window.CozyOS && window.CozyOS.IdentityEngine;
                if (identity && this.#currentUserId && typeof identity.getUser === "function") {
                    const rec = identity.getUser(this.#currentUserId);
                    if (rec && rec.username) headerUsername = rec.username;
                }
            } catch (_err) { /* stays "Administrator" */ }
            const headerInitials = headerUsername.trim().slice(0, 2).toUpperCase() || "AD";
            let headerNotificationCount = 0;
            try { headerNotificationCount = this.getNotificationFeed().length; } catch (_err) { /* stays 0 */ }
            const accountHtml = `
                <div class="cozy-sidebar-account">
                    <button type="button" class="cozy-account-notification-btn" aria-label="Notifications${headerNotificationCount > 0 ? `, ${headerNotificationCount} unread` : ""}">
                        🔔${headerNotificationCount > 0 ? `<span class="cozy-notification-badge">${this.#escapeHtml(String(headerNotificationCount))}</span>` : ""}
                    </button>
                    <div class="cozy-account-identity">
                        <span class="cozy-account-avatar">${this.#escapeHtml(headerInitials)}</span>
                        <span class="cozy-account-name">${this.#escapeHtml(headerUsername)}</span>
                    </div>
                    <button type="button" id="cozy-account-logout-btn" class="cozy-account-logout-btn" aria-label="Sign out">⏻</button>
                </div>
                <div class="cozy-theme-switcher" role="group" aria-label="Theme">
                    ${["dark", "green", "blue", "red"].map((t) => `<button type="button" class="cozy-theme-swatch cozy-theme-swatch-${t}${this.#currentTheme === t ? " active" : ""}" data-theme="${t}" aria-label="${t.charAt(0).toUpperCase() + t.slice(1)} theme" aria-pressed="${this.#currentTheme === t}"></button>`).join("")}
                </div>`;

            // Real, direct application to the document root - CSS
            // custom-property cascade, no business-logic change.
            try { document.documentElement.setAttribute("data-cozy-theme", this.#currentTheme); } catch (_err) { /* honest no-op */ }
            ...
            this.#domRoot.innerHTML = `
                <div class="${shellClasses.join(" ")}">
                    ...
                    <nav class="cozy-sidebar">
                        ...
                        <form id="cozy-header-search-form" class="cozy-header-search" role="search">
                            <span class="cozy-header-search-icon">🔎</span>
                            <input type="text" id="cozy-header-search-input" class="cozy-header-search-input" placeholder="Search anything" aria-label="Search anything" value="${this.#escapeHtml(this.#searchTerm || "")}" />
                        </form>
            ...
            if (!this.#documentClickDismissBound) {
                this.#domRoot.addEventListener("click", (evt) => {
                    ...
                    if (centerEl) {
                        ...
                        // Applications sidebar grouping — a real per-app
                        // sidebar link (data-app-id) sets the exact same
                        // #selectedContext the existing "Settings" button
                        // on each Application Center card already uses
                        // (#renderApplicationHealth) — reuses the real,
                        // existing per-app detail view, never a second
                        // navigation mechanism.
                        const appId = centerEl.getAttribute("data-app-id");
                        if (appId) { this.#selectedContext = { type: "app-health", id: appId }; }
                    ...
                    const settingsCard = evt.target.closest("[data-settings-action]");
                    if (settingsCard && settingsCard.getAttribute("data-settings-action")) {
                        const action = settingsCard.getAttribute("data-settings-action");
                        const resultEl = this.#domRoot.querySelector("#cozy-settings-result");
                        if (action === "goto-dashboard") { this.#activeCenter = "dashboard"; this.#render(); return; }
                        if (action === "goto-configuration") { this.#activeCenter = "configuration"; this.#render(); return; }
                        if (action === "totp") {
                            const coordinator = window.CozyOS && window.CozyOS.AuthCoordinator;
                            if (!coordinator || typeof coordinator.beginServerTotpEnrollment !== "function") {
                                if (resultEl) resultEl.innerHTML = `<p class="cozy-disclosure-note">Two-Factor Auth isn't available right now — the authentication coordinator isn't loaded.</p>`;
                                return;
                            }
                            if (resultEl) resultEl.innerHTML = `<p class="cozy-disclosure-note">Starting Two-Factor Auth enrollment…</p>`;
                            coordinator.beginServerTotpEnrollment().then((result) => {
                                if (!resultEl) return;
                                resultEl.innerHTML = result && result.available
                                    ? `<p class="cozy-disclosure-note">Two-Factor Auth enrollment started — follow your authenticator app's instructions.</p>`
                                    : `<p class="cozy-disclosure-note">Couldn't start Two-Factor Auth enrollment${result && result.reason ? `: ${this.#escapeHtml(result.reason)}` : "."}</p>`;
                            }).catch(() => { if (resultEl) resultEl.innerHTML = `<p class="cozy-disclosure-note">Couldn't start Two-Factor Auth enrollment due to an unexpected error.</p>`; });
                            return;
                        }
                        if (action === "login-history") {
                            const coordinator = window.CozyOS && window.CozyOS.AuthCoordinator;
                            if (!coordinator || typeof coordinator.getLoginHistory !== "function") {
                                if (resultEl) resultEl.innerHTML = `<p class="cozy-disclosure-note">Login History isn't available right now — the authentication coordinator isn't loaded.</p>`;
                                return;
                            }
                            if (resultEl) resultEl.innerHTML = `<p class="cozy-disclosure-note">Loading real login history…</p>`;
                            Promise.resolve(coordinator.getLoginHistory(this.#currentUserId)).then((result) => {
                                if (!resultEl) return;
                                const entries = (result && result.entries) || (Array.isArray(result) ? result : []);
                                resultEl.innerHTML = entries.length
                                    ? `<ul class="cozy-login-history-list">${entries.map((e) => `<li>${this.#escapeHtml(JSON.stringify(e))}</li>`).join("")}</ul>`
                                    : `<p class="cozy-disclosure-note">No real login history entries found.</p>`;
                            }).catch(() => { if (resultEl) resultEl.innerHTML = `<p class="cozy-disclosure-note">Couldn't load login history due to an unexpected error.</p>`; });
                            return;
                        }
                    }
                    if (evt.target.getAttribute("data-action") === "setup-passkey") {
                        // Enterprise Control Center Fingerprint/Face ->
                        // WebAuthn connection dependency. Calls the real,
                        // existing, server-authoritative
                        // AuthCoordinator.registerServerPasskey() —
                        // never a new authentication/session/identity
                        // system, never a fabricated biometric backend.
                        const resultEl = this.#domRoot.querySelector("#cozy-passkey-setup-result");
                        const coordinator = window.CozyOS && window.CozyOS.AuthCoordinator;
                        if (!coordinator || typeof coordinator.registerServerPasskey !== "function") {
                            if (resultEl) resultEl.innerHTML = `<p class="cozy-disclosure-note">Passkey setup isn't available right now — the authentication coordinator isn't loaded.</p>`;
                            return;
                        }
                        if (resultEl) resultEl.innerHTML = `<p class="cozy-disclosure-note">Follow your device's prompt (fingerprint, Face ID, or Windows Hello)…</p>`;
                        coordinator.registerServerPasskey().then((result) => {
                            if (!resultEl) return;
                            if (result && result.available) {
                                resultEl.innerHTML = `<p class="cozy-disclosure-note">Passkey set up successfully — your device's fingerprint/face verification can now be used to sign in.</p>`;
                            } else {
                                resultEl.innerHTML = `<p class="cozy-disclosure-note">Couldn't set up a passkey${result && result.reason ? `: ${this.#escapeHtml(result.reason)}` : "."}</p>`;
                            }
                        }).catch(() => {
                            if (resultEl) resultEl.innerHTML = `<p class="cozy-disclosure-note">Couldn't set up a passkey due to an unexpected error.</p>`;
                        });
                        return;
                    }
                    ...
                    /** Purpose & Human Impact toggle — same accordion-state pattern as #appManageExpanded above, a sibling read-only section. No write action, no permission gate: identical visibility to the rest of the Application Center. */
                    const purposeToggle = evt.target.closest("[data-app-purpose-toggle]");
                    if (purposeToggle) {
                        const appId = purposeToggle.getAttribute("data-app-purpose-toggle");
                        this.#appPurposeExpanded.has(appId) ? this.#appPurposeExpanded.delete(appId) : this.#appPurposeExpanded.add(appId);
                        this.#render();
                        return;
                    }
                    ...
                    const themeSwatch = evt.target.closest("[data-theme]");
                    if (themeSwatch) {
                        const theme = themeSwatch.getAttribute("data-theme");
                        if (["dark", "green", "blue", "red"].includes(theme)) {
                            this.#currentTheme = theme;
                            try { window.localStorage.setItem("cozy.workspace.theme", theme); } catch (_err) { /* ignore */ }
                            this.#render();
                        }
                        return;
                    }
                    if (evt.target.closest("#cozy-account-logout-btn")) {
                        const coordinator = window.CozyOS && window.CozyOS.AuthCoordinator;
                        if (coordinator && typeof coordinator.logout === "function") { try { coordinator.logout(); } catch (_err) { /* honest no-op */ } }
                        return;
                    }
                ...
                this.#domRoot.addEventListener("submit", (evt) => {
                    if (evt.target && evt.target.id === "cozy-header-search-form") {
                        evt.preventDefault();
                        const input = this.#domRoot.querySelector("#cozy-header-search-input");
                        const term = input ? input.value.trim() : "";
                        if (!term) return;
                        // Reuses the exact existing WorkspaceShell.search()
                        // authority and #renderSearch() destination - no
                        // second search engine/result template.
                        this.search(term);
                        this.#activeCenter = "search";
                        this.#render();
                    }
                });
                this.#domRoot.addEventListener("keydown", (evt) => {
                    if (evt.target && evt.target.id === "cozy-header-search-input" && evt.key === "Escape") {
                        evt.target.value = "";
                        evt.target.blur();
                        return;
                    }
