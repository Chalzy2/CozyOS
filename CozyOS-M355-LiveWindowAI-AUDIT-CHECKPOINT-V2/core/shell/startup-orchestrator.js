...
(function () {
    ...
    const DEFAULT_CONFIG = Object.freeze({
        ...
        videoRotationIntervalMs: 8000, // Rotating Live Login Backgrounds — real dwell time per scene before Background.startVideoRotation() advances to the next registered category.
        audioEnabled: true,
        audioFadeMs: 1500,
        soundsEnabled: true
    });
    ...
    class CozyStartupOrchestrator {
         ...
         *
         *   Login Sequence video integration addition: also starts the
         *   default registered live-background video, composing the
         *   existing, real window.CozyOS.Background.playVideoFromLibrary()
         *   (already handles its own cross-fade, muted/loop/playsInline,
         *   prefers-reduced-motion honesty, and sits at a lower z-index
         *   than this same canvas — see cozy-background.js). Deliberately
         *   best-effort and separate from the canvas result above: a
         *   missing/not-yet-registered video must never prevent the
         *   existing canvas scene from revealing, so failures here are
         *   reported but not thrown and do not affect `revealed` above.
         *
         *   Rotating Live Login Backgrounds addition: replaces the
         *   single fixed "always Forest" call with
         *   Background.startVideoRotation() over the same real, already-
         *   registered official categories loadOfficialVideoPack()
         *   provides (Forest first, matching the exact prior visible
         *   behavior on the very first frame, then Waterfalls/Ocean/
         *   Nature/Clouds in a fixed order) — same engine, same
         *   playVideoFromLibrary() underneath, no new video source. Any
         *   category with no real registered video (e.g. birds/nature-
         *   animal footage, still unresolved per assets/video-source-
         *   library/PROVENANCE.md) is skipped by startVideoRotation()
         *   itself, never fabricated. Idempotent, same as before.
         */
        revealLiveBackground() {
            ...
            const bg = window.CozyOS && window.CozyOS.Background;
            let video = { attempted: false };
            if (bg && typeof bg.startVideoRotation === "function") {
                const cfg = this.getConfig();
                video = { attempted: true, result: bg.startVideoRotation(
                    ["Forest", "Waterfalls", "Ocean", "Nature", "Clouds"],
                    { intervalMs: cfg.videoRotationIntervalMs }
                ) };
            } else if (bg && typeof bg.playVideoFromLibrary === "function") {
                // Honest fallback for an older/partial Background build
                // that hasn't picked up startVideoRotation() yet — same
                // single-video behavior as before this addition, never a
                // hard failure.
                video = { attempted: true, result: bg.playVideoFromLibrary("Forest", "cozyos-forest-v1") };
            }
            return { revealed: true, video };
        }
        ...
        /**
         * loadOfficialVideoPack()
         *   Login/Living Sequence video integration — the real, single
         *   registration point for the official CozyOS live-background
         *   video library, mirroring loadOfficialSoundPack() immediately
         *   above (same idempotent, registration-only pattern; playback
         *   still happens on demand elsewhere, not here). Composes the
         *   existing, real window.CozyOS.Background.registerVideo() —
         *   confirmed already implemented (registerVideo/setVideoSource/
         *   playVideoFromLibrary/VIDEO_CATEGORIES) but never previously
         *   called anywhere in this codebase before this addition; an
         *   unused real extension point, not a new engine. Every source
         *   video was visually inspected frame-by-frame and trimmed to a
         *   clean, watermark/caption-free segment before being placed at
         *   these paths — see assets/video-source-library/PROVENANCE.md
         *   for exactly which source, which segment, and why. Mapped
         *   onto the existing fixed VIDEO_CATEGORIES list rather than
         *   inventing new category names: "Ocean" is the closest existing
         *   category to underwater/fish content (no dedicated
         *   "Underwater" category exists), and "Nature" is used for the
         *   palm-trees clip for the same reason (no "Tropical" category
         *   exists). Idempotent — safe to call from every page without
         *   double-registering, exactly like loadOfficialSoundPack().
         */
        loadOfficialVideoPack() {
            const bg = window.CozyOS.Background;
            if (!bg || typeof bg.registerVideo !== "function") {
                return { success: false, reason: "CozyOS.Background is not loaded." };
            }
            const registrations = [
                ["Waterfalls", { id: "cozyos-waterfall-v1", url: "assets/video/waterfall.mp4", name: "Waterfall" }],
                ["Ocean", { id: "cozyos-underwater-v1", url: "assets/video/underwater.mp4", name: "Underwater" }],
                ["Forest", { id: "cozyos-forest-v1", url: "assets/video/forest.mp4", name: "Forest" }],
                ["Nature", { id: "cozyos-palm-trees-v1", url: "assets/video/palm-trees.mp4", name: "Palm Trees" }],
                ["Clouds", { id: "cozyos-clouds-v1", url: "assets/video/clouds.mp4", name: "Clouds" }],
            ];
            const results = registrations.map(([category, entry]) => ({
                category,
                id: entry.id,
                ...bg.registerVideo(category, entry),
            }));
