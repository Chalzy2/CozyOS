...
(function () {
    ...
    class CozyLivingMessageEngine {
         ...
         */
        /**
         * createMessage() — extended (Milestone 209) with the real
         * configuration fields the Admin Publisher Console needs: title
         * (distinct from body text), language, voicePlayback (a real
         * boolean flag the console reads before calling the existing
         * VoiceEngine/CozySpeech — this file never plays audio itself),
         * animationSpeed (separate from the existing animation type),
         * textColor, fontSize, expiresAt. All stored on the same message
         * record — no parallel storage, no second publishing path.
         */
        createMessage(userId, { category, title = null, text, orgId = null, priority = "normal", schedule = { type: "continuous" }, visibility = "entire-platform", animation = "fade", animationSpeed = "normal", textColor = null, fontSize = null, language = null, voicePlayback = false, expiresAt = null, durationMs = 10000 } = {}) {
            ...
            const message = { messageId, category, title: title ? String(title).slice(0, 200) : null, text: String(text).slice(0, 2000), orgId, priority, schedule, visibility, animation, animationSpeed, textColor, fontSize, language, voicePlayback: !!voicePlayback, expiresAt, durationMs, status: "enabled", viewCount: 0, dismissCount: 0, createdAt: new Date(Date.now()).toISOString() };
    ...
    if (window.CozyOS.ServiceRegistry && typeof window.CozyOS.ServiceRegistry.registerCoordinator === "function") {
        try {
            window.CozyOS.ServiceRegistry.registerCoordinator({ sourcePath: "core/ui/living-message-engine.js",
