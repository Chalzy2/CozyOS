...
(function () {
    ...
    const SpeechTranslationGeminiProvider = {
        ...
        register() {
            ...
            // Domain 4C dependency #3 — same real, optional
            // ProviderManager visibility registration as the NLLB
            // bridge above and the existing conversational providers.
            // Never touches the Gemini API key or the real translate()
            // path; purely observational.
            const pm = window.CozyOS.ProviderManager;
            if (pm && typeof pm.register === "function") {
                pm.register({
                    id: "gemini-translate",
                    name: "Gemini Translation Provider",
                    ...
                    getHealth() {
                        return {
                            health: "UNKNOWN",
                            reason: "Registered in SpeechTranslationProviders, but real availability depends on a same-origin backend holding a real GEMINI_API_KEY and having real network egress — neither is knowable without a live request, which getHealth() must not trigger as a side effect. See the most recent translate() result for genuine, observed availability."
                        };
                    }
                });
            }
