...
(function () {
    ...
    /**
     * _sameLanguagePassthrough(text, sourceLanguage, targetLanguage)
     *   KISWAHILI SOURCE LANGUAGE dependency (continuing from M399).
     *   Kiswahili (and any other language) can be a SOURCE/INTENT
     *   language in its own right for live spoken content (worship,
     *   pastor speech, live transactions — not implemented here, future
     *   scope). When source and target are the same registered
     *   language, the real content already IS in the requested
     *   language — that is preservation of the original, never a
     *   translation, and must never be sent to a provider (self.Translator/
     *   nllb/gemini) as if it needed translating.
     *
     *   This is the one place both real call sites (translateText(),
     *   previewTranslation()) route through before ever reaching a
     *   provider, so the rule is enforced once, not duplicated per
     *   provider or per caller. Comparison is case-insensitive only —
     *   no new language table, no CozyLanguageRegistry lookup needed
     *   here since this only compares the two codes the caller already
     *   supplied to each other, not against any registry.
     */
    function _sameLanguagePassthrough(text, sourceLanguage, targetLanguage) {
        if (!sourceLanguage || !targetLanguage) return null;
        if (String(sourceLanguage).trim().toLowerCase() !== String(targetLanguage).trim().toLowerCase()) return null;
        return { isReal: true, translatedText: text, providerName: "passthrough-same-language", preserved: true };
    }
    ...
    const SpeechTranslationAdapter = {
        ...
        async translateText(sessionId, text, opts = {}) {
            ...
            const translate = _translate();
            const passthrough = _sameLanguagePassthrough(text, sourceLanguage, targetLanguage);
            ...
            if (!result) {
                ...
                result = await translatorRecord.api.translate(text, { sourceLanguage, targetLanguage });
            }
