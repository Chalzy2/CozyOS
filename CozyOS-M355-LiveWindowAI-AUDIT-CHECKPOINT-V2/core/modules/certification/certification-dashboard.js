...
(function () {
    ...
    class CertificationDashboard {
        ...
        #renderQuickCertification() {
            ...
            return `<h1>Quick Certification</h1><p class="cz-subtitle">Certify one file — drag it in, pick it, paste source, or load one already uploaded elsewhere in CozyOS. Nothing here is ever executed; the text goes straight to CozyCertification.quickCertification().</p>
                <div class="cz-panel">
                    ${files.length > 0 ? `
                    <div class="cz-field">
                        <label>Load a file already uploaded to Workspace (from Builder, BugFixer, or a prior registration — no re-upload, and works for files too large to paste)</label>
                        <div class="cz-row">
                            <select class="cz-input" id="cz-quick-existing-file">
                            ...
                            <button class="cz-btn" data-action="load-existing-file">Load</button>
                        </div>
                    </div>` : ""}
        ...
        /** Loads a file already registered in WorkspaceShell's file registry — its real stored source — directly into the Quick Certification form. No second upload; works for files too large or unwieldy to paste by hand. */
        #loadExistingFile() {
            const workspace = this.#getWorkspace();
            const fileId = document.getElementById("cz-quick-existing-file")?.value;
            if (!workspace || !fileId) return;
            const file = workspace.getFile(fileId);
            if (!file) return;
            if (!file.source) {
                window.alert("This entry has no text source on file (registered as a handle-only reference). Use CozyBugFixer's Edit action to read its current on-disk content first.");
                return;
            }
            ...
            if (versionEl && file.builderVersion) versionEl.value = file.builderVersion;
            if (sourceEl) sourceEl.value = file.source;
            const detectedEl = document.getElementById("cz-quick-detected");
            if (detectedEl) detectedEl.innerHTML = `Loaded from Workspace: <b>${escapeHtml(file.filename)}</b> (${escapeHtml(file.source.length)} characters).`;
        }
        ...
        async #runQuickCertification() {
            ...
            const moduleId = (moduleIdEl && moduleIdEl.value.trim()) || this.#deriveModuleIdFromFilename(this.#pendingQuickFile?.file?.name) || "untitled_module";
...
Build a POS with M-Pesa integration.">${escapeHtml(ctx.promptText || "")}</textarea>
        ...
        #devRegisterToServiceRegistry(moduleId) {
            ...
            try {
                window.CozyOS.registerCoordinator({ sourcePath: "core/modules/certification/certification-dashboard.js", name: moduleId, category: "Business Domain", icon: `${moduleId.toLowerCase()}.svg`, description: `${moduleId} — registered from the Certification dashboard.` });
        ...
        #dispatchAction(action, el) {
            switch (action) {
                ...
                case "load-existing-file": this.#loadExistingFile(); break;
