 ...
 *   OWNERSHIP AUDIT: this file (window.CozyOS.MpesaFloat) is the
 *   confirmed, canonical, actually-loaded live owner of MpesaOS agent
 *   float — referenced by <script src="core/plugins/mpesaOS-float.js">
 ...
 *   docs/builder/knowledge/module-inventory.json. No second float
 *   engine exists or is created here.
 *
 *   Feature IDs reuse the exact convention already established by the
 *   receipts ("mpesa"/"mpesa.receipts", mpesaOS.js) and reports
 *   ("mpesa"/"mpesa.reports", mpesaOS-reporting.js) integrations —
 *   "mpesa.float" is added here, verbatim style, not invented fresh.
 *   APPLICATION-level ("mpesa") is checked before the FUNCTION-level
 *   ("mpesa.float") state, the same cascade the other two integrations
 *   use, so an ADMIN_DISABLED/PLAN_RESTRICTED application makes float
 *   management unavailable before "mpesa.float"'s own state is even
 *   consulted.
 *
 *   Scope of THIS integration pass: the three discretionary,
 *   administrator/agent-initiated write actions only — purchaseFloat()
 *   (buying more float), adjustFloat() (manual correction), and
 *   reconcileFloat() (comparing tracked vs. actual). All three are
 *   optional business actions an organisation may legitimately want to
 *   mute or restrict per plan, exactly the "stock/store function"
 *   class of capability this milestone targets.
 *
 *   recordTransactionImpact() is deliberately left UNGUARDED here. It
 *   is the real hook mpesaOS.js's engine calls after every committed
 *   Deposit/Withdrawal to keep the tracked float balance accurate
 *   (see file header above) — it is a direct consequence of a
 *   REQUIRED transaction, not a discretionary feature, and gating it
 *   would silently desynchronize the tracked float balance from real
 *   money movement the moment an organisation muted "mpesa.float" —
 *   exactly the kind of transaction-integrity regression Rule 9
 *   warns against. Reads (getCurrentFloat/getFloatHistory/
 *   getReconciliation/listReconciliations) are likewise left
 *   unguarded here: mpesaOS-reporting.js's getFloatMovementReport()
 *   already reads getFloatHistory()/getCurrentFloat() directly and is
 *   itself gated at the "mpesa.reports" boundary, so gating the same
 *   reads a second time here would only duplicate — not strengthen —
 *   an already-real enforcement point.
 *
 *   Fails closed (returns {available:false, reason, state}, never
 *   throws) when float management is not entitled for any reason —
 ...
 *   the same honest, fail-closed convention mpesaOS-reporting.js's
 *   report gate already uses. On success, the existing return shape
 *   (the deep-cloned movement/reconciliation record) is unchanged —
 *   this pass adds a new denial path, it does not alter the existing
 *   success contract any caller already relies on.
 */
(function () {
    "use strict";
    ...
    const MPESA_FEATURE_FLOAT = "mpesa.float";
    ...
    class MpesaFloatCoordinator {
        ...
        /**
         * #checkFloatEntitlement(organizationId)
         *   The real service-boundary gate for the three discretionary
         *   float actions below (see file header for the full scope
         *   discussion). Never throws; each guarded method reacts to
         *   the returned {allowed, state, reason} the same honest way
         *   mpesaOS-reporting.js's report gate already does. Fails
        ...
        #checkFloatEntitlement(organizationId) {
            ...
            if (!entitlement || typeof entitlement.guard !== "function") {
                return { allowed: false, state: "UNAVAILABLE", reason: "Entitlement engine is not available — float management fails closed." };
            }
            if (typeof organizationId !== "string" || !organizationId.trim()) {
                return { allowed: false, state: "UNAVAILABLE", reason: "organizationId is required to evaluate float entitlement." };
            }
            try {
                entitlement.guard(organizationId, MPESA_FEATURE_APP);
                entitlement.guard(organizationId, MPESA_FEATURE_FLOAT);
                return { allowed: true };
            } catch (err) {
                const state = (err && err.decision && err.decision.state) || "DENIED";
                return { allowed: false, state, reason: `Float management is not available for this organization (${state}).` };
            }
        }
        ...
        purchaseFloat(rawInput = {}) {
            const { companyId, branchId, amount: rawAmount, source, userId, organizationId } = sanitizeObject(rawInput);
            const gate = this.#checkFloatEntitlement(organizationId);
            if (!gate.allowed) {
                this.#logAudit("FLOAT_ACTION_BLOCKED", `purchaseFloat blocked (${gate.state}): ${companyId}/${branchId}`);
        ...
        adjustFloat(rawInput = {}) {
            const { companyId, branchId, amount: rawAmount, reason, userId, organizationId } = sanitizeObject(rawInput);
            const gate = this.#checkFloatEntitlement(organizationId);
            if (!gate.allowed) {
                this.#logAudit("FLOAT_ACTION_BLOCKED", `adjustFloat blocked (${gate.state}): ${companyId}/${branchId}`);
                return { available: false, reason: gate.reason, state: gate.state };
            }
            this.#validateCompanyBranch(companyId, branchId);
            const amount = Number(rawAmount);
        ...
        reconcileFloat(rawInput = {}) {
            const { companyId, branchId, actualFloat: rawActual, userId, organizationId } = sanitizeObject(rawInput);
            const gate = this.#checkFloatEntitlement(organizationId);
            if (!gate.allowed) {
                this.#logAudit("FLOAT_ACTION_BLOCKED", `reconcileFloat blocked (${gate.state}): ${companyId}/${branchId}`);
                return { available: false, reason: gate.reason, state: gate.state };
            }
            this.#validateCompanyBranch(companyId, branchId);
    ...
    function initRegistration() {
        ...
        if (window.CozyOS && window.CozyOS.PluginManager) {
            window.CozyOS.PluginManager.register(
                ...
                window.CozyOS.PluginManager.createMinimalIntentHandler(engineInstance, "MpesaOS Float Management")
            );
        } else {
