...
class DocumentStorageRegistry {
  ...
  /**
   * _requireOwnership(actorUserId, existingRow)
   *   Real authorization for PERSONAL documents — the exact analogue
   *   of _requireActiveMember()/_requireManageCapability() for the
   *   organization path, but simpler by design: a personal document
   *   has exactly one real owner (its user_id), so authorization is a
   *   direct equality check against the row's own stored owner, never
   *   a client-supplied claim. No membership/role concept applies to
   *   a personal document — there is nothing to be "an active member"
   *   of.
   */
  _requireOwnership(actorUserId, row) {
    if (!row || row.user_id !== actorUserId) throw new DocumentStorageError('not_authorized');
  }
  ...
  // ---------------------------------------------------------------------
  // Personal (user-owned) documents — Document Ownership Foundation.
  // Mirrors every organization method above exactly (identical
  // versioning/audit/soft-delete semantics), scoped by user_id instead
  // of organization_id, and authorized by direct ownership
  // (_requireOwnership) instead of organization membership/capability.
  // Never trusts a client-supplied owner: actorUserId always comes from
  // the caller's own real, authenticated session (session.userId at the
  // route layer), exactly like every organization method's actorUserId.
  // No new engine, no new table beyond the one already-shared
  // `documents` table (now supporting exactly one of organization_id /
  // user_id per row via the real Phase 5 CHECK constraint).
  // ---------------------------------------------------------------------

  /** savePersonal(actorUserId, record) — real create-or-new-version for a personal document. Identical identity/versioning rules to save(). */
  async savePersonal(actorUserId, rawRecord) {
    if (!actorUserId) throw new DocumentStorageError('not_authorized');
    if (!rawRecord || typeof rawRecord !== 'object') throw new TypeError('[document-storage] savePersonal(): record is required.');
    if (rawRecord.status && !VALID_STATUSES.has(rawRecord.status)) throw new TypeError(`[document-storage] savePersonal(): unsupported status "${rawRecord.status}".`);
    if (typeof rawRecord.documentId !== 'string' || !rawRecord.documentId) throw new TypeError('[document-storage] savePersonal(): documentId is required.');
    ...
    if (existing) {
      this._requireOwnership(actorUserId, existing);
      const newVersion = existing.current_version + 1;
      const finalRecord = { ...rawRecord, documentId: existing.id, version: newVersion };
      await this.db.run(
        'UPDATE documents SET document_type = ?, category = ?, status = ?, title = ?, raw_text = ?, checksum = ?, tags = ?, record_json = ?, current_version = ?, updated_at = ? WHERE id = ?',
      ...
      await this._logAudit(actorUserId, 'DOCUMENT_UPDATED', `${existing.id} -> v${newVersion} (personal)`);
      return { available: true, documentId: existing.id, version: newVersion };
    }
    ...
    const finalRecord = { ...rawRecord, documentId, version: 1 };
    await this.db.run(
      'INSERT INTO documents (id, organization_id, user_id, document_type, category, status, title, raw_text, checksum, tags, record_json, current_version, created_by, created_at, updated_at) VALUES (?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [documentId, actorUserId, rawRecord.documentType || 'unknown', rawRecord.category || null, rawRecord.status && VALID_STATUSES.has(rawRecord.status) ? rawRecord.status : 'draft', rawRecord.title || null, rawRecord.rawText || null, checksum, JSON.stringify(rawRecord.tags || []), JSON.stringify(finalRecord), 1, actorUserId, now, now]
    );
    await this.db.run(
      'INSERT INTO document_versions (id, document_id, version, snapshot_json, checksum, changed_by, change_summary, changed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [crypto.randomUUID(), documentId, 1, JSON.stringify(finalRecord), checksum, actorUserId, 'Initial save', now]
    );
    await this._logAudit(actorUserId, 'DOCUMENT_CREATED', `${documentId} (personal)`);
    return { available: true, documentId };
  }

  /** loadPersonal(actorUserId, documentId) — only the owning user may read; honest null otherwise, never leaking cross-user existence. */
  async loadPersonal(actorUserId, documentId) {
    if (typeof documentId !== 'string' || !documentId) return { available: false, reason: 'Document not found.' };
    const row = await this.db.get('SELECT * FROM documents WHERE id = ? AND user_id = ?', [documentId, actorUserId]);
    if (!row) return { available: false, reason: 'Document not found.' };
    return { available: true, record: JSON.parse(row.record_json), binarySize: row.binary_size, binaryMimeType: row.binary_mime_type, binaryChecksum: row.binary_checksum, hasBinary: !!row.binary_storage_ref };
  }

  async _transitionStatusPersonal(actorUserId, documentId, newStatus, eventType) {
    const row = await this.db.get('SELECT * FROM documents WHERE id = ? AND user_id = ?', [documentId, actorUserId]);
    if (!row) return { available: false, reason: 'Document not found.' };
    const updatedRecord = { ...JSON.parse(row.record_json), status: newStatus };
    await this.db.run('UPDATE documents SET status = ?, record_json = ?, updated_at = ? WHERE id = ?', [newStatus, JSON.stringify(updatedRecord), this.now(), documentId]);
    await this._logAudit(actorUserId, eventType, `${documentId} (personal)`);
    return { available: true, documentId };
  }

  archivePersonal(actorUserId, documentId) { return this._transitionStatusPersonal(actorUserId, documentId, 'archived', 'DOCUMENT_ARCHIVED'); }
  restorePersonal(actorUserId, documentId) { return this._transitionStatusPersonal(actorUserId, documentId, 'draft', 'DOCUMENT_RESTORED'); }
  deletePersonal(actorUserId, documentId) { return this._transitionStatusPersonal(actorUserId, documentId, 'deleted', 'DOCUMENT_DELETED'); }

  /** getVersionsPersonal(actorUserId, documentId) — only the owning user may read version history. */
  async getVersionsPersonal(actorUserId, documentId) {
    const doc = await this.db.get('SELECT id FROM documents WHERE id = ? AND user_id = ?', [documentId, actorUserId]);
    if (!doc) return { available: false, reason: 'Document not found.' };
    const rows = await this.db.all('SELECT version, snapshot_json, checksum, changed_by, change_summary, changed_at FROM document_versions WHERE document_id = ? ORDER BY version ASC', [documentId]);
    return { available: true, versions: rows.map((r) => ({ version: r.version, snapshot: JSON.parse(r.snapshot_json), checksum: r.checksum, changedBy: r.changed_by, changeSummary: r.change_summary, changedAt: r.changed_at })) };
  }

  /** searchPersonal(actorUserId, filters) — real, filterable search scoped strictly to the caller's own personal documents. */
  async searchPersonal(actorUserId, filters = {}) {
    if (!actorUserId) throw new DocumentStorageError('not_authorized');
  ...
  /**
   * savePersonalBinary(actorUserId, documentId, readableStream, opts)
   *   Real personal-ownership equivalent of saveBinary() above — same
   *   real FilesystemObjectStorageProvider, same real validation
   *   (checksum/size/mimeType/filename sanitization/cleanup-on-
   *   oversized), only the ownership/authorization check differs:
   *   _requireOwnership() (direct user_id match) instead of
   *   _requireManageCapability() (organization membership). No new
   *   object-storage provider, no schema change — the binary_* columns
   *   already exist on the shared `documents` table.
   */
  async savePersonalBinary(actorUserId, documentId, readableStream, { mimeType = 'application/octet-stream', originalFilename = null, maxBytes = null } = {}) {
    if (!this.objectStorage) return { available: false, reason: 'Binary storage is not configured.' };
    if (!actorUserId) throw new DocumentStorageError('not_authorized');
    const existing = await this.db.get('SELECT * FROM documents WHERE id = ? AND user_id = ?', [documentId, actorUserId]);
    if (!existing) return { available: false, reason: 'Document not found.' };

    // Real key namespacing: personal documents use the owning user's id
    // in place of an organizationId — buildKey() itself has no concept
    // of "organization" beyond being a plain string namespace segment,
    // so this reuses it exactly as-is, never fabricating an
    // organization for a personal document.
    const key = this.objectStorage.buildKey(`personal_${actorUserId}`, documentId, existing.current_version);
    ...
    await this._logAudit(actorUserId, 'DOCUMENT_BINARY_UPLOADED', `${documentId} v${existing.current_version} (${result.size} bytes, personal)`);
    return { available: true, documentId, storageRef: key, size: result.size, mimeType: result.mimeType, checksum: result.checksum };
  }

  /**
   * loadPersonalBinary(actorUserId, documentId)
   *   Real personal-ownership equivalent of loadBinary() above. Only
   *   the owning user may read — no membership concept applies.
   */
  async loadPersonalBinary(actorUserId, documentId) {
    if (!this.objectStorage) return { available: false, reason: 'Binary storage is not configured.' };
    if (!actorUserId) throw new DocumentStorageError('not_authorized');
    const existing = await this.db.get('SELECT * FROM documents WHERE id = ? AND user_id = ?', [documentId, actorUserId]);
    if (!existing) return { available: false, reason: 'Document not found.' };
    ...
    try {
      ...
      await this._logAudit(actorUserId, 'DOCUMENT_BINARY_DOWNLOADED', `${documentId} (personal)`);
