...
test('/chalzydashboard and /chalzydashboard.html are the SAME real route — byte-identical content, no unauthenticated bypass difference, both equally gated as the sole administrator entry (never two competing admin surfaces)', async () => {
  await withServer(async ({ base, rp }) => {
    // Unauthenticated: both forms must serve the identical gate page
    // (which itself decides Access Denied client-side) — never a 404,
    // never a different file, never a redirect difference.
    const noSessionCanonical = await fetch(base + '/chalzydashboard');
    const noSessionAlias = await fetch(base + '/chalzydashboard.html');
    assert.equal(noSessionCanonical.status, 200);
    assert.equal(noSessionAlias.status, 200);
    const noSessionCanonicalHtml = await noSessionCanonical.text();
    const noSessionAliasHtml = await noSessionAlias.text();
    assert.equal(noSessionCanonicalHtml, noSessionAliasHtml, 'both URL forms must serve byte-identical HTML — this is one real route with two accepted spellings, not two separate pages');

    // A genuine platform administrator's real session must be honored
    // identically regardless of which spelling was requested.
    const { cookie: adminCookie } = await registerAndLogin(base, rp, { email: 'route-equivalence-admin@example.com', admin: true });
    const adminCanonical = await fetch(base + '/chalzydashboard', { headers: { Cookie: adminCookie } });
    const adminAlias = await fetch(base + '/chalzydashboard.html', { headers: { Cookie: adminCookie } });
    assert.equal(adminCanonical.status, 200);
    assert.equal(adminAlias.status, 200);
    const adminCanonicalHtml = await adminCanonical.text();
    const adminAliasHtml = await adminAlias.text();
    assert.equal(adminCanonicalHtml, adminAliasHtml, 'an authenticated administrator must receive byte-identical content from both URL forms');
    assert.equal(adminCanonicalHtml, noSessionCanonicalHtml, 'the served FILE itself never varies by session — this is the same static gate page for everyone; the real admit/deny decision happens client-side against the real server session endpoint, confirmed by the other tests in this file');
  });
});
