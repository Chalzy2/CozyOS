...
(function () {
    ...
    async function showAuthenticationForm(container, onAuthenticated, proceed) {
            ...
            container.querySelector("#cozy-login-credentials-form").addEventListener("submit", async (e) => {
                ...
                // REAL FIX (M-ADMIN-AUTH-RESTORE, applied here): this used
                // to call AuthCoordinator.loginWithCredentials(), which
                // resolves purely against IdentityEngine's own local,
                // per-browser #users registry (IndexedDB) — never the
                // real server is_platform_admin authority. A real
                // administrator username set up via bootstrap-admin.js's
                // set-username CLI (e.g. "Chalzcozy") has no local
                // IdentityEngine record at all, so this always failed
                // with "No real administrator account found", even
                // though the real server account is valid. Switched to
                // the same real, already-tested, server-authoritative
                // method login.html's own Administrator section uses:
                // loginWithServerPassword() (POST /auth/login), which
                // already detects and sends either {username} or
                // {email} — no second auth engine, no new server call.
                try {
                    const result = await window.CozyOS.AuthCoordinator.loginWithServerPassword(username, password, { rememberMe: rememberMeChecked });
                    if (!result || result.available !== true) { showError(container, (result && result.reason) || "Invalid administrator username/email or password."); return; }
                    if (result.requiresOtp) {
                        // Real server MFA gate — no session exists yet.
                        // MFA completion (passkey + TOTP/recovery code) is
                        // handled by showMfaModal() below, which calls the
                        // same AuthCoordinator.completeServerLoginWithOtp()/
                        // completeServerLoginWithPasskeyMfa() methods
                        // login.html's showOtpStep() already uses — no
                        // second MFA implementation, same server authority.
                        showMfaModal({ pendingId: result.pendingId, rememberMe: rememberMeChecked, email: username }, async (mfaResult) => {
                            const audioEngine2 = window.CozyOS.LivingAudio;
                            if (audioEngine2 && typeof audioEngine2.play === "function") {
                                try { await audioEngine2.play("auth.login.success"); } catch (_err) { /* honest no-op */ }
                            ...
                            await offerBiometricEnrollmentIfEligible(container, mfaResult.email, proceed);
                        });
                        return;
                    }
                    ...
                    // Restored (was dropped when this switched off the
                    // legacy path): offers to enroll THIS device for
                    // biometric sign-in, keyed by the real identifier
                    // just used to authenticate — the same string the
                    // biometric sign-in handler above now looks up by.
                    // offerBiometricEnrollmentIfEligible() is completely
                    // opaque about this string (see its own header) —
                    // it never requires a local IdentityEngine record.
                    await offerBiometricEnrollmentIfEligible(container, result.email, proceed);
                } catch (err) {
            ...
            container.querySelector("#cozy-login-biometric-form").addEventListener("submit", async (e) => {
                ...
                // REAL FIX: this used to require a LOCAL
                // IdentityEngine.listUsers() record matching the typed
                // username before even attempting a device lookup — but
                // TrustedDeviceManager (and AuthCoordinator.
                // loginWithBiometrics()) are both completely opaque
                // about this string: it is only ever used as a key,
                // never cross-referenced against IdentityEngine
                // internally. A real server-authoritative administrator
                // (e.g. "Chalzcozy", mapped via bootstrap-admin.js
                // set-username) legitimately has no local IdentityEngine
                // record at all, so this always failed with an honest
                // but wrong "No real administrator account found" even
                // when a real trusted device had been enrolled for this
                // exact identifier (see offerBiometricEnrollmentIfEligible's
                // call site below, which now enrolls using this same
                // real identifier after a successful server login).
                // Uses the typed identifier directly — exact string
                // match against whatever was used at enrollment time,
                // same case-sensitivity the server's own username lookup
                // already has.
                if (!tdm || typeof tdm.generateFingerprint !== "function") {
                    showError(container, "TrustedDeviceManager is not loaded — cannot sign in with biometrics.");
                    return;
                }
                if (!username) { showError(container, "Enter your username or email."); return; }
                const fingerprint = await tdm.generateFingerprint();
                const device = tdm.findDeviceForUser(username, fingerprint);
                ...
                try {
                    const result = await window.CozyOS.AuthCoordinator.loginWithBiometrics({ userId: username, deviceId: device.deviceId });
    ...
    /**
     * showMfaModal({pendingId, rememberMe, email}, onSuccess)
     *   Completes a password-verified, MFA-pending server login
     *   (result.requiresOtp === true from AuthCoordinator.
     *   loginWithServerPassword()) — the same second-factor step
     *   login.html's showOtpStep() already implements, reusing the
     *   identical AuthCoordinator methods (no second MFA
     *   implementation, no new server route). Built as a self-
     *   contained overlay appended to document.body rather than into
     *   renderLoginForm()'s own template, so this can be added without
     *   touching that existing, tested markup at all.
     *   onSuccess(mfaResult) is called only after the server has
     *   already set the real session cookie (identical posture to
     *   every other success path in this file).
     */
    function showMfaModal({ pendingId, rememberMe, email }, onSuccess) {
        const overlay = document.createElement("div");
        overlay.id = "cozy-gate-mfa-overlay";
        overlay.style.cssText = "position:fixed;inset:0;z-index:100000;background:rgba(3,7,6,0.72);display:flex;align-items:center;justify-content:center;font-family:system-ui,sans-serif;";
        const passkeySupported = typeof navigator !== "undefined" && navigator.credentials && typeof navigator.credentials.get === "function";
        overlay.innerHTML = `
            <div class="cozy-living-card cozy-living-panel cozy-bloom cozy-living-glass cozy-living-border-glow" style="max-width:380px;width:90%;padding:26px;border-radius:16px;color:#f1f5f4;">
                <h2 style="margin:0 0 6px 0;font-size:17px;font-weight:800;color:#fff;">Enter Your Authenticator Code</h2>
                <p style="margin:0 0 16px 0;color:#94a3b8;font-size:12px;">This account has a second sign-in factor enabled. Enter the 6-digit code from your authenticator, or a recovery code if you don't have access to it.</p>
                <label style="font-size:11px;color:#cbd5e1;">Authenticator Code or Recovery Code
                    <input id="cozy-gate-mfa-code" type="text" inputmode="numeric" autocomplete="one-time-code" maxlength="20" class="cozy-living-input"
                        style="display:block;width:100%;box-sizing:border-box;padding:9px;margin-top:4px;background:rgba(10,15,13,0.8);border-radius:10px;color:#fff;">
                </label>
                <div style="display:flex;gap:8px;margin-top:14px;">
                    <button type="button" id="cozy-gate-mfa-cancel" style="flex:1;padding:10px;border:1px solid rgba(148,163,184,0.3);border-radius:10px;background:transparent;color:#cbd5e1;cursor:pointer;">Cancel</button>
                    <button type="button" id="cozy-gate-mfa-submit" style="flex:1;padding:10px;border:1px solid rgba(249,168,37,0.3);border-radius:10px;background:linear-gradient(90deg,#1B5E20,#2E7D32);color:#fff;font-weight:700;cursor:pointer;">Verify</button>
                </div>
                <button type="button" id="cozy-gate-mfa-passkey" style="display:${passkeySupported ? "block" : "none"};width:100%;text-align:center;margin-top:10px;padding:8px;background:transparent;border:none;color:#F9A825;font-size:12px;cursor:pointer;">Use fingerprint or device passkey instead</button>
                <p id="cozy-gate-mfa-error" style="display:none;margin-top:12px;padding:9px;border-radius:8px;background:rgba(239,68,68,0.12);color:#fca5a5;font-size:12px;"></p>
            </div>
        `;
        document.body.appendChild(overlay);

        const codeInput = overlay.querySelector("#cozy-gate-mfa-code");
        const submitBtn = overlay.querySelector("#cozy-gate-mfa-submit");
        const cancelBtn = overlay.querySelector("#cozy-gate-mfa-cancel");
        const passkeyBtn = overlay.querySelector("#cozy-gate-mfa-passkey");
        const errEl = overlay.querySelector("#cozy-gate-mfa-error");
        ...
        passkeyBtn.addEventListener("click", async () => {
            errEl.style.display = "none";
            passkeyBtn.disabled = true; passkeyBtn.textContent = "Waiting for your device...";
            const auth = window.CozyOS.AuthCoordinator;
            let result;
            try {
                result = await auth.completeServerLoginWithPasskeyMfa(pendingId, { rememberMe, email });
            } catch (err) {
                result = { available: false, reason: err.message };
            }
            passkeyBtn.disabled = false; passkeyBtn.textContent = "Use fingerprint or device passkey instead";
            if (!result || result.available !== true) {
                if (result && result.code === "no_passkeys_registered") {
                    passkeyBtn.style.display = "none";
                }
                if (!(result && result.code === "user_cancelled")) {
                    errEl.textContent = (result && result.reason) || "Passkey verification failed.";
                    errEl.style.display = "block";
                }
                return;
            }
    ...
    const CozyOSLoginGate = {
        ...
        async mountIfNeeded(container, onAuthenticated) {
            ...
            // M373 — real, confirmed fix: IdentityEngine.restorePersistedUsers()
            // (the real method that rehydrates #users from the real
            // IdentityStorage/IndexedDB layer) existed but was never
            // called anywhere in this repository, confirmed by
            // exhaustive search before this was written. Without it,
            // #users stays empty on every fresh page load - meaning
            // restoreSession()'s own fallback path
            // (restoreSessionForTrustedPointer(), which looks up
            // this.#users.get(userId)) would always fail even with a
            // correctly-stored Remember Me pointer. Must complete before
            // restoreSession() runs, not after.
            const identityForRestore = window.CozyOS.IdentityEngine;
            if (identityForRestore && typeof identityForRestore.restorePersistedUsers === "function") {
                await identityForRestore.restorePersistedUsers();
            }
            ...
            const proceed = (explicitUserId = null) => {
                ...
                // M-IDGATE — real, confirmed fix for the Admin identity
                // mismatch: getDashboardConfig(currentUserId) above only
                // ever consults IdentityEngine's own local #users
                // registry, populated solely by ITS OWN createUser()/
                // register()/restorePersistedUsers(). A userId
                // authenticated entirely through the server-side
                // WebAuthn/password Admin RP (AuthCoordinator.
                // loginWithServerPasskey()/loginWithServerPassword() ->
                // CozyOS.Session.establishFromExternalAuth()) has no
                // local record and never will — that path intentionally
                // never calls createUser(), so config is unconditionally
                // {available:false, reason:"Unknown userId..."} for it,
                // every time, regardless of retries.
                //
                // This gate is not, and must not become, the place that
                // decides Admin authorization. The one real caller of
                // mountIfNeeded() (admin-workspace.html) already performs
                // its OWN independent, server-authoritative check inside
                // its onAuthenticated callback: a credentialed
                // GET /webauthn/session followed by AdminGateCore.
                // decideGateAction() (see admin-workspace.html's
                // mountWorkspaceIfAdmin()) — the exact same real check
                // chalzydashboard.html performs, never a second, divergent
                // implementation. Calling onAuthenticated() here grants
                // nothing by itself; it only lets an identity CozyOS.Session
                // itself already reports as established through a real,
                // external, already-verified auth bridge (source ===
                // "external" — set exclusively by AuthCoordinator's own
                // #finishServerLogin()/Firebase bridge, never arbitrary
                // client-editable state) reach that real check, instead of
                // being wrongly turned away before ever reaching it.
                //
                // Deliberately narrow: only bypasses this local check when
                // IdentityEngine has NO record for currentUserId at all
                // (identity.getUser() returns null/undefined) — a KNOWN
                // local account whose config says unavailable (disabled,
                // suspended, archived, or genuinely unknown to Session too)
                // keeps failing closed exactly as before this fix.
                const hasLocalIdentityRecord = identity && currentUserId && typeof identity.getUser === "function"
                    ? !!identity.getUser(currentUserId)
                    : !!config; // getUser() unavailable: preserve prior fail-closed behavior rather than guessing
                const session = window.CozyOS.Session && typeof window.CozyOS.Session.current === "function"
                    ? window.CozyOS.Session.current() : null;
                const hasExternallyVerifiedSession = !!(session && session.source === "external"
                    && currentUserId && session.uid === currentUserId);
                if (!hasLocalIdentityRecord && hasExternallyVerifiedSession) {
                    onAuthenticated();
                    return;
                }
                if (container) {
