...
// ---------------------------------------------------------------
// LEVEL 2A — application-specific capability discovery
// ---------------------------------------------------------------

test('LEVEL 2A: an app with a real getNavigation() shows its real capability list as VERIFIED, never fabricated', async () => {
  const win = freshUserDashboard({ visibleApps: AUTHORIZED_APPS, applicationLauncher: makeFakeLauncher() });
  win.CozyOS.Modules = {
    quarryos: { getNavigation: () => [{ id: 'dashboard', label: 'Dashboard' }, { id: 'orders', label: 'Orders' }] },
  };
  const container = new FakeElement('div');
  await win.CozyOS.UserDashboard.render(container, 'user-1');

  container.querySelector('[data-drawer-toggle-app="quarryos"]').click();
  const capHost = container.querySelector('[data-drawer-app-capabilities="quarryos"]');
  assert.match(capHost.textContent, /Dashboard/);
  assert.match(capHost.textContent, /Orders/);
});

test('LEVEL 2B: an app with a real launch path but no getNavigation() honestly discloses NOT_IMPLEMENTED (openable, no breakdown), never invents capability items', async () => {
  const win = freshUserDashboard({ visibleApps: AUTHORIZED_APPS, applicationLauncher: makeFakeLauncher() });
  win.CozyOS.Modules = {}; // quarryos exposes nothing real here
  win.CozyOS.ApplicationVisibility.getRealLaunchPath = () => 'applications/QuarryOS/quarry.html'; // real entry point exists
  const container = new FakeElement('div');
  await win.CozyOS.UserDashboard.render(container, 'user-1');

  container.querySelector('[data-drawer-toggle-app="quarryos"]').click();
  const capHost = container.querySelector('[data-drawer-app-capabilities="quarryos"]');
  assert.match(capHost.textContent, /does not expose a structured capability/i);
  assert.match(capHost.textContent, /can be opened/i);
});

test('LEVEL 2B: an app with neither getNavigation() nor a real launch path is honestly classified BLOCKED, not NOT_IMPLEMENTED', async () => {
  const win = freshUserDashboard({ visibleApps: AUTHORIZED_APPS, applicationLauncher: makeFakeLauncher() });
  win.CozyOS.Modules = {};
  win.CozyOS.ApplicationVisibility.getRealLaunchPath = () => null; // no real entry point registered
  const container = new FakeElement('div');
  await win.CozyOS.UserDashboard.render(container, 'user-1');

  container.querySelector('[data-drawer-toggle-app="quarryos"]').click();
  const capHost = container.querySelector('[data-drawer-app-capabilities="quarryos"]');
  assert.match(capHost.textContent, /No real, resolvable launch destination/i);
});

test('LEVEL 2A: a getNavigation() that throws is treated as NOT_IMPLEMENTED, never crashes the drawer', async () => {
  const win = freshUserDashboard({ visibleApps: AUTHORIZED_APPS, applicationLauncher: makeFakeLauncher() });
  win.CozyOS.Modules = { quarryos: { getNavigation: () => { throw new Error('boom'); } } };
  const container = new FakeElement('div');
  await win.CozyOS.UserDashboard.render(container, 'user-1');

  container.querySelector('[data-drawer-toggle-app="quarryos"]').click();
  const capHost = container.querySelector('[data-drawer-app-capabilities="quarryos"]');
  assert.match(capHost.textContent, /getNavigation\(\) threw/);
});

test('LEVEL 2A: capability discovery never calls ApplicationLauncher — pure read of the app\'s own real module', async () => {
  const launcher = makeFakeLauncher();
  const win = freshUserDashboard({ visibleApps: AUTHORIZED_APPS, applicationLauncher: launcher });
  win.CozyOS.Modules = { quarryos: { getNavigation: () => [{ id: 'dashboard', label: 'Dashboard' }] } };
  const container = new FakeElement('div');
  await win.CozyOS.UserDashboard.render(container, 'user-1');

  container.querySelector('[data-drawer-toggle-app="quarryos"]').click();
  assert.deepEqual(launcher.calls.open, []);
  assert.deepEqual(launcher.calls.close, []);
});

test('LEVEL 2B: clicking a real capability item calls the same real ApplicationLauncher.open() — never a fabricated deep link', async () => {
  const launcher = makeFakeLauncher();
  const win = freshUserDashboard({ visibleApps: AUTHORIZED_APPS, applicationLauncher: launcher });
  win.CozyOS.Modules = { quarryos: { getNavigation: () => [{ id: 'dashboard', label: 'Dashboard' }, { id: 'orders', label: 'Orders' }] } };
  const container = new FakeElement('div');
  await win.CozyOS.UserDashboard.render(container, 'user-1');

  container.querySelector('[data-drawer-toggle-app="quarryos"]').click();
  const capButtons = container.querySelectorAll('[data-drawer-open-capability="quarryos"]');
  assert.equal(capButtons.length, 2);
  capButtons[1].click();
  await flushMicrotasks();
  assert.deepEqual(launcher.calls.open, ['quarryos']);
});

test('LEVEL 2A: an unauthorized application never has a capability panel to inspect at all', async () => {
  const win = freshUserDashboard({ visibleApps: { available: true, applications: [] }, applicationLauncher: makeFakeLauncher() });
  win.CozyOS.Modules = { quarryos: { getNavigation: () => [{ id: 'dashboard', label: 'Dashboard' }] } };
  const container = new FakeElement('div');
  await win.CozyOS.UserDashboard.render(container, 'user-1');

  assert.equal(container.querySelector('[data-drawer-app-capabilities="quarryos"]'), null);
});
