...
test('F2: real-device regression — on small screens (<=480px), the max-width cap must NOT be able to force the box narrower than its real content (real clipping bug: "ABOVE ONLY" rendered as "ABOVE ONI" on a real ~400px-wide phone)', () => {
  const css = read('core/shell/launch-sequence.css');
  const mqStart = css.indexOf('@media (max-width: 480px)');
  assert.ok(mqStart !== -1, 'the existing small-screen media query must still exist');
  const mqBlock = css.slice(mqStart, css.indexOf('@media (prefers-reduced-motion: reduce)', mqStart));
  assert.match(mqBlock, /#cozy-launch-above-only/, 'the small-screen override for this element must still exist');
  // The base rule's max-width:min(75vw,900px) can itself be smaller
  // than the real rendered text at small viewports (e.g. 75vw=300px at
  // a 400px-wide phone, while the text needs ~330px+ there) — width:
  // max-content alone does not help if max-width still clamps the box
  // narrower than that. The small-screen override must neutralize the
  // cap entirely so width:max-content is the only thing sizing the box.
  assert.match(mqBlock, /max-width:\s*none/, 'small screens must not inherit a max-width that can be narrower than the real rendered content');
});

test('F3: computed effective max-width at a real ~400px device viewport is never narrower than a conservative estimate of the real rendered text', () => {
  // Pure arithmetic check of the same real numbers the CSS declares -
  // not a browser layout engine, but enough to catch the exact class of
  // regression that shipped here (a numeric cap smaller than realistic
  // content width at the breakpoint's own font-size).
  const VIEWPORT_PX = 400;
  const FONT_SIZE_PX = Math.min(64, Math.max(26, 0.15 * VIEWPORT_PX)); // clamp(26px, 15vw, 64px)
  const TEXT = 'ABOVE ONLY';
  const CONSERVATIVE_CHAR_WIDTH_EM = 0.55; // a real bold serif face at this weight; deliberately not a tight/generous estimate
  const estimatedTextWidthPx = TEXT.length * FONT_SIZE_PX * CONSERVATIVE_CHAR_WIDTH_EM;

  const baseMaxWidthPx = Math.min(0.75 * VIEWPORT_PX, 900); // the base rule, if it applied unmodified
  assert.ok(baseMaxWidthPx < estimatedTextWidthPx, 'sanity: confirms the base max-width really would have been narrower than the text at this real viewport (proves the bug was real, not hypothetical)');

  // The fix: small-screen max-width is "none", i.e. effectively
  // unbounded — never narrower than any real content width.
...
test('Single-Voice: Owner Voice (default, no override) speaks welcome/above-only/motto ALL via the real recordings — never reaching CozySpeech/TTS for any of the three owner phrases', async (t) => {
  mock.timers.enable({ apis: ['setTimeout', 'setInterval', 'Date'] });
  t.after(() => mock.timers.reset());
  ...
  // OWNER VOICE LOGIN-SEQUENCE RECORDING (2026-09-15): a real motto.m4a
  // now exists (the new, human-verified owner recording of "Built for
  // Africa. Ready for the World."), registered exactly like "welcome"
  // and "above-only" already were. The hard requirement is now "Do NOT
  // use TTS even if an audio asset fails to load" for the owner
  // introduction, so all three phrases must be served by LivingSounds
  // and NONE of them may ever reach CozySpeech/TTS in the Owner Voice
  // path - superseding the prior (now removed) disclosed-fallback
  // behavior from when no real motto recording existed yet.
  assert.ok(livingSoundsCalls.includes('welcome'));
  assert.ok(livingSoundsCalls.includes('above-only'));
  assert.ok(livingSoundsCalls.includes('motto'), 'motto must now also be served by the real LivingSounds recording, not CozySpeech');
  const mottoCall = speechCalls.find((c) => c.context === 'motto');
  assert.ok(!mottoCall, 'motto must NOT reach CozySpeech/TTS now that a real Owner recording exists - no TTS fallback for the owner introduction, ever');
});
...
test('Single-Voice: existing ABOVE ONLY stage timing is completely unchanged by voice-selection logic (still 10s/9s); the ~20s design target and fixed breathing pause are also unaffected', () => {
  const { sandbox } = buildSandbox();
  const exported = runLaunchSequence(sandbox);
  assert.equal(exported.STARTUP_TIMING.TOTAL_DURATION_MS, 20000, 'design-target reference value, no longer used to gate a hold');
  assert.equal(exported.STARTUP_TIMING.BREATHING_MS, 2200);
...
test('U: AUTHORIZED REBUILD — the artificial "hold until TOTAL_DURATION_MS" is gone; the final pause is now the fixed BREATHING_MS regardless of real elapsed time', () => {
  const { sandbox } = buildSandbox();
  const exported = runLaunchSequence(sandbox);
  assert.equal(exported.STARTUP_TIMING.TOTAL_DURATION_MS, 20000, '~20s is a design-target reference value only, never a hold floor');
  assert.equal(exported.STARTUP_TIMING.BREATHING_MS, 2200);
