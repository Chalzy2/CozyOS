...
(function () {
    ...
    class CozyApplicationVisibility {
        ...
        /**
         * getRealLaunchPath(appId)
         *   M365.0 addition — the single, shared source for each real
         *   business application's actual static entry point. Previously
         *   this exact 2-entry lookup lived only inline inside
         *   cozy-workspace.js's M364.8 "Open" click handler; consolidated
         *   here so both the Administrator Application Center and the
         *   User Dashboard call the same real data instead of each
         *   keeping their own copy.
         *
         *   PROMPT-3 CORRECTION (registry reconciliation): this used to be
         *   a hardcoded 2-entry map (shopos, quarry_manager_001) with a
         *   comment claiming MpesaOS was "intentionally absent." That
         *   claim did not match the repository: mpesaOS-engine.js already
         *   registers a real `entryPoint: "applications/MpesaOS/index.html"`
         *   with ServiceRegistry, and that file genuinely exists on disk.
         *   The hardcoded map was simply never updated when MpesaOS added
         *   its real entry point — exactly the "const apps = [...]"
         *   anti-pattern this file's own header warns against elsewhere.
         *
         *   Now reads the live ServiceRegistry record's own `entryPoint`
         *   field instead of a second, dashboard-local list. Any
         *   application that self-registers a real entryPoint (present or
         *   future — ChurchOS, WholesaleOS, or anything else) becomes
         *   launchable automatically, with no dashboard file edit
         *   required. An application that never set entryPoint (ChurchOS,
         *   WholesaleOS today) still, correctly and honestly, returns
         *   null — that is real missing data, not a fixable oversight
         *   here.
         */
        getRealLaunchPath(appId) {
            const registry = window.CozyOS.ServiceRegistry;
            if (!registry || typeof registry.getApplication !== "function") return null;
            let manifest;
            try { manifest = registry.getApplication(appId); } catch (_err) { manifest = null; }
            if (!manifest || typeof manifest.entryPoint !== "string" || !manifest.entryPoint.trim()) return null;
    ...
    if (window.CozyOS.ServiceRegistry && typeof window.CozyOS.ServiceRegistry.registerCoordinator === "function") {
        try {
            window.CozyOS.ServiceRegistry.registerCoordinator({ sourcePath: "core/platform/application-visibility.js",
