...
(function () {
    ...
    // Feature IDs follow the exact naming convention already established in
    // core/modules/entitlement/tests/entitlement-engine.test.js ("mpesa",
    // "mpesa.receipts", "mpesa.calculations", "mpesa.transactions",
    // "mpesa.reports") — reused verbatim rather than inventing a second
    // "mpesaos.*" scheme.
    //
    // Scope of THIS integration pass: the real receipt-write boundary only.
    // Transactions and calculations are deliberately left unguarded here —
    // both are REQUIRED by business definition (a licensing/entitlement
    // subsystem being absent or misconfigured must never stop money from
    // being recorded), and gating them would risk exactly the regression
    // "preserve existing MpesaOS behavior" warns against. They are still
    // registered below as REQUIRED features so the APPLICATION -> FUNCTION
    // hierarchy model is real and complete (an admin panel built later can
    // show "Calculations: Required" / "Transactions: Required" honestly),
    // without adding any new failure mode to the money-movement path today.
    const MPESA_FEATURE_APP = "mpesa";
    const MPESA_FEATURE_TRANSACTIONS = "mpesa.transactions";
    const MPESA_FEATURE_CALCULATIONS = "mpesa.calculations";
    const MPESA_FEATURE_RECEIPTS = "mpesa.receipts";

    function getEntitlementEngine() {
        return (window.CozyOS && window.CozyOS.Entitlement) || null;
    }

    /** Idempotent — registerRequiredFeature() is itself a Set.add(). Called lazily (not at module load) because script load order between mpesaOS.js and entitlement-engine.js is not guaranteed. */
    function ensureMpesaRequiredFeaturesRegistered(entitlement) {
        if (typeof entitlement.isRequiredFeature !== "function" || typeof entitlement.registerRequiredFeature !== "function") return;
        if (!entitlement.isRequiredFeature(MPESA_FEATURE_TRANSACTIONS)) entitlement.registerRequiredFeature(MPESA_FEATURE_TRANSACTIONS);
        if (!entitlement.isRequiredFeature(MPESA_FEATURE_CALCULATIONS)) entitlement.registerRequiredFeature(MPESA_FEATURE_CALCULATIONS);
    }
    ...
    class CozyBusinessEngine {
        ...
        /**
         * _generateReceipt(tenantId, { internalTxId, timestamp })
         *   The single, real receipt-write operation for MpesaOS. Both the
         *   transactional txEngine path and the non-transactional fallback
         *   path (see processAutomatedWorkflow below) call this one method
         *   — there is exactly one receipt-writing operation in this file,
         *   not two independently-guarded copies. The entitlement check
         *   lives INSIDE this method, not only at its call sites, so a
         *   caller that invokes it directly (bypassing
         *   processAutomatedWorkflow's own orchestration) still cannot
         *   skip the guard — there is no unguarded path to a receipt
         *   write anywhere in this file.
         *
         *   Application-level ("mpesa") is checked before the function-
         *   level ("mpesa.receipts") state — an ADMIN_DISABLED/
         *   PLAN_RESTRICTED application makes receipts (and every other
         *   child function) unavailable before the function's own
         *   override/plan state is even consulted. This composes two
         *   real guard() calls at the call site; EntitlementEngine's own
         *   merge logic is untouched, so there remains exactly one
         *   permission authority.
         *
         *   Fails closed and returns null (never throws) when receipts
         *   are not entitled for any reason — ADMIN_DISABLED, MUTED,
         *   PLAN_RESTRICTED, or Entitlement being completely
         *   unavailable/unwired. A receipt is a downstream, optional
         *   artifact of a transaction, never the other way around, so
         *   its denial is recorded and swallowed here rather than
         *   thrown — the caller's REQUIRED transaction commit must
         *   never be blocked by an optional receipt failing to write.
         */
        _generateReceipt(tenantId, { internalTxId, timestamp }) {
            const entitlement = getEntitlementEngine();
            if (!entitlement || typeof entitlement.guard !== "function") {
                this._logAudit("RECEIPT_MUTED", `${internalTxId}: Entitlement engine unavailable — failing closed on receipts only.`);
                return null;
            }
            ensureMpesaRequiredFeaturesRegistered(entitlement);
            try {
                entitlement.guard(tenantId, MPESA_FEATURE_APP);
                entitlement.guard(tenantId, MPESA_FEATURE_RECEIPTS);
            } catch (err) {
                const state = (err && err.decision && err.decision.state) || "DENIED";
                this._logAudit("RECEIPT_MUTED", `${internalTxId}: receipts not entitled (${state}).`);
                return null;
            }
            return { id: generateId("rec"), txId: internalTxId, receiptNumber: internalTxId, generatedAt: timestamp };
        }
        ...
        async processAutomatedWorkflow(rawAction, tenantId) {
            ...
            try {
                ...
                // Real entitlement check for the receipt sub-feature only —
                // computed once, before either write path, so both branches
                // below make the exact same decision. Never affects
                // ledgerBlock.auditHash (already sealed above) or the
                // transaction itself; see _generateReceipt() for the guard.
                const receiptRecord = this._generateReceipt(tenantId, { internalTxId, timestamp });
                ledgerBlock.receiptStatus = receiptRecord ? "GENERATED" : "MUTED";
                ...
                if (typeof storage.beginTransaction === "function") {
                    // "receipts" only enters the participating-store list
                    // when the receipt was actually entitled — muting
                    // receipts must not mute (or even touch) any other
                    // store in this transaction, including "transactions"
                    // itself, which remains REQUIRED and unaffected.
                    const storesToTouch = [
                        "customer_history", "transactions", "daily_register",
                        "commissions", "analytics", "offline_queue"
                    ];
                    if (receiptRecord) storesToTouch.splice(5, 0, "receipts");
                    const txEngine = await storage.beginTransaction(storesToTouch, tenantId);
                    ...
                    try {
                        ...
                        if (receiptRecord) await txEngine.save("receipts", receiptRecord);
                    ...
                    const writeResults = await Promise.allSettled([
                        ...
                        receiptRecord ? storage.save("receipts", receiptRecord, tenantId) : Promise.resolve(null),
    ...
    async function mpesaExecutionCore(query, kernelContext) {
        ...
        try {
            if (cleanQuery.includes("run_automated_workflow") || cleanQuery === "execute workflow") {
                ...
                const receiptClause = simulatedResult.receiptStatus === "MUTED" ? "Receipt Muted (Entitlement)" : "Receipt Generated";
                return {
                    responseText: `🪪 [CozyOS BusinessOS Automated Pipeline Passed]\n• Action Taken: Serve Customer Withdrawal\n• Automated Tasks Completed: Customer Profile Cached, Transaction Logged, Daily Register Appended, Commission Accrued (KES ${simulatedResult.commission}), ${receiptClause}, Cryptographic Non-Repudiation Audit Hash Sealed: ${simulatedResult.auditHash}\n🔒 All tables updated via core/storage.js simultaneously without any secondary inputs.`
                };
            }
    ...
    function initRegistration() {
        ...
        if (window.CozyOS && window.CozyOS.PluginManager) {
            window.CozyOS.PluginManager.register(
                manifest,
                PluginManager.createMinimalIntentHandler(
                    mpesaExecutionCore,
                    "MpesaOS"
                )
            );
