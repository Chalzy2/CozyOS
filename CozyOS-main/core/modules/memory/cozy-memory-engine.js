 ...
 *   - Retrieval boundary (Milestone E): listKeys(), searchMemory(),
 *     searchAllNamespaces(), and tagSearch() now take the same optional
 *     actorId that readMemory()/recall() always took, and enforce the
 *     same real #checkReadVisibility() gate. Previously only
 *     readMemory()/recall() enforced it — the other four read paths
 *     returned every entry regardless of its visibility, so a private
 *     entry (visibility defaults to "private" in saveMemory()) was
 *     unreachable via readMemory() but still fully listed/searchable.
 *     actorId defaults to "system" everywhere, so every pre-existing
 *     call site that never passed one behaves exactly as before.
...
(function () {
    ...
    class CozyOSMemoryEngine {
        ...
        /**
         * listKeys(namespace, predicate, actorId)
         *   actorId is optional and defaults to "system" — every existing
         *   call site that doesn't pass it continues to behave exactly as
         *   before (unconditional listing), since "system" always passes
         *   the real visibility check. New callers that want real
         *   enforcement pass a real actorId. Fixes a real gap (Milestone
         *   E): this was the one read path that bypassed
         *   #checkReadVisibility entirely, so a private entry — private
         *   is the real default visibility set by saveMemory() — was
         *   listable by any caller even though readMemory() already
         *   refused to return it directly.
         */
        listKeys(namespace, predicate, actorId = "system") {
            const map = this.#ns(namespace);
            const list = Array.from(map.entries())
                .filter(([, entry]) => this.#checkReadVisibility(entry, actorId))
                .map(([key, entry]) => ({ key, ...this.#deepClone(entry.current) }));
            return Object.freeze(predicate ? list.filter(predicate) : list);
        }
        ...
        /**
         * searchMemory(namespace, query, actorId)
         *   Real full-text/keyword search across a namespace's current
         *   values and tags. actorId is optional and defaults to
         *   "system" — every existing call site that doesn't pass it
         *   continues to behave exactly as before. New callers that want
         *   real enforcement pass a real actorId; a private entry (the
         *   real default visibility) is now excluded from results for
         *   any other actorId, same rule readMemory() already enforces.
         */
        searchMemory(namespace, query, actorId = "system") {
        ...
        /** searchAllNamespaces() — real search fan-out across every namespace, tagged with which one each hit came from. actorId optional, defaults to "system" (see searchMemory()). */
        searchAllNamespaces(query, actorId = "system") {
            const all = [];
            for (const namespace of this.#namespaces.keys()) {
                for (const r of this.searchMemory(namespace, query, actorId)) all.push({ namespace, ...r });
            }
        ...
        tagSearch(namespace, tag, actorId = "system") {
            return this.listKeys(namespace, e => e.tags.includes(tag), actorId);
        }
    ...
    })({ sourcePath: "core/modules/memory/cozy-memory-engine.js",
