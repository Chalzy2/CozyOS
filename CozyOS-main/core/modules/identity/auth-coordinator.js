...
(function () {
    ...
    class CozyOSAuthCoordinator {
         ...
         */
        // REAL FIX (username sign-in was silently broken): this method's
        // first parameter is whatever the visitor typed into a
        // "Username or Email" field — login.html's ordinary form and
        // cozy-login-gate.js's Administrator form both already label it
        // that way — but every call site here used to unconditionally
        // send it as {email: identifier}. A real username (e.g.
        // "Chalzcozy", set server-side via bootstrap-admin.js
        // set-username) has no "@" in it, so the server's real
        // WHERE email = ? lookup always missed and login silently failed
        // with a generic invalid-credentials message. The server's real
        // POST /auth/login already accepts EITHER {email} OR {username}
        // (see server/webauthn-rp/server.js /
        // rp.authenticateWithPassword()) — this is the one place on the
        // client that must pick the right one. A bare "@" check is
        // sufficient and matches the server's own identifier contract:
        // it never guesses or sends both (the server explicitly rejects
        // that as identifier_ambiguous).
        async loginWithServerPassword(identifier, password, { rememberMe = true } = {}) {
            ...
            const requestBody = looksLikeEmail ? { email: identifier, password } : { username: identifier, password };
            ...
            // NOTE: the server's success response deliberately does not
            // echo back the resolved canonical email (see server.js's
            // POST /auth/login handler) — only {ok, isPlatformAdmin}.
            // isPlatformAdmin above is the one real, server-authoritative
            // value this method trusts. `identifier` here (whatever the
            // visitor actually typed — email OR username) is used only
            // as a local, non-authoritative display/pointer identity,
            // exactly as the pre-existing email-only code already did
            // with the email case.
            return this.#finishServerLogin({ email: identifier, isPlatformAdmin: !!body.isPlatformAdmin, rememberMe: !!rememberMe });
        }
        ...
        async loginWithServerFirebase(email, password) {
            ...
            if (!signInResult.available || !signInResult.user) {
                this.#diagnostics.serverLoginFailures++;
                // DIAGNOSTIC FIX (M-ADMIN-FIREBASE-CODE): previously
                // discarded the real Firebase SDK error entirely and
                // always returned one generic message, making it
                // impossible to tell "wrong password" apart from "no such
                // Firebase user" or "misconfigured Firebase project" from
                // the UI alone. Classifies by signInResult.code — the
                // Firebase SDK's own short, documented, non-secret error
                // code (see firebase-auth.js) — into a safe, specific
                // human-readable label. Never forwards err.message
                // verbatim, never touches/logs the password, idToken, or
                // any cookie. Diagnostic only: still returns
                // available:false either way, changes no authentication
                // or authorization logic, and leaves the real
                // loginWithServerFirebase() -> /webauthn/firebase/session
                // -> cozy_admin_session flow below completely untouched.
                const code = signInResult.code || "auth/unknown-error";
                const FIREBASE_ERROR_LABELS = {
                    "auth/user-not-found": "No administrator account exists in Firebase for this email.",
                    "auth/wrong-password": "The password does not match this Firebase account.",
                    "auth/invalid-credential": "This email/password combination was rejected by Firebase.",
                    "auth/invalid-api-key": "Firebase is misconfigured for this deployment (invalid API key).",
                    "auth/network-request-failed": "Could not reach Firebase — check network connectivity.",
                    "auth/not-ready": "Firebase Auth has not finished loading in this browser session.",
                    "auth/too-many-requests": "Too many attempts — Firebase has temporarily locked this account.",
                    "auth/user-disabled": "This Firebase account has been disabled.",
                };
                const label = FIREBASE_ERROR_LABELS[code] || `Firebase rejected this sign-in (${code}).`;
                return { available: false, code, reason: label };
            }
        ...
        /**
         * completeServerLoginWithPasskeyMfa(pendingId, {rememberMe, email})
         *   The next Admin authentication step: completes an EXISTING
         *   password-verified pending-auth session (the same pendingId
         *   completeServerLoginWithOtp() above uses) with a real
         *   WebAuthn/passkey assertion instead of a typed TOTP code —
         *   the fingerprint-capable-authenticator path. Mirrors
         *   loginWithServerPasskey()'s exact real ceremony shape
         *   (POST .../begin -> navigator.credentials.get() ->
         *   POST .../complete), reusing the same fromB64url/toB64url
         *   helpers, but targets the new second-factor routes
         *   (/auth/mfa/webauthn/begin, /auth/mfa/webauthn/complete)
         *   rather than the first-factor passwordless ones — the server
         *   is still the sole verification authority; this method only
         *   relays its decision, exactly like completeServerLoginWithOtp()
         *   does for TOTP. Only on a real 200 does the server set the
         *   session cookie; #finishServerLogin() is called only after
         *   that has already happened.
         *
         *   `email` MUST be supplied by the caller for the same reason
         *   documented on completeServerLoginWithOtp() above — the
         *   server's MFA-completion responses intentionally return only
         *   isPlatformAdmin, never email.
         *
         *   Returns one of:
         *     { available:false, code:"invalid_challenge", reason }
         ...
         *     { available:false, code:<server AuthError code from /complete>, reason }
         *       — mfa_session_invalid, mfa_session_expired,
         *         mfa_session_cancelled, mfa_attempts_exceeded,
         *         invalid_mfa_code (the server intentionally returns
         *         this generic code for a failed WebAuthn attempt in
         *         the MFA context — see rp.js
         *         completePendingAuthWithWebAuthn() — never a detailed
         *         WebAuthn error here, unlike first-factor passkey login).
         *     { available:true, source:"server", email, isPlatformAdmin }
         */
        async completeServerLoginWithPasskeyMfa(pendingId, { rememberMe, email } = {}) {
            if (!pendingId) return { available: false, code: "invalid_challenge", reason: "Invalid or already-used authentication challenge. Please sign in again." };
            if (typeof fetch !== "function") {
                return { available: false, code: "client_error", reason: "This browser does not support the required network APIs." };
            }
            ...
            // ---- begin: real challenge/options scoped to this pending session ----
            let beginResponse;
            try {
                beginResponse = await fetch("/auth/mfa/webauthn/begin", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    credentials: "include",
                    body: JSON.stringify({ pendingId }),
            ...
            if (!beginResponse.ok || !beginBody || typeof beginBody.challenge !== "string") {
                const beginErrCode = (beginBody && beginBody.error) || "challenge_request_failed";
                if (beginErrCode === "no_passkeys_registered") {
                ...
                return { available: false, code: beginErrCode, reason: beginReasons[beginErrCode] || "Could not start passkey verification. Please try again." };
            }
            ...
            try {
                ...
                return {
                    ...
                    reason: cancelled ? "Passkey verification was cancelled." : `Real WebAuthn assertion failed: ${err.message}`,
                };
            }
            ...
            try {
                completeResponse = await fetch("/auth/mfa/webauthn/complete", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    credentials: "include",
            ...
            if (!completeResponse.ok || !completeJson || completeJson.ok !== true) {
                const errCode = (completeJson && completeJson.error) || "invalid_mfa_code";
                const reasons = {
                    invalid_mfa_code: "Passkey verification failed.",
                    mfa_attempts_exceeded: "Too many failed attempts. Please sign in again.",
                    mfa_session_expired: "This authentication attempt has expired. Please sign in again.",
                    mfa_session_cancelled: "This authentication attempt was cancelled. Please sign in again.",
                    mfa_session_invalid: "Invalid or already-used authentication challenge. Please sign in again.",
            ...
            const result = this.#finishServerLogin({ email, isPlatformAdmin: !!completeJson.isPlatformAdmin, rememberMe: !!rememberMe });
            return { ...result, method: "webauthn" };
        }
         ...
         *   establishment.
         *
         *   CORRECTED (was "DISCLOSED ARCHITECTURAL LIMIT"): earlier
         *   revisions of this comment claimed server/webauthn-rp/
         *   server.js gated both enrollment endpoints on
         *   `session.isPlatformAdmin`. Re-inspected against the live
         *   server code: neither POST /webauthn/passkeys/enroll/begin
         *   nor .../complete ever reads session.isPlatformAdmin — both
         *   only require `currentSession(req)` to resolve to *any*
         *   authenticated session, and self-service passkey enrollment
         *   for ordinary (non-admin) accounts already works end-to-end,
         *   covered by server/webauthn-rp/test/http-integration.test.js
         *   ("an ordinary authenticated (non-admin) user can self-enroll
         *   a passkey for their own account"). This method still does
         *   not bypass, relax, or duplicate the server's authorization
         *   decision — it forwards whatever the server actually returns.
         *   The `not_authenticated_admin` handling below is kept as
         *   defensive, honest error-forwarding (a real prior server
         *   error code, not removed just because live server code no
         *   longer emits it) rather than assumed to be unreachable.
         *
         *   Returns one of:
         *     { available:false, code:"webauthn_unavailable", reason, deviceUnavailable:true }
         ...
         */
        /**
         * #tryServerRestore(emailHint)
         *   The real, server-authoritative check (GET /webauthn/session,
         *   credentials:"include") shared by both real callers below:
         *   the pointer-based "server" branch (pointer.email is a
         *   non-secret local hint only, same as before this refactor —
         *   the server's own response is what's actually trusted) and
         *   the new pointer-less fallback added for the real, disclosed
         *   defect this method fixes: identical logic, one real owner,
         *   never a second implementation of the same check.
         */
        async #tryServerRestore(emailHint) {
            const session = this.#session();
            ...
            try {
                session.establishFromExternalAuth({
                    uid: body.email || emailHint,
                    roles: body.isPlatformAdmin ? ["platform-admin"] : [],
                    profile: { email: body.email || emailHint, authMode: "server-password", restored: true },
                });
            } catch (err) {
                this.#persistPointer(null);
                this.#diagnostics.restoreFailures++;
            ...
            return { restored: true, source: "server", email: body.email || emailHint };
        }
        ...
        async restoreSession() {
            ...
            if (!pointer) {
                // REAL BUG FIX (CHALZYDASHBOARD-ADMIN-HANDOFF): confirmed root
                // cause of "authenticated:true, isPlatformAdmin:true at the
                // server yet the client renders Signed in — Unknown userId".
                // The real server session lives in an httpOnly cookie and is
                // valid in EVERY tab/origin-context regardless of this
                // browser's local storage; the pointer this method used to
                // require first is a same-tab-or-device, non-authoritative
                // client hint (localStorage for rememberMe=true,
                // sessionStorage — per-tab only — for rememberMe=false; see
                // #persistPointer()'s own header). A brand-new tab, a
                // cleared/private-adjacent storage state, or simply
                // navigating straight to a bookmarked /chalzydashboard
                // legitimately has NO local pointer at all while the real
                // cookie session is still perfectly valid — and this method
                // used to give up before ever asking the server, which is
                // exactly backwards: the pointer was gatekeeping the one
                // check that is actually authoritative. This adds no new
                // authority and no new network call shape — it is the exact
                // same #tryServerRestore() the "server" pointer branch below
                // already performed, now also attempted, honestly, as a
                // last resort when there is nothing local to go on, rather
                // than failing closed on local absence alone. A genuinely
                // unauthenticated visitor still fails closed here: the
                // server call below still requires a real
                // authenticated:true response, same as always.
                const result = await this.#tryServerRestore(null);
                if (result.restored) return result;
                this.#diagnostics.restoreFailures++;
                return { restored: false, reason: "No persisted session pointer." };
            }
            ...
            if (pointer.source === "server") {
                ...
                // itself sufficient to re-establish a session. Delegates
                // to #tryServerRestore() — see that method's header and
                // the no-pointer branch above for why this is now shared.
                return this.#tryServerRestore(pointer.email);
            }
