...
(function () {
    ...
    class CozyOSIdentityEngine {
        ...
        assignApplication(userId, appName) {
            ...
            // Real fix (Level 2B repair): the established ServiceRegistry
            // canonical application-id convention already in real use
            // elsewhere in this repository (e.g. "pharmacyos_core_001",
            // "wholesaleos_core_001", "churchos_core_001",
            // "quarry_manager_001") includes underscores. This
            // validation previously rejected every one of them,
            // meaning no real user could ever be assigned any of those
            // real, registered applications through this authoritative
            // method. Minimum correction: underscore added to the
            // allowed character set. Intent preserved — still a plain,
            // safe identifier (letters, digits, hyphen, underscore
            // only), still rejects anything else (spaces, slashes,
            // dots, script-like content, etc.).
            if (typeof appName !== "string" || !/^[a-z0-9_-]+$/i.test(appName)) throw new TypeError("[Identity] assignApplication(): appName must be a simple alphanumeric/hyphen/underscore identifier.");
