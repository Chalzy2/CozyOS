...
(function () {
    ...
    /**
     * buildShopOSSkeleton()
     *   Module-scope, stateless — the exact same real markup already
     *   shipped in applications/ShopOS/shopos.html, shared by both the
     *   real instance's getDashboard() and the module wrapper's
     *   pre-singleton fallback below. Real fix (see the fix note on
     *   ShopOSModule.getDashboard() further down): ApplicationLauncher's
     *   real Mode 3 path calls the wrapper's getDashboard() BEFORE its
     *   init() ever runs, so singletonInstance is still null at that
     *   exact call - the skeleton must be available with no instance at
     *   all, not just once one exists.
     */
    function buildShopOSSkeleton() {
        return `
    <div class="sp-app" id="sp-app-root">
        <div class="sp-topbar">
            <div class="sp-brand-cluster">
                <div class="sp-brand-badge">S</div>
                <div>
                    <div class="sp-brand-name" data-i18n="app_name">ShopOS</div>
                    <div class="sp-brand-sub" data-i18n="app_sub">CozyOS Business Engine</div>
                </div>
            </div>
            <div class="sp-topbar-actions">
                <span class="sp-status-pill" id="sp-engine-status">● <span data-i18n="engine_ready">Ready</span></span>
            </div>
        </div>
        <nav class="sp-tabs" id="sp-tabs">
            <button class="sp-tab active" data-tab="dashboard" data-i18n="tab_dashboard">Dashboard</button>
            <button class="sp-tab" data-tab="products" data-i18n="tab_products">Products</button>
            <button class="sp-tab" data-tab="customers" data-i18n="tab_customers">Customers</button>
            <button class="sp-tab" data-tab="orders" data-i18n="tab_orders">Orders</button>
        </nav>
        <section class="sp-section" id="sp-adaptive-profile-section" style="display:none;" aria-label="Business profile">
            <h3 data-i18n="business_profile">Business Profile</h3>
            <div id="sp-adaptive-profile-content"></div>
        </section>
        <section class="sp-section" id="sp-no-branch-section" style="display:none;">
            <div class="sp-empty-state">
                <p data-i18n="no_branch_configured">No branch is configured for this shop yet.</p>
                <button class="sp-btn sp-btn-primary" id="sp-quick-setup-btn" data-i18n="quick_setup">Quick Setup</button>
            </div>
        </section>
        <main id="sp-tab-content"></main>
        <footer class="sp-footer">
            <span data-i18n="app_shell">ShopOS Application</span> &middot; CozyOS
        </footer>
    </div>`;
    }
    ...
    class ShopOSModule {
        ...
        async #ensureCoordinatorsLoaded() {
            ...
            await Promise.all(missing.map(([, file]) => new Promise((resolve) => {
                ...
                // Real fix (Level 1 dependency-first discovery, browser-
                // confirmed via 404s on http://host/plugins/<file>): the
                // real coordinator files live at core/plugins/<file>
                // (confirmed by this same file's own header comment and
                // by core/plugins/shopOS-product.js etc. actually
                // existing on disk there) - "plugins/" alone does not
                // exist anywhere in this repository. "../../" resolves
                // correctly relative to either real host location for
                // this module (dashboard.html at the repo root, where
                // the extra ".." segments are harmlessly clamped at
                // root, or the standalone applications/ShopOS/
                // shopos.html, exactly two directories deep) - only the
                // missing "core/" segment was ever wrong.
                script.src = `../../core/plugins/${file}`;
         ...
         *   Real fix (Level 1 dependency-first discovery): the
         *   authoritative loader contract this method must satisfy is
         *   core/shell/application-launcher.js's real Mode 3 path
         *   (`container.innerHTML = jsModule.getDashboard()`) - the
         *   exact same contract core/modules/developer/developer-hub.js
         *   and core/modules/Cozy-Authenticator/authenticator.js's own
         *   `getDashboard() { return '<div id="...")"></div>'; }`
         *   already honor: a real HTML string, always. This was
         *   previously returning a structured data object instead -
         *   confirmed via real-browser testing to render as the literal
         *   text "[object Object]" once ApplicationLauncher's real,
         *   existing, unmodified String-coercion assignment ran, and to
         *   then make init()'s real #render() throw immediately after
         *   (it queries #sp-no-branch-section/#sp-tabs/#sp-tab-content
         *   by real id, none of which the object-returning version ever
         *   provided) - the true reason ApplicationLauncher never
         *   reached this.#instances.set() and isOpen("shopos") stayed
         *   false.
         *
         *   Real fix: return the exact same real skeleton markup
         *   already shipped in applications/ShopOS/shopos.html (the
         *   file this module was always written against - #render(),
         *   #renderTabContent(), and every #sp-* id above reference it
         *   verbatim), rather than inventing new markup or any
         *   business/branch/product data. init()'s own real #render()
         *   fills #sp-tab-content and toggles #sp-no-branch-section
         *   exactly as before - nothing about that logic changes here.
         */
        getDashboard() {
            return buildShopOSSkeleton();
        }

        /**
         * getDashboardData(branchId)
         *   The real structured-data shape this method used to expose
         *   as getDashboard() (see the fix note above) - preserved
         *   under its own name rather than deleted, for the same "any
         *   future non-HTML consumer" real use case its own prior
         *   comment described. No other real code in this repository
         *   currently calls it (confirmed by search) - an honest,
         *   currently-unused-but-not-fabricated capability, not a
         *   duplicate engine.
         */
        getDashboardData(branchId = this.#branchId) {
            const dash = window.CozyOS.ShopDashboard;
    ...
    if (window.CozyOS.Modules && window.CozyOS.Modules["shopos"] && window.CozyOS.Modules["shopos"].version) {
        ...
        window.CozyOS.Modules["shopos"] = {
            ...
            getDashboard() { return singletonInstance ? singletonInstance.getDashboard() : buildShopOSSkeleton(); },
            getDashboardData(branchId) { return singletonInstance ? singletonInstance.getDashboardData(branchId) : { available: false, reason: "Not initialized." }; },
    ...
    // Real fix: this file previously also called ModuleRegistry.register()
    // for "shopos" here, duplicating what module-registry.js already
    // registers at its own initialization (confirmed the documented
    // single source of truth for this exact id). The duplicate call
    // was always silently rejected (caught, non-fatal) and only visible
    // via ModuleRegistry's own audit log as "REGISTRATION_REJECTED:
    // shopos: Duplicate id" - removed rather than left as dead,
    // always-failing code.
})();
