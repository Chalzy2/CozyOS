 ...
 *
 * v1.1.0 (M372 Layer 2): analyzeCode()/analyzeRepository() additionally
 * extract exportedAs, dependsOnGlobals, and eventsListened per file — the
 * same regex-only, non-executing discipline as every other field. This is
 * the entire M372 Layer 2 change to this file; all cross-file reasoning
 * (module/dependency/API/event/ownership graphs) is composed in the new,
 * separate core/modules/builder/layer2-graph-composer.js, which owns none
 * of this file's responsibilities and re-parses nothing.
 *
 * RESPONSIBILITY
...
(function () {
    ...
    const ENGINE_VERSION = "1.1.0-ENTERPRISE";
    // v1.1.0 (M372 Layer 2): analyzeCode()/analyzeRepository() gained three
    // additive per-file fields — exportedAs, dependsOnGlobals, eventsListened
    // — real regex extraction of facts already visible in source, same
    // non-execution discipline as every other field here. Nothing else in
    // this file changed. This is the ONLY interface change Layer 2 needed:
    // all cross-file graph-building lives in core/modules/builder/
    // layer2-graph-composer.js, which composes these new fields rather than
    // re-parsing source. See docs/builder/reports/layer2-compose-analysis-
    // AA-001.md for why the graph logic does not live in this file.
    ...
    class CozyOSUnderstandingEngine {
        ...
        analyzeCode(sourceText) {
            ...
            // v1.1.0 additions (M372 Layer 2) — same regex-only, non-
            // executing discipline as every field above. This codebase does
            // not use require()/import for internal wiring (verified: zero
            // matches repo-wide for internal modules); its real dependency
            // idiom, used consistently across every file read for this
            // milestone, is a live read of window.CozyOS.<Name>. That is
            // the signal extracted here — a raw reference list, not a
            // resolved/validated dependency claim.
            const exportMatch = /window\.CozyOS\.([A-Za-z0-9_]+)\s*=\s*new\s+[A-Za-z0-9_$]+\s*\(/.exec(sourceText);
            const globalRefMatches = Array.from(sourceText.matchAll(/window\.CozyOS\.([A-Za-z0-9_]+)/g)).map(m => m[1]);
            const exportedAs = exportMatch ? exportMatch[1] : null;
            const IGNORED_GLOBAL_REFS = new Set(["registerCoordinator", "__pendingCoordinatorRegistrations"]);
            const dependsOnGlobals = Array.from(new Set(globalRefMatches)).filter(name => name !== exportedAs && !IGNORED_GLOBAL_REFS.has(name));
            const listenerMatches = Array.from(sourceText.matchAll(/\.on\(\s*["']([a-zA-Z0-9_:.-]+)["']/g)).map(m => m[1]);
            ...
            const result = {
                ...
                exportedAs,
                dependsOnGlobals,
                eventsListened: Array.from(new Set(listenerMatches))
            };
    ...
    })({ sourcePath: "core/modules/builder/understanding-engine.js",
        name: "UnderstandingEngine", category: "Code Generation", icon: "understanding-engine.svg",
        description: "CozyBuilder's Understanding Engine — provider-based text/code/PDF/image/screenshot/repository analysis, a deterministic Requirement Gap Detector, and a human-reviewed Knowledge Review Queue. Never auto-learns. v1.1.0 adds per-file exportedAs/dependsOnGlobals/eventsListened extraction, composed (not duplicated) by layer2-graph-composer.js."
    });
})();
