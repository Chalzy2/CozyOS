...
test('real path: SpeechLanguageAdapter.resolve("sw") succeeds against the real registry (not a fake), returning the registry\'s own value', () => {
    const CozyOS = loadRealChain(makeFakeApi());
    const resolved = CozyOS.SpeechLanguageAdapter.resolve('sw');
    assert.equal(resolved.success, true);
    // Kiswahili Capability Dependency #1 (STT locale qualification):
    // the real registry now genuinely defines bcp47Tag:'sw-KE' for
    // Kiswahili (previously absent - this assertion's own prior comment
    // explicitly anticipated this as a real, deliberate future addition,
    // not an invented one). Every other language without an explicit
    // override still resolves to its own bare languageCode, unchanged -
    // see the REGRESSION tests in
    // kiswahili-dep1-stt-locale-qualification.test.js.
    assert.equal(resolved.bcp47, 'sw-KE', 'the real registry now genuinely defines a real, deliberate bcp47Tag for "sw" (Kiswahili Capability Dependency #1) - the honest resolved value is that registry value, not an invented one');
});
...
test('full real path: Living Hearing config { languageCode:"sw", continuous:true, interimResults:true } reaches the real recognition instance exactly as validated by the real registry chain', () => {
    ...
    assert.equal(Api.instances[0].lang, 'sw-KE', 'the language actually set on the real recognition instance must be the real registry\'s resolved value (Kiswahili Capability Dependency #1: sw-KE), not the bare pre-dependency code and not a raw/guessed string');
