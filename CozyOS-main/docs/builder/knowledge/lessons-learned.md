...
- **"Bootstrap" is not one thing in this repo — check which environment it assumes before extending it.** `core/bootstrap/bootstrap.js` (used by `chalzydashboard.html`) is entirely browser-scoped: `window`, `document`, `navigator`, `location`. It cannot run under plain Node.js/Termux, and nothing in this repository provided a Node-side runtime entry point before CP5. Confirmed by reading the file, not assumed from its name — first hit when starting CP5 (Android/Termux runtime foundation). A future session asked to "extend the bootstrap" for any non-browser target should re-verify which one is meant rather than assuming the browser one applies.
- **Reuse a plain-Node CP4 server as a black box rather than re-deriving its config.** `server/static-boundary-server.js`'s `createBoundaryServer()` has zero browser dependency and was already fully usable as the local Workspace launch target for CP5's Termux/Android runtime — no new HTTP/static-serving/auth code was needed, only a local-only config (loopback host, runtime-local sqlite path, `localhost` RP id/origin) and a health-gated start/stop lifecycle around it.
- **A health check must never execute the thing it's checking.** CP5's `checkModuleHealth()` uses `require.resolve()`, not `require()`, specifically so that checking whether a dependency is loadable can never itself trigger that dependency's side effects (e.g. opening a database, binding a port) as a side effect of asking "is it safe to start?".
