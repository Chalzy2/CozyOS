...
// /dashboard.html are ordinary, ungated User Dashboard routes again —
// NOT protected admin routes. return-destination-core.js's
// ALLOWED_DESTINATIONS no longer contains them, so an unauthenticated
// visit to /dashboard must NOT be treated as a protected-route deep
// link at all: there is no gate page at that URL to bounce back to,
// so login.html's ordinary index.html default is correct, exactly as
// it is for any other public page. These two tests replace the old
// "preserve /dashboard as a protected return path" regression tests,
// which encoded the now-reverted behavior.
test('7. unauthenticated /dashboard: NOT treated as a protected admin return path (routing reversion)', async () => {
  const CozyOS = loadRealClientModules();
  const redirect = computeChalzyRedirect(CozyOS, '/dashboard');
  assert.equal(redirect, 'login.html', '/dashboard must not survive as a ?return= value anymore');

  assert.equal(resolvePostLoginDestination(CozyOS, ''), 'index.html');
});

test('8. unauthenticated /dashboard.html: NOT treated as a protected admin return path (routing reversion)', async () => {
  const CozyOS = loadRealClientModules();
  const redirect = computeChalzyRedirect(CozyOS, '/dashboard.html');
  assert.equal(redirect, 'login.html', '/dashboard.html must not survive as a ?return= value anymore');
});

test('9. real admin session (no /dashboard involvement): gate resolves PLATFORM via /chalzydashboard exactly as before', async () => {
...
test('10. real non-admin session (no /dashboard involvement): gate resolves DENIED, never PLATFORM', async () => {
  await withServer(async ({ base, rp }) => {
