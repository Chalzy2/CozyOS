...
function createServer({
  ...
  const routes = {
    ...
    // Mirrors every organization route above exactly, minus
    // organizationId (never accepted here — a personal document's
    // owner is always session.userId, never a client-supplied value).
    // ---------------------------------------------------------------

    'POST /documents/personal': async (req, res) => {
      const session = await currentSession(req);
      if (!session) return sendJson(res, 401, { error: 'not_authenticated' });
      const body = await readJsonBody(req, 10 * 1024 * 1024);
      const result = await documentStorage.savePersonal(session.userId, body.record || {});
      return sendJson(res, 200, result);
    },

    'POST /documents/personal/load': async (req, res) => {
      const session = await currentSession(req);
      if (!session) return sendJson(res, 401, { error: 'not_authenticated' });
      const body = await readJsonBody(req);
      if (!body.documentId) return sendJson(res, 400, { error: 'documentId_required' });
      const result = await documentStorage.loadPersonal(session.userId, body.documentId);
      return sendJson(res, result.available ? 200 : 404, result);
    },

    'POST /documents/personal/archive': async (req, res) => {
      const session = await currentSession(req);
      if (!session) return sendJson(res, 401, { error: 'not_authenticated' });
      const body = await readJsonBody(req);
      if (!body.documentId) return sendJson(res, 400, { error: 'documentId_required' });
      const result = await documentStorage.archivePersonal(session.userId, body.documentId);
      return sendJson(res, result.available ? 200 : 404, result);
    },

    'POST /documents/personal/restore': async (req, res) => {
      const session = await currentSession(req);
      if (!session) return sendJson(res, 401, { error: 'not_authenticated' });
      const body = await readJsonBody(req);
      if (!body.documentId) return sendJson(res, 400, { error: 'documentId_required' });
      const result = await documentStorage.restorePersonal(session.userId, body.documentId);
      return sendJson(res, result.available ? 200 : 404, result);
    },

    'POST /documents/personal/delete': async (req, res) => {
      const session = await currentSession(req);
      if (!session) return sendJson(res, 401, { error: 'not_authenticated' });
      const body = await readJsonBody(req);
      if (!body.documentId) return sendJson(res, 400, { error: 'documentId_required' });
      const result = await documentStorage.deletePersonal(session.userId, body.documentId);
      return sendJson(res, result.available ? 200 : 404, result);
    },

    'POST /documents/personal/versions': async (req, res) => {
      const session = await currentSession(req);
      if (!session) return sendJson(res, 401, { error: 'not_authenticated' });
      const body = await readJsonBody(req);
      if (!body.documentId) return sendJson(res, 400, { error: 'documentId_required' });
      const result = await documentStorage.getVersionsPersonal(session.userId, body.documentId);
      return sendJson(res, result.available ? 200 : 404, result);
    },

    'POST /documents/personal/search': async (req, res) => {
      const session = await currentSession(req);
      if (!session) return sendJson(res, 401, { error: 'not_authenticated' });
      const body = await readJsonBody(req);
      const result = await documentStorage.searchPersonal(session.userId, body.filters || {});
      return sendJson(res, 200, result);
    },

    // Real personal-ownership equivalents of /documents/binary above —
    // same real FilesystemObjectStorageProvider, same real validation,
    // only the ownership check differs (user_id, not organization
    // membership). Organization binary routes above are unchanged.
    'POST /documents/personal/binary': async (req, res) => {
      const session = await currentSession(req);
      if (!session) return sendJson(res, 401, { error: 'not_authenticated' });
      ...
      if (!documentId) return sendJson(res, 400, { error: 'documentId_header_required' });

      const MAX_BINARY_BYTES = 25 * 1024 * 1024;
      try {
        const result = await documentStorage.savePersonalBinary(session.userId, documentId, req, { mimeType, originalFilename, maxBytes: MAX_BINARY_BYTES });
        return sendJson(res, result.available ? 200 : 404, result);
      } catch (err) {
    ...
    'GET /documents/personal/binary': async (req, res) => {
      const session = await currentSession(req);
      if (!session) return sendJson(res, 401, { error: 'not_authenticated' });
      const url = new URL(req.url, 'http://internal');
      const documentId = url.searchParams.get('documentId');
      if (!documentId) return sendJson(res, 400, { error: 'documentId_required' });

      const result = await documentStorage.loadPersonalBinary(session.userId, documentId);
