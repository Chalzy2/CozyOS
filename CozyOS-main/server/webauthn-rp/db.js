...
function openDb(dbPath) {
  ...
  migrateAddDocumentPersonalOwnership(db);
  return db;
}
...
// CozyOS File Phase 5 — mirrors
// server/webauthn-rp/migrations/018_document_personal_ownership.sql.
// See that file's own header for the full discovery/design rationale.
// Real SQLite table-rebuild (no ALTER COLUMN DROP NOT NULL exists) —
// guarded idempotent via the same PRAGMA table_info() column-existence
// check migrateAddDocumentBinaryStorage() already established: if
// user_id already exists on documents, this migration already ran, and
// the (destructive, one-time) rebuild below must never run twice.
function migrateAddDocumentPersonalOwnership(db) {
  const cols = db.prepare('PRAGMA table_info(documents)').all();
  const existing = new Set(cols.map((c) => c.name));
  if (existing.has('user_id')) return;
