...
test('a Firebase-issued session still cannot reach /admin, but reaches the public /dashboard and /dashboard.html', async () => {
  await withBoundaryServer('boundary-holds', async ({ base, rp }) => {
    ...
    // RP-ADMIN-ROUTING-SPLIT: /dashboard and /dashboard.html are the
    // public User Dashboard entry point (see static-boundary-server.js
    // header and FORBIDDEN_ADMIN_ALIASES comment) — served unconditionally
    // to every visitor, admin or not. Only /admin (and its aliases) are
    // gated. This mirrors the already-passing password-auth-boundary
    // equivalent of this test in static-boundary-password-auth.test.js.
    for (const publicRoute of ['/dashboard', '/dashboard.html']) {
      const res = await fetch(base + publicRoute, { headers: { Cookie: cookie } });
      assert.equal(res.status, 200, `${publicRoute} is the public User Dashboard and must be reachable`);
    }
    ...
    const user = rp.db.prepare('SELECT * FROM users WHERE email = ?').get('nonadmin@example.com');
