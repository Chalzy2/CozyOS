...
## AA-007 — Kiswahili (`sw`) capability status diverges across two real registries with no stated relationship

**Status:** Closed — Resolved by Phase 2 unified capability contract design.

**Stage history (Rule 61 — no stage skipped):**
- 🟡 **Composed** (Phase 1 audit session): finding recorded below, evidence gathered from both live registries; Repair Queue row opened same pass.
- 🟠 **Planned** (Phase 2 session): decided the smallest safe repair was an additive, read-only data contract (`core/modules/builder/unified-capability-contract.js`) that preserves both registries' raw values unchanged and derives an overall status via an explicit, documented function — not a rewrite of either source registry.
- 🔵 **Implementing**: `core/modules/builder/unified-capability-contract.js` and its test suite (`core/modules/builder/tests/unified-capability-contract.test.js`) written; `buildKiswahiliValidationRecord()` reads both real registries live at call time.
- 🟢 **Verified**: `node core/modules/builder/tests/unified-capability-contract.test.js` — 15/15 passed, including test 10 confirming the record shows `response_generation: AVAILABLE`, `vocabulary: NOT_READY`, overall `PARTIALLY_VERIFIED` (never falsely `VERIFIED`), and a `DIMENSION_DIFFERENCE` finding rather than a same-key `CONFLICT`. Existing regression re-run clean and unmodified: `cozy-language-registry.test.js` 15/15, `cozy-language-pack-registry.test.js` 32/32.
- 🟢 **Closed**: see Closure Criteria below, now met.

**1. Evidence (every conflicting source, verbatim/paraphrased with attribution):**

| Source | What it says about `sw` |
|---|---|
| `core/modules/intelligence/language/cozy-language-registry.js` (RP-027) | `sw` is one of 5 languages with `state: "AVAILABLE"` — meaning verified response templates exist for every RP-027 conversational intent, re-verified by RP-027's own test suite |
| `core/modules/intelligence/language-packs/cozy-language-pack-registry.js` (RP-030/RP-035, canonical per `docs/history/RP-035-Phase1-ADR-Canonical-Registry.md`) | `sw` is one of the 13 `DEFAULT_IDENTITIES`, with `resourceState: "NOT_READY"` — vocabulary/phrases not populated, matching the Repair Queue's own open `RP-030-CONTENT` entry ("populate vocabulary/phrases/grammar for the 13 registered language packs; currently all NOT_READY") |

Both files are real, live, and internally honest — neither fabricates a
status. The RP-035 Phase 1 ADR already established the pack registry as
canonical for *language-pack identity* (the set of 13 codes), but it does
not address which registry is canonical for *capability status*, since
the two registries track different axes (response-template readiness vs.
vocabulary-content acquisition) that were never previously compared
side by side.

**2. Possible explanations:**
1. Both statuses are correct and simply describe different axes of the
   same capability (conversational-template readiness vs. vocabulary-pack
   content) — no conflict, only an undisclosed relationship between them.
2. One registry's status is stale relative to the other (e.g. templates
   were verified after the pack registry was last updated, or vice versa).
3. A caller reading only one registry could reasonably conclude Kiswahili
   is either fully ready or entirely unready for CozyAI, when the honest
   answer is "ready on one axis, not on the other."

**3. Risk assessment:** Medium. Not a runtime defect — nothing crashes or
returns fabricated data. The risk is exactly the one Rule 52 exists to
catch: a future caller or Builder session trusting a single registry's
`sw` status as the whole answer, in either direction.

**4. Evidence needed to resolve:** A determination of whether Explanation
1 (different axes, no conflict) is the correct and complete account —
this reading of both files' own headers and the RP-030-CONTENT Repair
Queue entry supports Explanation 1 as most likely, but this has not been
independently re-verified by running either registry's own test suite
this session (`cozy-language-registry.test.js`,
`cozy-language-pack-registry.test.js` — neither executed as part of this
audit).

**5. Implementation lock:** No file may be modified to "fix" this by
changing either registry's status field until the relationship between
the two axes is explicitly designed and documented — doing so
unilaterally risks silently overwriting one axis's honest signal with
the other's.

**6. Related:** This is the concrete case behind the Unified Capability
Registry task's own Kiswahili self-diagnosis requirement (see the
Unified Capability Registry Phase 1 audit checkpoint, this session). It
is also directly related to, but distinct from, the open Repair Queue
entry `RP-030-CONTENT`.

**7. Closure criteria (MET):** The unified capability layer's Phase 2
schema design (`core/modules/builder/unified-capability-contract.js`)
explicitly states how `AVAILABLE` (templates) and `NOT_READY` (pack
content) compose into one reported capability status for Kiswahili:
both are preserved verbatim as separate `DimensionRecord`s
(`response_generation`, `vocabulary`), each carrying its own source
registry/field/raw value untouched, and the overall status is derived
as `PARTIALLY_VERIFIED` — reusing `cozy-knowledge-registry.js`'s real
existing evidence taxonomy rather than inventing a new one. This mapping
was run against both registries' real, live data (not asserted) and
confirmed by `unified-capability-contract.test.js` test 10. Note: this
closes the *architecture ambiguity* (the relationship between the two
axes is now explicit and tested) — it does not close `RP-030-CONTENT`,
which remains open and tracks the separate, real task of actually
populating Kiswahili vocabulary content.
