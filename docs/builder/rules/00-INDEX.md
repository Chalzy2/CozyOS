...
| `31-interruption-safety-mid-milestone-patch-rule.md` | 86 | Interruption Safety / Mandatory Mid-Milestone Differential Patch — closes the gap Rule 85 leaves when a session is interrupted *before* a full recovery ZIP is produced: whenever a major subtask completes, or the likely tool/session limit is approaching, the Builder must produce a lightweight DIFFERENTIAL patch (added/modified/deleted files only, against the last certified baseline) containing a manifest, implementation status (done vs. mid-edit, explicitly distinguished), exact test counts, limitations, missing dependencies, a protected-file audit, dual matching SHA-256, and an explicit `NEXT BUILD MUST START WITH`. The patch must be independently mergeable onto the baseline without any other session artifact. Never wait for session termination to create it; the next session resumes from the latest MID patch, never from chat narrative. Neither this patch nor a Rule 85 ZIP is a certification artifact on its own — the full Rule 60/65/67/70/79/80 gate is unchanged. Every future implementation prompt for a governed subsystem must repeat this rule's mandatory header at the top |
