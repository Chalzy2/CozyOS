...
(function () {
    ...
    window.CozyOS.Firebase.Auth = Object.freeze({
        ...
        async signInWithEmailAndPassword(email, password) {
            if (!authInstance) return { available: false, code: "auth/not-ready", reason: "Firebase Auth is not ready. Failing closed." };
            try {
                const cred = await sdk.signInWithEmailAndPassword(authInstance, email, password);
                return { available: true, user: cred.user };
            } catch (err) {
                // DIAGNOSTIC FIX (M-ADMIN-FIREBASE-CODE): expose the
                // Firebase SDK's own short error CODE (e.g.
                // "auth/wrong-password", "auth/user-not-found") — a
                // documented, non-secret classification string the SDK
                // itself defines — never err.message verbatim, since some
                // Firebase error messages can be more verbose than needed
                // for a login-failure UI. Never touches, logs, or returns
                // the password or any token; err here is the SDK's own
                // rejection of the credential, not a value we chose.
                return { available: false, code: err.code || "auth/unknown-error", reason: err.message || "Firebase sign-in failed." };
            }
        },
        /**
