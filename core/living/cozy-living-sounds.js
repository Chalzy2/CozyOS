...
(function () {
    ...
    class CozyLivingSounds {
        ...
        /**
         * #clampVolume(v)
         *   Targeted fix (post-M364.4 audit finding): the two fade
         *   ramps below (play()'s fade-in step, fadeOut()'s fade-out
         *   step) each computed a progress fraction `t` as
         *   `(now - start) / durationMs`, clamped only at the upper
         *   bound via `Math.min(1, ...)` — never at the lower bound.
         *   requestAnimationFrame's own callback timestamp can, on the
         *   very first frame, land microseconds before the `start`
         *   time captured via a separate performance.now() call (real,
         *   observed clock-domain drift, not a logic error in the
         *   surrounding code) — producing a `t` fractionally below 0
         *   and, downstream, a `.volume` fractionally below 0 (or, for
         *   the symmetric fadeOut() case, fractionally above 1).
         *   HTMLMediaElement.volume throws on either. This is the
         *   single, shared, final boundary both fades already write
         *   through before handing a number to the real audio element
         *   — clamping only here (in addition to clamping `t` itself,
         *   just below) guarantees the assignment can never throw
         *   regardless of any future arithmetic upstream, without a
         *   second volume system or any change to the fade design
         *   itself.
         */
        #clampVolume(v) {
        ...
        fadeOut(eventName, fadeMs = 800) {
            ...
            const step = (now) => {
                ...
                entry.audioEl.volume = this.#clampVolume(startVolume * (1 - t));
