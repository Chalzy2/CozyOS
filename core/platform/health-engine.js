...
(function () {
    ...
    class CozyHealthEngine {
        ...
        /**
         * reportCoreServices()
         *   Real (Milestone 204, Gate 9) — checks actual presence of the
         *   real, named core services, since the milestone's "all GREEN
         *   before applications load" requirement has no existing
         *   equivalent — report() above checks file-registry status, not
         *   service readiness. Reads real window.CozyOS.* globals,
         *   never assumes.
         */
        reportCoreServices() {
            const services = {
                Identity: "IdentityEngine", Session: "Auth", Router: "Router",
                Workspace: "WorkspaceShell", Memory: "CozyMemory", Voice: "CozySpeech",
                OCR: "OCR", Theme: "Theme"
            };
            const status = {};
            let allGreen = true;
            for (const [label, globalName] of Object.entries(services)) {
                const present = !!window.CozyOS[globalName];
                status[label] = present ? "GREEN" : "RED";
                if (!present) allGreen = false;
            }
            return { allGreen, status, bestEffort: true };
        }
    }
    ...
    if (window.CozyOS.ServiceRegistry && typeof window.CozyOS.ServiceRegistry.registerCoordinator === "function") {
        try {
            window.CozyOS.ServiceRegistry.registerCoordinator({ sourcePath: "core/platform/health-engine.js",
