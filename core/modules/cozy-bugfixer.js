...
`;
    ...
    class CozyOSBugFixerCoordinator {
            ...
            if (window.CozyOS.Certification && typeof window.CozyOS.Certification.quickCertification === "function") {
                try {
                    ...
                    certification = {
                        ...
                        defects: result.defects.map(d => ({ id: d.id, severity: d.severity, description: d.description, autoFixable: !!DETERMINISTIC_FIXERS[d.id] }))
                    };
                } catch (err) {
        ...
        repair(fileId, { ruleIds } = {}) {
            ...
            if (window.CozyOS.Certification && typeof window.CozyOS.Certification.quickCertification === "function") {
                try {
                    ...
                    if (!candidateRuleIds) candidateRuleIds = result.defects.filter(d => DETERMINISTIC_FIXERS[d.id]).map(d => d.id);
                } catch (err) {
            ...
            for (const ruleId of candidateRuleIds) {
                const fixer = DETERMINISTIC_FIXERS[ruleId];
                ...
                if (result.applied) {
                    ...
                    appliedFixes.push({ ruleId, description: result.description });
                } else {
        ...
        /**
         * save(fileId, { proposedSource, approve, enforcedProtectedOverride })
         ...
         */
        async save(fileId, { proposedSource, approve = false, enforcedProtectedOverride = false, ruleIdsFixed = [] } = {}) {
            const record = this.#files.get(fileId);
            ...
            const autoRepairActive = this.#autoRepairSessions.has(fileId);
            if (!approve && !autoRepairActive) {
                throw new Error("[CozyBugFixer] save(): requires approve:true (or Auto Repair enabled for this file this session).");
            }
