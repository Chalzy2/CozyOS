...
const _kernel = {
    ...
    registerLanguage(config) {
        ...
        _languages.set(config.languageCode, Object.freeze({
            ...
            // Kiswahili Capability Dependency #1 — real defect found:
            // this function previously dropped config.bcp47Tag entirely
            // (not merely unused — never stored at all), so
            // SpeechLanguageAdapter.resolve()'s existing
            // known.bcp47Tag read could never see anything but
            // undefined, regardless of what a caller passed. Fixed at
            // this exact root cause with the same optional-field
            // pattern already used for region/family above — every
            // existing call site that never passed bcp47Tag is
            // completely unaffected (stores null, exactly as region/
            // family already do).
            bcp47Tag:       config.bcp47Tag ?? null,
...
if (typeof window !== "undefined") {
    ...
    (function _seedVoiceEngineDefaults() {
        ...
        // Kiswahili Capability Dependency #1 (STT locale qualification) —
        // real-world Web Speech API recognition generally requires a
        // region-qualified BCP-47 tag rather than a bare macrolanguage
        // code; "sw" alone risks silently failing or degrading in real
        // browsers. This is a real, additive extension of the EXISTING
        // Kiswahili registration below — not a new language, not a new
        // registry, not a second STT engine. SpeechLanguageAdapter.resolve()
        // already reads bcp47Tag if present (confirmed by reading its
        // source before this change) — no change needed there. Every
        // other language's registerLanguage() call below is completely
        // unaffected. This does NOT claim any improvement in real
        // recognition accuracy — that can only be judged with a real
        // microphone and real Kiswahili speech (device-dependent,
        // NOT-RUN in this environment); it only ensures the correct
        // locale tag reaches the browser API in the first place.
        const BCP47_OVERRIDES = { sw: "sw-KE" };
        INITIAL_LANGUAGES.forEach(([languageCode, name]) => {
            if (!speech.listLanguages().some(l => l.languageCode === languageCode)) {
                const bcp47Tag = BCP47_OVERRIDES[languageCode];
                speech.registerLanguage(bcp47Tag ? { languageCode, name, bcp47Tag } : { languageCode, name });
            }
        });
