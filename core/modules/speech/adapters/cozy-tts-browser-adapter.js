...
(function () {
    ...
    function speakPreview(config = {}) {
        ...
        return new Promise((resolve) => {
            ...
            // TTS language-propagation dependency: a direct config.language
            // (the real conversational language, e.g. from VoiceManager's
            // own request) is used only when no settingsId-derived language
            // is already present - a stored voice-settings preference still
            // takes priority, unchanged. Reuses the exact same, unmodified
            // findVoiceForLanguage() below either way - no second
