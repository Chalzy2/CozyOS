...
(function () {
    ...
    const SpeechTranslationNLLBProvider = {
        ...
        register(baseUrl) {
            ...
            const result = providers.register(provider);
            // Domain 4C dependency #3 — real, optional visibility
            // registration into the existing, already-proven
            // core/shell/provider-manager.js (the exact same pattern
            // already used for the conversational providers and visual
            // engines). ProviderManager never becomes the owner of
            // translation execution — every existing caller of
            // translate()/isAvailable() above is completely unchanged.
            // Honest health: registering here only proves the JS
            // provider object exists in SpeechTranslationProviders, not
            // that the real local bridge process is reachable — that
            // genuinely requires the real, async /health network call
            // this file's own isAvailable() already performs, which a
            // synchronous getHealth() call must not silently trigger as
            // a side effect (the same discipline already established
            // for the Gemini conversational provider).
            const pm = window.CozyOS.ProviderManager;
            if (pm && typeof pm.register === "function") {
                pm.register({
                    id: "nllb-bridge",
                    name: "NLLB Translation Bridge",
                    ...
                    getHealth() {
                        return {
                            health: "UNKNOWN",
                            reason: "Registered in SpeechTranslationProviders, but real availability depends on a local NLLB bridge process being reachable — this cannot be determined without a live network call, which getHealth() must not trigger as a side effect. Call isAvailable() or a real translate() for genuine, observed status."
                        };
                    }
                });
            }
