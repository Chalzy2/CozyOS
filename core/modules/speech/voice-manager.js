...
(function () {
    ...
    const CONTEXTS = Object.freeze([
        ...
        "onboarding",
        // CP16A (Kiswahili -> English Translation Foundation): a
        // distinct context so translated speech delivery can be
        // resolved/overridden independently of every other voice use —
        // never conflated with "startup"/"onboarding"/"assistant".
        // Defaults to "charles" (Owner Voice) via this same engine's
        // existing unconditional default, exactly as every other
        // context already does — no new voice-resolution mechanism.
        // See core/modules/translate/translation-service.js.
        "translation",
    ]);
    ...
    class VoiceManager {
        ...
        async speak(request = {}) {
            ...
            const attempt = async (providerId, isFallback) => {
                ...
                try {
                    const result = await p.speak({ text: request.text, context: request.context, settingsId: request.settingsId, language: request.language, speed: this.#settings.speed, pitch: this.#settings.pitch, volume: this.#settings.volume });
