...
VERSION: 1.4.1 (Enterprise Certification Hardening Pass — Audit Coverage,
Validation Coverage, Event Coverage; Compensation Coverage Verified)
...
CHANGE NOTE (1.4.0 -> 1.4.1):
Certification hardening pass — engineering-only fixes, no new subsystems,
no route/permission/schema/finance-path changes:
  - Validation: _validate() now called at the top of every execute*
    method that accepts a payload (previously 10 of 40). Existing inline
    guard clauses (e.g. explicit employeeId/customerId checks) were kept
    as-is since _validate() itself no-ops unless window.CozyOS.Shared.
    QuarryValidation is loaded — the inline checks remain the actual
    runtime enforcement; _validate() calls make the module consistent
    and forward-compatible with that shared validator once present.
  - Audit coverage: added _auditLog() (thin wrapper around window.CozyOS.
    Shared.QuarryAudit, same adapter _routeExternalFinancialLegger and
    executeStockAdjustment already used) and wired it into every
    non-financial mutation that previously had none — employee updates,
    employee status changes, employee transfers, driver updates, parcel
    registration, maintenance updates, and stock deductions triggered
    from a sale. Also fixed a dead ternary in the pre-existing stock
    adjustment audit call that always evaluated to the same value
    regardless of its condition.
  - Compensation: audited all 9 call sites of
    _routeExternalFinancialLegger — every one was already wrapped in
    _withCompensation. No gap found; no change needed.
  - Event coverage: added the 6 previously-missing domain events —
    customer.registered, stock.adjusted (manual + sale-driven),
    fuel.issued, maintenance.logged, delivery.confirmed, expense.logged
    — using the exact same _publishEvent()/CustomEvent mechanism already
    in place. No new event bus was introduced.
  - Fixed a pre-existing malformed file-header comment (started with
    "**" instead of "/**"), which was a latent syntax error.
Explicitly deferred (per certification scope): universal collection-name
constants migration, localization/language engine, offline sync scope
expansion beyond production, manifest auto-generation from a route
...
// Immutable Seven-Key Structural Schema Invariant Template  
    ...
    // and diffable.  
    // QuarryOS Integration & Unification milestone: roleMatrix's permitted-
    // route strings were already out of sync with this file's own real
    // case names before this change (e.g. "log_attendance" here vs the
    // real "track_attendance" case; "manage_parcel" here vs the real
    // "register_parcel" case) — a genuine, pre-existing bug, not
    // introduced now. Fixing all 45 routes' mappings is a separate,
    // larger correctness pass, out of scope for this milestone (which is
    // integration, not a full audit of this engine's internals). Only the
    // 2 real routes this milestone's UI actually calls are added below,
    // narrowly, so the one real save/AI action this unification wires up
    // can actually succeed for a non-Administrator role.
    this.roleMatrix = new Map([  
        ...
        ["Manager",       ["assign_workforce", "log_attendance", "manage_parcel", "log_maintenance", "view_analytics", "log_crusher_production", "ask_ai_advisor"]],  
        ["Supervisor",    ["assign_workforce", "log_attendance", "log_maintenance", "log_crusher_production", "ask_ai_advisor"]],  
        ["Accountant",    ["process_payroll", "view_analytics"]],  
        ["Machine Operator", ["log_crusher_production", "ask_ai_advisor"]]  
    ]);  
...
async executeAttendanceLog(payload) {  
    this._validate(payload, ["operatorId", "status"]);
...
async executeParcelRegistration(payload) {  
    ...
    this._auditLog(
        "parcel_registered",
        C?.COLLECTIONS?.AUDIT_LOG || "quarry_audit_log",
        this._buildHeader(payload, payload.landOwnerId),
        null,
        parcelRecord,
        "Parcel registration"
    );
...
async executeMaintenanceUpdate(payload) {  
    ...
    // Covers both branches above: when costValue > 0 the finance leg is
    // already audited by _routeExternalFinancialLegger, but the
    // maintenance record itself (status/downtime/next-service fields)
    // is not a finance object, so it's logged explicitly here either way.
    this._auditLog(
        "maintenance_status_change",
        C?.COLLECTIONS?.AUDIT_LOG || "quarry_audit_log",
        this._buildHeader(payload),
        null,
        maintenanceRecord,
        payload.reason || "Maintenance update"
    );

    this._publishEvent(C?.EVENTS?.MAINTENANCE_LOGGED || "maintenance.logged", { machineId: payload.machineId, maintenanceId: maintenanceRecord.maintenanceId, repairCost: costValue });
...
/**
 * ── Engineering Pass: Reusable Audit Entry Point ──
 * Non-financial mutations (employee/driver/parcel updates, maintenance
 * status changes, stock deductions triggered from a sale) do not pass
 * through _routeExternalFinancialLegger, so they never hit its automatic
 * audit hook. This wraps the same window.CozyOS.Shared.QuarryAudit
 * adapter already used there and in executeStockAdjustment, so every
 * call site — financial or not — writes through one audit interface.
 * No-ops safely (with a console fallback) if the shared audit module
 * isn't loaded, matching the existing degrade-gracefully pattern used
 * by _validate/_withCompensation.
 */
_auditLog(action, collection, header, previousValue, newValue, reason) {
    const audit = window.CozyOS?.Shared?.QuarryAudit;
    if (audit && typeof audit.record === "function") {
        audit.record({ collection, action, header, previousValue, newValue, reason }).catch(() => {});
    } else {
        console.log(`📝 [Quarry Audit] ${action} on '${collection}':`, { previousValue, newValue, reason });
    }
}
...
async executeEmployeeUpdate(payload) {  
    ...
    // _routeExternalFinancialLegger's automatic audit hook. Logged
    // directly here with the pre/post record.
    this._auditLog(
        "employee_update",
        C?.COLLECTIONS?.AUDIT_LOG || "quarry_audit_log",
        this._buildHeader(payload, payload.employeeId),
        previousRecord,
        updatePatch,
        payload.reason || "Profile update"
    );
...
async executeEmployeeTransfer(payload) {  
    ...
    this._auditLog(
        ...
        C?.COLLECTIONS?.AUDIT_LOG || "quarry_audit_log",
        this._buildHeader(payload, payload.employeeId),
        { department: transferRecord.fromDepartment, branchId: transferRecord.fromBranchId },
        { department: transferRecord.toDepartment, branchId: transferRecord.toBranchId },
...
async executeCustomerRegistration(payload) {  
    ...
    this._publishEvent(C?.EVENTS?.CUSTOMER_REGISTERED || "customer.registered", { customerId, type: customerRecord.type });  
...
async executeSaleRecord(payload) {  
    ...
    // ── Engineering Pass: audit coverage ──
    // Stock deductions triggered indirectly from a sale never passed
    // through executeStockAdjustment's audit call, so they were the one
    // stock-mutating path with no audit trail at all. Logged per line
    // item here, plus a stock.adjusted event per the event-coverage pass.
    (payload.items || []).forEach(item => {  
        if (!item.stockCategory) return;  
        const qty = parseFloat(item.quantity) || 0;  
        const before = this.stockLevels.get(item.stockCategory) || 0;  
        this._deductStock(item.stockCategory, qty);  
        const after = this.stockLevels.get(item.stockCategory) || 0;  
        this._auditLog(
            "stock_deduction_from_sale",
            C?.COLLECTIONS?.AUDIT_LOG || "quarry_audit_log",
            header,
            before,
            after,
        ...
        this._publishEvent(C?.EVENTS?.STOCK_ADJUSTED || "stock.adjusted", { category: item.stockCategory, delta: -qty, resultingLevel: after, source: "sale", saleId: sale.saleId });
    });  
...
async executeDriverUpdate(payload) {  
    ...
    this._auditLog(
        "driver_update",
        C?.COLLECTIONS?.AUDIT_LOG || "quarry_audit_log",
        this._buildHeader(payload, payload.driverId),
        previousRecord,
        updatePatch,
        payload.reason || "Profile update"
    );
...
async executeStockAdjustment(payload) {  
    ...
    // (Fixed: this previously wrote to "quarry_stock_adjustments" via a
    // dead ternary that resolved to the same value on both branches — it
    // now correctly targets the audit log collection via the shared helper.)
    this._auditLog(
        "stock_adjustment",
        C?.COLLECTIONS?.AUDIT_LOG || "quarry_audit_log",
        this._buildHeader(payload),
    ...
    this._publishEvent(C?.EVENTS?.STOCK_ADJUSTED || "stock.adjusted", { category, delta, resultingLevel: updated, source: "manual" });
...
executeRoyaltyRateSet(payload) {  
    this._validate(payload, ["landOwnerId", "ratePerTon"]);
...
async executeExpenseLog(payload) {  
    ...
    this._publishEvent(C?.EVENTS?.EXPENSE_LOGGED || "expense.logged", { expenseId: expense.expenseId, category, amount });  
...
async executeReportGeneration(payload) {  
    this._validate(payload, ["reportType"]);
...
// Real, additive self-registration — Discovery/Application Visibility read
// from ServiceRegistry, never scan a filesystem. Adding this here (not in
// any dashboard file) is what makes "no manual registration" true: any page
// that loads this script automatically makes QuarryOS discoverable.
if (window.CozyOS.ServiceRegistry && typeof window.CozyOS.ServiceRegistry.registerApplication === "function") {
    try {
        window.CozyOS.ServiceRegistry.registerApplication({
            ...
            launcher: "modules/quarry/quarry.html"
        });
    } catch (_err) { /* non-fatal */ }
}
// Application Visibility Registry — real, additive self-declaration.
// HONEST DISCLOSURE, not silently omitted: this makes QuarryOS
// discoverable and launchable as a real navigation target. It does NOT
// mean every in-page action on that target page currently works — a real
// mismatch was found between this engine's handle(context) API,
// quarry-linker.js's expected DOM (a different, older UI), and the actual
// quarry.html page's own inline script (which expects a third interface,
// window.CozyOS.AI.quarry, that doesn't exist anywhere). See the
// migration log for the full finding — not resolved here, since picking
// one of the three real implementations to keep and discarding the others
// would be exactly the "redesign/rebuild" this milestone said not to do
// without explicit direction.
window.CozyOS.Modules.QuarryManager.visibility = Object.freeze({
    appId: "quarry_manager_001", name: "QuarryOS", icon: "⛏️", category: "business-application",
    launchTarget: Object.freeze({ href: "modules/quarry/quarry.html" }),
    audience: "all"
});
