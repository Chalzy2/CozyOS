...
(function () {
    ...
    class CozyOSDeveloperHub {
        ...
        async repairModule(moduleId, { approve = false, useAI = false } = {}) {
            this.#diagnostics.actionsDelegated++;
            const workspace = this.#requireWorkspace();
            const fileId = this.#requireFileId(moduleId);
            if (!this.#bugfixer()) throw new Error("[DeveloperHub] CozyBugFixer is not connected.");
            const result = await workspace.repairAndRecertify(fileId, { approve, useAI });
            this.#logAudit("REPAIR", `${moduleId}: changed=${result.changed}`);
            this.#logTimeline(`Repair run: ${moduleId}`);
            this.emit("hub:repaired", { moduleId, changed: result.changed });
            return result;
        }

        async openWithBugFixer(moduleId) {
            this.#diagnostics.actionsDelegated++;
            const workspace = this.#requireWorkspace();
            const fileId = this.#requireFileId(moduleId);
            if (!this.#bugfixer()) throw new Error("[DeveloperHub] CozyBugFixer is not connected.");
            return workspace.shareToBugFixer(fileId);
        }
