    ...
    class CozyDeveloperHubUI {
        ...
        #renderBuilder() {
            ...
            return `<h1>Builder</h1>
                <p class="cz-subtitle">${selected ? `Opened with "${escapeHtml(selected)}" already loaded — no re-upload.` : "Describe what you want to build."}</p>
                <div class="cz-panel">
        ...
        async #hubAnalyze() {
            ...
            if (!text) { this.#devOutput('<p class="cz-muted">Type a description first.</p>'); return; }
            try {
        ...
        async #hubBuildPlan() {
            ...
            const text = document.getElementById("cz-hub-builder-prompt")?.value.trim();
            try {
                const plan = this.#lastAnalysis?.understanding?.plan || (await hub.openWithBuilder(text.replace(/^build\s+/i, "").replace(/\s+coordinator$/i, "")));
        ...
        #renderQuickCert() {
            ...
            return `<h1>Quick Certification</h1>
                <div class="cz-panel">
                    ${files.length > 0 ? `
                    <div class="cz-field">
                        <label>Load a previously uploaded file (already in Workspace — no re-upload needed)</label>
                        <div class="cz-row">
                    ...
                    </div>` : `<p class="cz-muted">No files uploaded to Workspace yet — paste source below or upload one from Builder/Module Explorer first.</p>`}
                    ...
                    <div class="cz-field"><label>Source</label><textarea class="cz-input" id="cz-hub-qc-source" placeholder="Paste source, or load an already-uploaded file above..."></textarea></div>
        ...
        #hubQuickCert() {
            ...
            try {
                ...
                this.#devOutput(`<div class="cz-row"><span class="cz-badge ${verdictBadgeClass(result.verdict)}">${escapeHtml(result.verdict)}</span><span>${escapeHtml(result.summary.scorePercent)}%</span><span>Grade ${escapeHtml(result.overallGrade)}</span></div>`);
            } catch (err) { this.#devOutput(`<p class="cz-muted">${escapeHtml(err.message)}</p>`); }
        }
