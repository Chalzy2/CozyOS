...
(function () {
    ...
    class CozyDeveloperHubUI {
        ...
        #getRetainedSource(moduleId) {
            ...
            return this.#retainedSources.get(moduleId) || null;
        }
                ...
                <div class="cz-panel cz-dev-actions">
                    <div class="cz-row" style="flex-wrap:wrap;gap:6px;">
                        ${actions.map(([action, label]) => {
                            ...
                            const workspaceOnlyActions = ["hub-register-workspace", "hub-save", "hub-save-as", "hub-duplicate", "hub-rename", "hub-move", "hub-compare"];
                            const workspaceUnavailable = workspaceOnlyActions.includes(action) && !window.CozyOS.WorkspaceShell;
                            const disabled = rollbackUnavailable || workspaceUnavailable;
                            const title = rollbackUnavailable
                                ? "Rollback unavailable. CozyCertification does not currently expose a rollback API."
                                : workspaceUnavailable ? "Workspace not connected. This action becomes available once WorkspaceShell is loaded." : "";
                            return `<button class="cz-btn" data-action="${action}" data-module="${escapeHtml(moduleId)}"${disabled ? ` disabled title="${escapeHtml(title)}"` : ""}>${escapeHtml(label)}</button>`;
                        }).join("")}
                    </div>
                </div>
                <div class="cz-panel cz-dev-action-output-panel" id="cz-hub-output"></div>`;
