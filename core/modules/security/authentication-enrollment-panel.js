...
(function () {
    ...
    /**
     * getCurrentUserId() — real, composes CozyOS.Auth's own canonical
     * "who is signed in" pointer. Never invents a user identity.
     *
     * ORDINARY-USER FIX: CozyOS.Auth (core/security/cozy-auth.js) only
     * ever populates its current-identity pointer for Platform-
     * Administrator or Developer sessions — an ordinary user's sign-in
     * is explicitly rejected and never recorded there (see that file's
     * own #handleSessionStarted fail-closed branch). Now that this panel
     * is also mounted on the ordinary-user dashboard, this falls back to
     * CozyOS.Session.current() — the same real, auth-provider-agnostic
     * session snapshot every sign-in path (native or Firebase) already
     * establishes ("Deliberately unaware of any specific auth provider"
     * per that engine's own header) — never a second/parallel session
    ...
    const FACTOR_DEFS = [
        {
            ...
            //
            // CORRECTED (was DISCLOSED LIMIT): earlier revisions of this
            // comment claimed the server's enroll endpoints required an
            // authenticated platform-administrator session. Re-inspected
            // against the live server/webauthn-rp/server.js: both
            // POST /webauthn/passkeys/enroll/begin and .../complete only
            // check for an existing session (currentSession(req)) — they
            // never read session.isPlatformAdmin — and any authenticated
            // user (admin or ordinary) may self-enroll a passkey for
            // their own account, verified by the server's own
            // http-integration.test.js ("an ordinary authenticated
            // (non-admin) user can self-enroll a passkey for their own
            // account"). The not_authenticated_admin handling below is
            // kept as defensive, honest error-forwarding (never removed
            // just because live server code no longer emits it) — if a
            // future server revision ever reintroduced that gate, this
            // panel would still report it accurately rather than
            // fabricating success.
