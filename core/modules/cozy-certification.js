...
(function () {
    ...
    class CozyOSCertificationCoordinator {
        ...
        #registerDefaultRules() {
            ...
            R({ id: "UI-003", group: "uisafety", severity: SEVERITY.CRITICAL, description: "No document.write / raw HTML sinks", reason: "document.write and similar sinks bypass any escaping discipline used elsewhere.", impact: "A raw HTML sink is a direct XSS vector regardless of other precautions.", recommendation: "Never use document.write(); build DOM nodes or sanitized innerHTML only.", test: (s) => !/document\.write\(/.test(s) });
        ...
        certifyModule(sourceText, metadata = {}) {
            ...
            const warnings = nonWaivedFailed.length - blockingFailures;
            ...
            const severityCounts = {
                critical: nonWaivedFailed.filter(r => r.rule.severity === SEVERITY.CRITICAL).length,
                high: nonWaivedFailed.filter(r => r.rule.severity === SEVERITY.HIGH).length,
                medium: nonWaivedFailed.filter(r => r.rule.severity === SEVERITY.MEDIUM).length,
                low: nonWaivedFailed.filter(r => r.rule.severity === SEVERITY.LOW).length,
                info: nonWaivedFailed.filter(r => r.rule.severity === SEVERITY.INFO).length
            };
  ...
  <p><b>Certification ID:</b><br>${esc(report.certificationId)}</p>
        ...
        fullCertification() {
            const discoveredNames = (typeof window !== "undefined" && window.CozyOS)
                ? Object.keys(window.CozyOS).filter(name => name !== "Certification")
                : [];
