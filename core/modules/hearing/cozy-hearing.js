 ...
 *   not even as a fallback. **Correction (M387.5b, AA-004 resolution):** the
 *   canonical Listening Engine this file depends on is
 *   `core/engines/audio/cozy-audio-engine.js` (window.CozyOS.AudioEngine) —
 *   NOT `audio-manager.js` as this comment previously said. Real evidence:
 *   this file calls `registerInputAdapter()`/`startListening()`/
 *   `stopListening()`, which only `cozy-audio-engine.js` implements;
 *   `audio-manager.js` implements a different API (`registerProvider()`,
 *   device/mixer management) and is exposed separately as
 *   `window.CozyOS.AudioManager`. It owns the platform's one real
 *   getUserMedia call site. This file registers itself as a "hearing"
 *   input adapter via AudioEngine.registerInputAdapter() and receives the
 *   real, routed MediaStream — it still owns building its own
 *   AudioContext/AnalyserNode on top of that stream for real DSP analysis
 *   (that remains this file's job, not AudioEngine's — "Never process
 *   audio itself" per the
