...
(function () {
     ...
     *
     * "nayo" added by the LIVE WINDOW AI CONSUMPTION AUDIT — the same
     * kind of bare pronoun ("with it/her", the natural contracted
     * follow-up in "Ni nani anayenufaika nayo?") found live-testing
     * every application through the real provider; without it, "nayo"
     * would be treated as a literal (nonexistent) application name
     * instead of resolving via conversation context, same as "it"/"hii".
     */
    const APP_IMPORTANCE_PRONOUNS = new Set(["it", "this", "that", "hii", "hiyo", "nayo", "yake"]);
    ...
    function extractAppImportanceCandidate(text) {
        ...
        for (let i = 1; i < m.length; i++) {
            if (m[i]) {
                ...
                // LIVE WINDOW AI CONSUMPTION AUDIT — real bug found
                // live-testing every application: "Programu hii ni
                // muhimu kwa nini?" ("Why does THIS application
                // matter?") captured the literal two-word text
                // "Programu hii", which is not itself a member of
                // APP_IMPORTANCE_PRONOUNS (only the bare "hii" is), so
                // it was treated as a real (nonexistent) application
                // name instead of resolving via conversation context.
                // Strips the exact same filler-word prefix/suffix
                // getApplicationHumanPurposeFact() already normalizes
                // away (this dependency, cozy-knowledge-registry.js)
                // before the pronoun check, so "programu hii"/"app
                // hiyo"/"application it" are recognized as the same
                // bare pronoun as "hii"/"hiyo"/"it" alone.
                const normalizedForPronounCheck = trimmed.toLowerCase()
                    // "Na hiyo app inamsaidia nani?" ("And that app,
                    // who does it help?") — a real, required phrasing
                    // (this audit's own Kiswahili-first example) that
                    // otherwise captured the literal three-word text
                    // "Na hiyo app" and looked it up as a nonexistent
                    // application name. "Na" is a common Kiswahili
                    // sentence-initial connector ("And..."), not part
                    // of any real application name, so it's stripped
                    // here (pronoun-check only — never in the knowledge
                    // registry's own app-name normalization) before the
                    // existing pro*/application/app filler strip below.
                    .replace(/^na\s+/i, "")
                    .replace(/^(?:pro\w*|application|app)\s+(?:ya\s+)?/i, "")
                    .replace(/\s+app$/i, "")
                    .trim();
                return APP_IMPORTANCE_PRONOUNS.has(normalizedForPronounCheck) ? "" : trimmed;
            }
        }
        return "";
    }
