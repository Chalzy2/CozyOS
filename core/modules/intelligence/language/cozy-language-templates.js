...
(function () {
    ...
    const TEMPLATES = Object.freeze({
        ...
        // UNIVERSAL LIVE WINDOW AI CONSUMPTION AUDIT — real bug found by
        // driving the actual provider live: an application whose
        // knowledge record honestly has NO current capability yet (e.g.
        // HospitalOS/SchoolOS — genuinely just a mock/demo stub, per
        // cozy-knowledge-registry.js's own APPLICATION_HUMAN_PURPOSE_DATA
        // comment) rendered as dangling, blank-looking sections
        // ("Real-life problems it addresses: .", "Who benefits: .") once
        // .join() was called on a real, honestly-empty array — the
        // array itself was correct (never fabricated), only its
        // rendering was broken. Fixed with the exact same
        // `.join(...) || "<honest fallback>"` idiom this file already
        // uses for "language-support-list:verified" above, for the
        // identical reason: an empty real list is a genuine state to
        // disclose, not blank text to hide.
        "app-importance:known": Object.freeze({
            en: (name, p) => `${name} matters because: ${p.humanPurpose}\n\nReal-life problems it addresses: ${p.realLifeProblems.join(", ") || "not yet documented"}.\n\nWho benefits: ${p.whoBenefits.join(", ") || "not yet documented"}.\n\nIntended human benefits: ${p.humanBenefits.join(", ") || "not yet documented"}.\n\nCURRENTLY VERIFIED (implemented and tested today): ${p.currentVerifiedCapabilities.join("; ") || "none yet"}.\n\nVISION / DESTINATION (not yet implemented): ${p.visionCapabilities.join("; ") || "not yet documented"}.\n\n${p.visionSourceNote}`,
            sw: (name, p) => `${name} ni muhimu kwa sababu: ${p.humanPurpose}\n\nMatatizo ya kweli ya maisha inayoshughulikia: ${p.realLifeProblems.join(", ") || "bado hazijaandikwa"}.\n\nWanaonufaika: ${p.whoBenefits.join(", ") || "bado hazijaandikwa"}.\n\nManufaa yaliyokusudiwa kwa binadamu: ${p.humanBenefits.join(", ") || "bado hazijaandikwa"}.\n\nZILIZOTHIBITISHWA SASA (zimejengwa na kujaribiwa leo): ${p.currentVerifiedCapabilities.join("; ") || "hakuna bado"}.\n\nDIRA / MWELEKEO WA BAADAYE (bado hazijajengwa): ${p.visionCapabilities.join("; ") || "bado hazijaandikwa"}.\n\n${p.visionSourceNote}`,
            fr: (name, p) => `${name} est important car : ${p.humanPurpose}\n\nProblèmes réels traités : ${p.realLifeProblems.join(", ") || "pas encore documenté"}.\n\nBénéficiaires : ${p.whoBenefits.join(", ") || "pas encore documenté"}.\n\nBénéfices humains visés : ${p.humanBenefits.join(", ") || "pas encore documenté"}.\n\nACTUELLEMENT VÉRIFIÉ : ${p.currentVerifiedCapabilities.join("; ") || "aucun pour l'instant"}.\n\nVISION / DESTINATION (pas encore implémenté) : ${p.visionCapabilities.join("; ") || "pas encore documenté"}.\n\n${p.visionSourceNote}`,
            ar: (name, p) => `${name} مهم لأن: ${p.humanPurpose}\n\nالمشاكل الواقعية التي يعالجها: ${p.realLifeProblems.join("، ") || "لم يتم توثيقه بعد"}.\n\nالمستفيدون: ${p.whoBenefits.join("، ") || "لم يتم توثيقه بعد"}.\n\nالفوائد الإنسانية المقصودة: ${p.humanBenefits.join("، ") || "لم يتم توثيقه بعد"}.\n\nموثّق حاليًا: ${p.currentVerifiedCapabilities.join("؛ ") || "لا شيء بعد"}.\n\nالرؤية / الوجهة (لم تُنفَّذ بعد): ${p.visionCapabilities.join("؛ ") || "لم يتم توثيقه بعد"}.\n\n${p.visionSourceNote}`,
            so: (name, p) => `${name} muhiim ayuu u yahay sababtoo ah: ${p.humanPurpose}\n\nDhibaatooyinka dhabta ah ee uu wax ka qabanayo: ${p.realLifeProblems.join(", ") || "weli lama qorin"}.\n\nKuwa faa'iidaysta: ${p.whoBenefits.join(", ") || "weli lama qorin"}.\n\nFaa'iidooyinka bini'aadamka loogu talagalay: ${p.humanBenefits.join(", ") || "weli lama qorin"}.\n\nHADDA LA XAQIIJIYAY: ${p.currentVerifiedCapabilities.join("; ") || "weli waxba ma jiraan"}.\n\nARIMAHA MUSTAQBALKA (weli lama dhisin): ${p.visionCapabilities.join("; ") || "weli lama qorin"}.\n\n${p.visionSourceNote}`
        }),
