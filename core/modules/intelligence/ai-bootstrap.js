...
(function () {
    ...
    const VERSION = "1.1.0"; // P-023: Vision now gets a real live OCR health check instead of a hardcoded DISABLED stub
    ...
    // ── Honest stubs for the 6 genuinely-absent provider concepts.
    //    Vision is deliberately NOT in this list anymore - it now gets a
    //    real, live health check (see getVisionHealth()) instead of a
    //    hardcoded DISABLED status, because a real OCR engine
    //    (core/modules/ocr/cozy-ocr.js) genuinely exists and is genuinely
    //    composable via CognitiveCoordinator.runFromImage() - it was only
    //    ever reported DISABLED because nothing checked it, not because
    //    no real capability exists. ──
    ...
    /**
     * getVisionHealth()
     *   Real, live status - composes the existing, unmodified
     *   window.CozyOS.OCR (core/modules/ocr/cozy-ocr.js), never a second
     *   OCR/vision engine. Reports ONLINE only when OCR itself honestly
     *   reports a real backend loaded (Tesseract.js or a registered
     *   plugin) - never fabricates availability. If the Tesseract.js
     *   vendor library isn't present in this build, this honestly
     *   reports DISABLED with OCR's own real reason, exactly as OCR
     *   itself would - this function adds no capability OCR doesn't
     *   already, genuinely have.
     */
    function getVisionHealth() {
        const ocr = window.CozyOS.OCR;
        if (!ocr || typeof ocr.isAvailable !== "function") {
            return { health: "DISABLED", reason: "window.CozyOS.OCR (core/modules/ocr/cozy-ocr.js) is not loaded." };
        }
        const status = typeof ocr.getProviderStatus === "function" ? ocr.getProviderStatus() : { available: ocr.isAvailable() };
        if (status.available) {
            return { health: "ONLINE", engine: status.engine || "Tesseract.js", note: "Real OCR engine active - composed via CognitiveCoordinator.runFromImage(), never a second vision engine." };
        }
        return { health: "DISABLED", reason: status.note || "CozyOS.OCR is loaded but no real OCR backend is available yet." };
    }
    ...
    function registerWithProviderManager() {
        ...
        pm.register({ id: "vision", name: "Vision Provider (OCR)", category: "cognitive", version: version(window.CozyOS.OCR), getHealth: () => { const r = getProviderRegistry(); return r.vision; } });
    ...
    window.CozyOS.Modules["ai-bootstrap"] = Object.freeze({
        version: VERSION,
        description: "AI Bootstrap (M366.9, P-023 vision wiring) — registers real, honest providers with CozyInterpretation/CozyThinking (previously zero providers, the confirmed cause of raw-JSON fallback) and a real consistency rule with CozyReasoning. Reports real status for Conversation/Memory/Voice/Intelligence (already complete, no new registration needed). Vision now gets a real, live health check composing the existing window.CozyOS.OCR (Tesseract-backed) instead of a hardcoded DISABLED stub — reports ONLINE only when OCR itself honestly reports a real backend loaded. Honest DISABLED stubs remain for the 6 provider concepts with genuinely no backing engine in this repository: Knowledge/Planning/Decision/Creativity/Security/Automation. No new AI, no fabricated ONLINE status."
    });
})();
