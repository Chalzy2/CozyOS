 ...
 *
 *   Referenced From
 *     - Identity Engine (deprecated compatibility delegation only —
 *       see identity-engine.js's own Rule 25 declaration)
 *     - Payment Engine (companyId/branchId references only)
 *     - Document Engine (companyId/branchId references only)
 *     - Vault (future)
 *     - Applications (MpesaOS, ShopOS, and all future applications)
 *
 *   No other engine may re-implement CRUD for any of the six concepts
 *   above. New engines needing organizational scoping reference this
 *   engine's IDs; they never own a second copy of this data (Rule 2,
 *   Rule 3). This declaration exists so a future session — any Claude
 *   account, another AI, or a human developer — does not accidentally
 *   reintroduce duplicate ownership.
 * ═══════════════════════════════════════════════════════════════════════
 *
 * RESPONSIBILITY
...
(function () {
    ...
    class CozyOSCompanyManagement {
        ...
        // ---- Organization: the canonical owner as of this phase — real,
        // top-level registry above Company in the hierarchy. IdentityEngine's
        // Organization methods now delegate here when Company is connected,
        // kept only for backward compatibility (Rule 24).
        #orgs = new Map();
        // ---- Division: sits between Branch and Department in the
        // hierarchy — genuinely new, no prior overlap with any engine.
        #divisions = new Map();
        // ---- Team: sits below Department — genuinely new, no prior
        // overlap with any engine.
        #teams = new Map();
         ...
         * queryable rather than only documentation. Any engine or
         * tooling can check this at runtime before deciding whether to
         * implement its own CRUD for one of these concepts.
         */
        getCanonicalOwnership() {
            return Object.freeze({
                canonicalOwner: "CompanyEngine",
                owns: Object.freeze(["Organization", "Company", "Branch", "Division", "Department", "Team"]),
                referencedFrom: Object.freeze(["IdentityEngine (deprecated compatibility delegation only)", "PaymentEngine", "DocumentEngine", "Vault (future)", "Applications"])
            });
        }

        // =====================================================================
        // ─── ORGANIZATION CRUD ────────────────────────────────────────────────
        // Canonical owner as of this phase (Company Engine). Method
        // signatures match IdentityEngine's original Organization API
        // exactly, since those methods now delegate here for backward
        // compatibility (Rule 24) rather than maintaining a second,
        // independent implementation (Rule 2).
        // =====================================================================

        createOrganization(name) {
            if (!name || typeof name !== "string" || !name.trim()) throw new TypeError("[CozyCompany] createOrganization(): name is required.");
            const id = this.#generateId("org");
            const now = new Date().toISOString();
            const record = { id, name: this.#escapeHtml(name), status: "active", createdAt: now, updatedAt: now };
            this.#orgs.set(id, record);
            this.#diagnostics.organizationsCreated++;
            this.#logAudit("ORG_CREATED", name);
            this.emit("organization-created", { orgId: id, name });
        ...
        getOrganization(orgId) { const o = this.#orgs.get(orgId); return o ? this.#deepClone(o) : null; }
        updateOrganization(orgId, changes) {
            const org = this.#orgs.get(orgId);
            if (!org) throw new Error(`[CozyCompany] updateOrganization(): unknown orgId "${orgId}".`);
            const clean = {};
            ...
            this.emit("organization-updated", { orgId });
            return this.#deepClone(org);
        }
        archiveOrganization(orgId) {
            const org = this.#orgs.get(orgId);
            if (!org) throw new Error(`[CozyCompany] archiveOrganization(): unknown orgId "${orgId}".`);
            org.status = "archived"; org.updatedAt = new Date().toISOString();
            this.#logAudit("ORG_ARCHIVED", orgId);
            this.emit("organization-archived", { orgId });
            return true;
        }
        restoreOrganization(orgId) {
            const org = this.#orgs.get(orgId);
            if (!org) throw new Error(`[CozyCompany] restoreOrganization(): unknown orgId "${orgId}".`);
            org.status = "active"; org.updatedAt = new Date().toISOString();
            this.#logAudit("ORG_RESTORED", orgId);
            this.emit("organization-restored", { orgId });
            return true;
        }
        deleteOrganization(orgId) {
            const org = this.#orgs.get(orgId);
            if (!org) throw new Error(`[CozyCompany] deleteOrganization(): unknown orgId "${orgId}".`);
            org.status = "deleted"; org.updatedAt = new Date().toISOString();
            this.#logAudit("ORG_DELETED", orgId);
            this.emit("organization-deleted", { orgId });
            return true;
        }
        listOrganizations({ includeDeleted = false } = {}) {
        ...
        /** archiveDepartment()/restoreDepartment() — real, additive (Rule 23): status transitions built on the existing updateDepartment(), never duplicating its merge logic. */
        archiveDepartment(companyId, departmentId) {
            this.updateDepartment(companyId, departmentId, { status: "archived" });
            this.#logAudit("DEPARTMENT_ARCHIVED", `${departmentId} archived for ${companyId}.`);
            return true;
        }
        restoreDepartment(companyId, departmentId) {
            this.updateDepartment(companyId, departmentId, { status: "active" });
            this.#logAudit("DEPARTMENT_RESTORED", `${departmentId} restored for ${companyId}.`);
            return true;
        }

        // =====================================================================
        // ─── DIVISION CRUD ────────────────────────────────────────────────────
        // Genuinely new — no prior overlap with any engine. Sits between
        // Branch and Department in the hierarchy: Company -> Branch ->
        // Division -> Department -> Team.
        // =====================================================================

        createDivision(companyId, branchId, divisionInput) {
            this.#getCompanyOrThrow(companyId);
            if (!divisionInput || typeof divisionInput.name !== "string" || !divisionInput.name.trim()) {
                throw new TypeError("[CozyCompany] createDivision(): missing required field name.");
            }
            const divisionId = this.#generateId("div");
            const now = new Date().toISOString();
            const record = { divisionId, companyId, branchId: branchId ?? null, name: this.#escapeHtml(divisionInput.name), status: "active", createdAt: now, updatedAt: now };
            this.#divisions.set(divisionId, record);
            this.#diagnostics.divisionsCreated++;
            this.#logAudit("DIVISION_CREATED", `${divisionId} (${divisionInput.name}) for ${companyId}.`);
            this.emit("division-created", { companyId, branchId, divisionId, name: divisionInput.name });
            return this.#deepClone(record);
        }
        getDivision(divisionId) { const d = this.#divisions.get(divisionId); return d ? this.#deepClone(d) : null; }
        updateDivision(divisionId, changes) {
            const division = this.#divisions.get(divisionId);
            if (!division) throw new Error(`[CozyCompany] updateDivision(): unknown divisionId "${divisionId}".`);
            const clean = {};
            if (changes && typeof changes.name === "string" && changes.name.trim()) clean.name = this.#escapeHtml(changes.name);
            Object.assign(division, clean, { updatedAt: new Date().toISOString() });
            this.#logAudit("DIVISION_UPDATED", divisionId);
            this.emit("division-updated", { divisionId });
            return this.#deepClone(division);
        }
        archiveDivision(divisionId) {
            const division = this.#divisions.get(divisionId);
            if (!division) throw new Error(`[CozyCompany] archiveDivision(): unknown divisionId "${divisionId}".`);
            division.status = "archived"; division.updatedAt = new Date().toISOString();
            this.#logAudit("DIVISION_ARCHIVED", divisionId);
            return true;
        }
        restoreDivision(divisionId) {
            const division = this.#divisions.get(divisionId);
            if (!division) throw new Error(`[CozyCompany] restoreDivision(): unknown divisionId "${divisionId}".`);
            division.status = "active"; division.updatedAt = new Date().toISOString();
            this.#logAudit("DIVISION_RESTORED", divisionId);
            return true;
        }
        deleteDivision(divisionId) {
            const division = this.#divisions.get(divisionId);
            if (!division) throw new Error(`[CozyCompany] deleteDivision(): unknown divisionId "${divisionId}".`);
            division.status = "deleted"; division.updatedAt = new Date().toISOString();
            this.#logAudit("DIVISION_DELETED", divisionId);
            return true;
        }
        listDivisions({ companyId = null, branchId = null, includeDeleted = false } = {}) {
            return Array.from(this.#divisions.values())
                .filter(d => (includeDeleted || d.status !== "deleted") && (!companyId || d.companyId === companyId) && (!branchId || d.branchId === branchId))
                .map(d => this.#deepClone(d));
        }

        // =====================================================================
        // ─── TEAM CRUD ────────────────────────────────────────────────────────
        // Genuinely new — no prior overlap with any engine. Sits below
        // Department in the hierarchy. References departmentId as a plain
        // identifier only (Department itself is owned by this same engine's
        // existing per-company Department CRUD above).
        // =====================================================================

        createTeam(companyId, departmentId, teamInput) {
            this.#getCompanyOrThrow(companyId);
            if (!teamInput || typeof teamInput.name !== "string" || !teamInput.name.trim()) {
                throw new TypeError("[CozyCompany] createTeam(): missing required field name.");
            }
            const teamId = this.#generateId("team");
            const now = new Date().toISOString();
            const record = { teamId, companyId, departmentId: departmentId ?? null, name: this.#escapeHtml(teamInput.name), lead: teamInput.lead ? this.#escapeHtml(teamInput.lead) : null, status: "active", createdAt: now, updatedAt: now };
            this.#teams.set(teamId, record);
            this.#diagnostics.teamsCreated++;
            this.#logAudit("TEAM_CREATED", `${teamId} (${teamInput.name}) for ${companyId}.`);
            this.emit("team-created", { companyId, departmentId, teamId, name: teamInput.name });
            return this.#deepClone(record);
        }
        getTeam(teamId) { const t = this.#teams.get(teamId); return t ? this.#deepClone(t) : null; }
        updateTeam(teamId, changes) {
            const team = this.#teams.get(teamId);
            if (!team) throw new Error(`[CozyCompany] updateTeam(): unknown teamId "${teamId}".`);
            const clean = {};
            if (changes && typeof changes.name === "string" && changes.name.trim()) clean.name = this.#escapeHtml(changes.name);
            if (changes && typeof changes.lead === "string") clean.lead = this.#escapeHtml(changes.lead);
            Object.assign(team, clean, { updatedAt: new Date().toISOString() });
            this.#logAudit("TEAM_UPDATED", teamId);
            this.emit("team-updated", { teamId });
            return this.#deepClone(team);
        }
        archiveTeam(teamId) {
            const team = this.#teams.get(teamId);
            if (!team) throw new Error(`[CozyCompany] archiveTeam(): unknown teamId "${teamId}".`);
            team.status = "archived"; team.updatedAt = new Date().toISOString();
            this.#logAudit("TEAM_ARCHIVED", teamId);
            return true;
        }
        restoreTeam(teamId) {
            const team = this.#teams.get(teamId);
            if (!team) throw new Error(`[CozyCompany] restoreTeam(): unknown teamId "${teamId}".`);
            team.status = "active"; team.updatedAt = new Date().toISOString();
            this.#logAudit("TEAM_RESTORED", teamId);
            return true;
        }
        deleteTeam(teamId) {
            const team = this.#teams.get(teamId);
            if (!team) throw new Error(`[CozyCompany] deleteTeam(): unknown teamId "${teamId}".`);
            team.status = "deleted"; team.updatedAt = new Date().toISOString();
            this.#logAudit("TEAM_DELETED", teamId);
            return true;
        }
        listTeams({ companyId = null, departmentId = null, includeDeleted = false } = {}) {
            return Array.from(this.#teams.values())
                .filter(t => (includeDeleted || t.status !== "deleted") && (!companyId || t.companyId === companyId) && (!departmentId || t.departmentId === departmentId))
    ...
    })({ sourcePath: "core/modules/company/cozy-company.js",
        name: "Company",
        category: "Foundation",
        icon: "company.svg",
    ...
    /**
     * Kernel registration — real, guarded, optional. Identical pattern to
     * the one already certified for IdentityEngine: Bootstrap.
     * registerService() -> initializeService() -> verifyService() (a real
     * health check, not a fabricated always-true) -> startService().
     * Diagnostics observes this automatically via its own event
     * subscriptions — no separate recording call is made here (Rule 2).
     *
     * Never breaks Company's own function if the Kernel Bridge isn't
     * present or fails — Kernel integration is additive, not a hard
     * requirement for this engine to work standalone.
     */
    ...
    async function registerWithKernel() {
        ...
        try {
            await bootstrap.registerService({ name: "Company", version: COMPANY_VERSION, apiVersion: "1.0.0", mandatory: true, dependencies: [] });
            bootstrap.initializeService("Company");
            await bootstrap.verifyService("Company", async () => window.CozyOS.Company.getVersion() === COMPANY_VERSION);
            bootstrap.startService("Company");
        } catch (_err) { /* non-fatal — Company remains fully functional standalone even if Kernel registration fails */ }
    }
    registerWithKernel();
