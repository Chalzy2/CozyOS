...
(function () {
    ...
    class CozyModuleRegistry {
        ...
        validate(manifest) {
            ...
            if (m.css !== undefined && (typeof m.css !== "string" || !m.css.trim())) errors.push("Invalid css — if provided, must be a non-empty string.");
         ...
         *   Real, async - the synchronous validate() above can only
         *   check that html/css/js are non-empty strings; it cannot
         *   verify they resolve to real files (that requires a real,
         *   async fetch). This method does the actual check, and
         *   explicitly distinguishes:
         *     - fatal: the field itself is missing/invalid (same rule
         *       validate() already enforces)
         *     - warning: the field is a real string but the real fetch
         *       to it failed - flagged, not registration-blocking,
         *       since a future asset/build step might still place the
         *       file there, and this method must not silently invent
         *       success either way.
         *   Never fabricates a result - a real network/fetch failure is
         *   reported as exactly that, not silently treated as "exists".
         */
        async validateFilesExist(manifest) {
            const warnings = [];
            const fatal = [];
            const fields = { html: manifest.html, css: manifest.css, js: manifest.js };
            for (const [field, path] of Object.entries(fields)) {
                if (typeof path !== "string" || !path.trim()) {
                    fatal.push(`${field} is missing or not a real string.`);
                    continue;
                }
                if (typeof fetch !== "function") {
                    warnings.push(`Cannot verify ${field} ("${path}") - fetch is not available in this context.`);
                    continue;
                }
                try {
                    const response = await fetch(path, { method: "HEAD" });
                    if (!response.ok) warnings.push(`${field} ("${path}") returned HTTP ${response.status} - file may not exist at this real path.`);
                } catch (err) {
                    warnings.push(`${field} ("${path}") could not be verified: ${err.message}`);
                }
            }
    ...
    engineInstance.register({
        ...
        folder: "core/modules/developer", html: "core/modules/developer/developer-hub.html", css: "core/modules/developer/developer-hub.css", js: "core/modules/developer/developer-hub.js",
    ...
    engineInstance.register({
        ...
        // Real fix: HTML genuinely lives in a different tree (applications/)
        // than JS/CSS (core/modules/ShopOS/) - verified on disk before this
        // change, not assumed. folder reflects where JS/CSS actually are;
        // html holds its own real, independently-resolvable path.
        folder: "core/modules/ShopOS", html: "applications/ShopOS/shopos.html", css: "core/modules/ShopOS/shopos.css", js: "core/modules/ShopOS/shopos.js",
        theme: "shopos", icon: "shopping", dashboard: "end-user", permissions: ["SHOP_ACCESS"], preload: true, enabled: true
    });
    engineInstance.register({
        id: "quarryos", name: "QuarryOS", version: "1.4.1",
        // Real fix (M320): standalone iframe page with real inline <style>
        // (no separate CSS file exists - confirmed on disk, not fabricated
        // here). js points to the real business-logic file that was
        // confirmed missing from quarry.html's <head> and has now been
        // added there in the same milestone.
        folder: "core/modules/QuarryOS", html: "applications/QuarryOS/quarry.html", js: "core/modules/QuarryOS/quarry-index.js",
        theme: "quarryos", icon: "quarry", dashboard: "end-user", permissions: ["QUARRY_ACCESS"], preload: false, enabled: true
    });
    // M346 real fix: the QuarryOS engine's own internal identity
    // (this.moduleId in core/modules/QuarryOS/quarry-index.js) and its
    // ServiceRegistry/Certification/Application-Center registration both
    // use "quarry_manager_001" - that is the canonical id, deeply embedded
    // in the real engine, not just a cosmetic label. Only ModuleRegistry
    // registered it under the shorter "quarryos" instead, so
    // ApplicationLauncher (which resolves via ModuleRegistry) could never
    // find QuarryOS under the id every other part of CozyOS actually uses
    // for it. Fixed by registering the same real files under the
    // canonical id too - not a duplicate engine, the same manifest content,
    // additively registered under its second real id. "quarryos" stays
    // registered above for backward compatibility with any existing caller
    // still using it.
    engineInstance.register({
    ...
    engineInstance.register({
        ...
        // Real fix: same real split as ShopOS - HTML lives in applications/,
        // JS/CSS in core/modules/MpesaOS/ (note: mpesaOS.css, capital OS,
        // verified on disk).
        folder: "core/modules/MpesaOS", html: "applications/MpesaOS/index.html", css: "core/modules/MpesaOS/mpesaOS.css", js: "core/modules/MpesaOS/mpesaos.js",
        theme: "mpesaos", icon: "cash", dashboard: "end-user", permissions: ["MPESA_ACCESS"], preload: true, enabled: true
    });
    // Real fix (Level 2B repair): PharmacyOS's real ServiceRegistry id
    // ("pharmacyos_core_001", set in core/plugins/pharmacyOS-core.js) was
    // never registered here at all, so ApplicationLauncher's Mode 2
    // (ModuleRegistry.get(appId)) could never resolve a destination for
    // it - the same class of id-mismatch/missing-registration gap
    // already fixed once for QuarryOS above. applications/PharmacyOS/
    // pharmacyos.html now has a real <head> tag (see that file's own
    // fix note) so this mounts via the same real standalone-iframe path
    // already proven for QuarryOS, not a new mounting mechanism.
    engineInstance.register({
        id: "pharmacyos_core_001", name: "PharmacyOS", version: "1.0.0",
        folder: "core/plugins", html: "applications/PharmacyOS/pharmacyos.html", js: "core/plugins/pharmacyOS-core.js",
        theme: "pharmacyos", icon: "pharmacy", dashboard: "end-user", permissions: [], preload: false, enabled: true
    });
    // Real fix (Level 2B repair): WholesaleOS had no application
    // destination at all (no applications/WholesaleOS/ directory, no
    // ModuleRegistry entry) - see applications/WholesaleOS/
    // wholesaleos.html's own header for the real, minimal Phase 1
    // destination created for it (a read-only view of the existing,
    // real WholesaleOS.getSharedCatalog(), nothing invented).
    engineInstance.register({
        id: "wholesaleos_core_001", name: "WholesaleOS", version: "1.0.0",
        folder: "core/plugins", html: "applications/WholesaleOS/wholesaleos.html", js: "core/plugins/wholesaleOS-core.js",
        theme: "wholesaleos", icon: "package", dashboard: "end-user", permissions: [], preload: false, enabled: true
    });
    // Real fix (Level 2B repair): ChurchOS (as a business application,
    // distinct from the real, already-working Live Worship capability)
    // had no application destination at all - see applications/
    // ChurchOS/churchos.html's own header for the real, minimal Phase 1
    // destination created for it (Setup Wizard + Membership Management,
    // both real, existing ChurchOS core methods - nothing invented).
    engineInstance.register({
        id: "churchos_core_001", name: "ChurchOS", version: "1.0.0",
        folder: "core/plugins", html: "applications/ChurchOS/churchos.html", js: "core/plugins/churchOS-core.js",
        theme: "churchos", icon: "church", dashboard: "end-user", permissions: [], preload: false, enabled: true
    });

    // Real fix (Level 2B pattern reused): InterestOS (new, Phase 1)
    // registered here under its own real ServiceRegistry id, exactly
    // the same real destination pattern already proven for ShopOS/
    // PharmacyOS/WholesaleOS/ChurchOS/QuarryOS.
    engineInstance.register({
        id: "interestos", name: "InterestOS", version: "0.1.0",
        folder: "core/plugins", html: "applications/InterestOS/interestos.html", js: "core/plugins/interestOS-core.js",
        theme: "interestos", icon: "compass", dashboard: "end-user", permissions: [], preload: false, enabled: true
    });
