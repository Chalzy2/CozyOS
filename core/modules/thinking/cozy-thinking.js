...
(function () {
    ...
    class CozyThinkingEngine {
         ...
         */
        think({ evidence, interpretationsUsed = [], strategy = "evidence-based", type = "general", providerId = null, sessionId = null } = {}) {
            ...
            stamp("reasoning");
            let raw;
            try { raw = provider.fn({ evidence, interpretationsUsed, strategy, type }); }
            catch (err) {
        ...
        reason(request) { return this.think(request); }
        ...
        analyse(request) { return this.think({ ...request, strategy: request.strategy || "analytical" }); }

        // ── Alternatives / evaluation (provider-backed, never invented) ────
        generateAlternatives(evidence, { providerId = null, criteria = "best-option" } = {}) {
            ...
            const result = this.think({ evidence, strategy: "comparative", providerId });
        ...
        identifyRisks(evidence, opts = {}) { const r = this.think({ evidence, strategy: "risk-analysis", ...opts }); return r.success ? { success: true, isReal: true, risks: r.risks } : r; }
        identifyOpportunities(evidence, opts = {}) { const r = this.think({ evidence, strategy: "evidence-based", ...opts }); return r.success ? { success: true, isReal: true, opportunities: r.opportunities } : r; }
        generateProsCons(evidence, opts = {}) {
            const r = this.think({ evidence, strategy: "pros-and-cons", ...opts });
