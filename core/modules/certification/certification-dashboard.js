...
(function () {
    ...
    class CertificationDashboard {
        ...
        #sidebarSections() {
            return [
                ["dashboard", "Dashboard"], ["quick", "Quick Certification"], ["bugfixer", "CozyBugFixer"], ["builder", "CozyBuilder"], ["full", "Full Certification"],
        ...
        #devOpenWithBuilder(moduleId) {
            if (!this.#getBuilder()) { this.#builderContext = { moduleId, error: "CozyBuilder is not connected." }; this.#setSection("builder"); return; }
            try {
                const plan = this.#getBuilder().planFromDescription(`Build ${moduleId} Coordinator`);
                this.#builderContext = { moduleId, plan, error: null };
            } catch (err) {
                this.#builderContext = { moduleId, error: err.message };
            }
        ...
        #renderBuilderView() {
            ...
            if (!ctx) return `<h1>CozyBuilder</h1><div class="cz-panel">No module opened with CozyBuilder yet.</div>`;
            if (ctx.error || !ctx.plan) {
                return `<h1>CozyBuilder: ${escapeHtml(ctx.moduleId)}</h1>
                    <div class="cz-panel">
                        <p class="cz-muted">${escapeHtml(ctx.error || "Could not build a plan.")}</p>
                        <p class="cz-muted">CozyBuilder generates NEW coordinators from a plan — it doesn't edit "${escapeHtml(ctx.moduleId)}" in place. Use Repair with CozyBugFixer to modify this file.</p>
                    </div>`;
            }
            const plan = ctx.plan;
            return `<h1>CozyBuilder: ${escapeHtml(ctx.moduleId)}</h1>
                <p class="cz-subtitle">CozyBuilder generates NEW coordinators — it can't edit "${escapeHtml(ctx.moduleId)}" in place (use Repair with CozyBugFixer for that). This is a fresh build plan seeded from its name.</p>
                <div class="cz-panel">
                    <h3>Plan Preview</h3>
                    ${this.#renderKeyValueTable({
                        "Export Name": plan.exportName, "Responsibility": plan.responsibility, "Category": plan.category,
                        "Entities": (plan.entities || []).map(e => e.name).join(", ") || "none",
                        "Event Naming": (plan.eventSeparator || ":") + "-notation"
                    })}
                    <button class="cz-btn cz-btn-primary" data-action="dev-generate-builder" data-module="${escapeHtml(ctx.moduleId)}">🏗 Generate Coordinator</button>
                </div>
                <div class="cz-panel cz-dev-action-output-panel" id="cz-dev-action-output"></div>`;
        }

        #devGenerateFromBuilder(moduleId) {
            const ctx = this.#builderContext;
            if (!ctx || !ctx.plan) { this.#devOutput('<p class="cz-muted">No plan available.</p>'); return; }
            try {
                const result = this.#getBuilder().buildCoordinator(ctx.plan, { mode: "coordinator" });
                const fileEntries = Object.keys(result.files);
                this.#devOutput(`<h3>Generated ${fileEntries.length} file(s)</h3>
                    <p>Certification preview: <b>${escapeHtml(result.certificationPreview.verdict || "n/a")}</b> ${result.certificationPreview.available !== false ? `(${escapeHtml(result.certificationPreview.scorePercent)}%)` : ""}</p>
                    ${fileEntries.map(name => `<p>${escapeHtml(name)}</p>`).join("")}`);
        ...
        #dispatchAction(action, el) {
            switch (action) {
                ...
                case "dev-generate-builder": this.#devGenerateFromBuilder(el.getAttribute("data-module")); break;
