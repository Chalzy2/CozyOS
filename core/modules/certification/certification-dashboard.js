...
(function () {
    ...
    class CertificationDashboard {
        ...
        async #devOpenWithBugFixer(moduleId) {
            if (!this.#getBugFixer()) { this.#devOutput("<p>CozyBugFixer is not connected.</p>"); return; }
            try {
                const fileId = this.#ensureWorkspaceFile(moduleId);
                const bfFileId = await this.#getWorkspace().shareToBugFixer(fileId);
                const analysis = this.#getBugFixer().analyze(bfFileId);
        ...
        #devOpenWithBuilder(moduleId) {
            ...
            const builderInfo = this.#getBuilder() ? `CozyBuilder v${escapeHtml(this.#getBuilder().getVersion())} is connected.` : "CozyBuilder is not connected.";
