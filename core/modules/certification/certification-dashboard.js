...
(function () {
    ...
    class CertificationDashboard {
        ...
        constructor() {
            ...
            this.#bugfixer = window.CozyOS && window.CozyOS.BugFixer ? window.CozyOS.BugFixer : null;
            this.#builder = window.CozyOS && window.CozyOS.Builder ? window.CozyOS.Builder : null;
        }
        ...
        async #handleQuickFileSelected(file) {
            ...
            const existing = meta.moduleId ? this.#workspace?.getExistingFileInfo?.(meta.moduleId) : null;
        ...
        #renderCertificationResult(result) {
            ...
            return `
                ...
                <div class="cz-panel" id="cz-dev-action-output"></div>`;
        }
         ...
         */
        #renderExistingFileInfo(moduleId) {
            if (!this.#workspace || typeof this.#workspace.getExistingFileInfo !== "function") return "";
            let info;
            try { info = this.#workspace.getExistingFileInfo(moduleId); } catch (_err) { return ""; }
            if (!info) return "";
        ...
        /** Gets-or-creates a Workspace fileId for a module already certified in this session. */
            ...
            if (!this.#workspace) throw new Error("WorkspaceShell is not connected.");
            const existing = this.#workspace.listFiles({ coordinator: moduleId })[0];
            ...
            return this.#workspace.registerFile({ filename, source });
        }

        async #devRepair(moduleId) {
            try {
                const fileId = this.#ensureWorkspaceFile(moduleId);
                const preview = await this.#workspace.repairAndRecertify(fileId, { approve: false });
        ...
        async #devConfirmRepair(moduleId, fileId) {
            try {
                const result = await this.#workspace.repairAndRecertify(fileId, { approve: true });
        ...
        async #devOpenWithBugFixer(moduleId) {
            if (!this.#bugfixer) { this.#devOutput("<p>CozyBugFixer is not connected.</p>"); return; }
            try {
                const fileId = this.#ensureWorkspaceFile(moduleId);
                const bfFileId = await this.#workspace.shareToBugFixer(fileId);
                const analysis = this.#bugfixer.analyze(bfFileId);
                this.#devOutput(`<h3>Opened in CozyBugFixer: ${escapeHtml(moduleId)}</h3>
                    <p>Suspicious findings: ${analysis.suspiciousFindings.length}. Certification available: ${analysis.certification.available}.</p>`);
            } catch (err) { this.#devOutput(`<p class="cz-muted">${escapeHtml(err.message)}</p>`); }
        }

        #devOpenWithBuilder(moduleId) {
            // CozyBuilder generates NEW coordinators from a plan — it does
            // not open or edit an existing file (that's CozyBugFixer's
            // job). Honest about that here rather than implying an "edit
            // in Builder" mode that doesn't exist.
            const builderInfo = this.#builder ? `CozyBuilder v${escapeHtml(this.#builder.getVersion())} is connected.` : "CozyBuilder is not connected.";
            this.#devOutput(`<h3>Open with CozyBuilder</h3>
                <p>${builderInfo}</p>
                <p class="cz-muted">CozyBuilder generates new coordinators from a plan — it doesn't edit an existing file like "${escapeHtml(moduleId)}" in place. Use Repair with CozyBugFixer to modify this file, or use the Builder dashboard separately to generate something new inspired by it.</p>`);
        }
        ...
        async #devSave(moduleId) {
            try {
                ...
                const result = await this.#workspace.saveFile(fileId, { proposedSource: source, approve: true });
        ...
        async #devSaveAs(moduleId) {
            ...
            try {
                ...
                const fileId = this.#workspace.registerFile({ filename: handle.name, source, handle });
        ...
        #devCompareVersions(moduleId) {
            try {
                const fileId = this.#ensureWorkspaceFile(moduleId);
                const comparison = this.#workspace.compareVersions(fileId);
