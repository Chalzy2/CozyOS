/**
 * CozyOS Certification Dashboard — Stylesheet
 * File Reference: core/modules/certification/certification-dashboard.css
 */
...
:root {
    ...
    --cz-panel-alt: #f6f7fb;
    --cz-sidebar: #10141f;
    --cz-sidebar-hover: #1b2130;
    --cz-sidebar-active: #2563eb;
    --cz-border: #e4e6ef;
    --cz-text: #101321;
    --cz-text-muted: #6b7280;
    --cz-text-inverse: #e7e9f2;
    --cz-accent: #2563eb;
    --cz-accent-soft: #eef2ff;
    --cz-critical: #dc2626;
    --cz-high: #ea580c;
    --cz-medium: #ca8a04;
    --cz-low: #2563eb;
    ...
    --cz-blocked: #dc2626;
    --cz-radius: 10px;
    --cz-shadow: 0 1px 2px rgba(16, 19, 33, 0.06), 0 4px 16px rgba(16, 19, 33, 0.06);
    --cz-font: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
    --cz-mono: "SFMono-Regular", Consolas, "Liberation Mono", Menlo, monospace;
}
...
html, body {
    ...
    color: var(--cz-text);
    background: var(--cz-panel-alt);
    -webkit-font-smoothing: antialiased;
}

#cozy-cert-dashboard-root { height: 100vh; }
...
.cz-sidebar {
    grid-area: sidebar;
    background: var(--cz-sidebar);
    color: var(--cz-text-inverse);
    overflow-y: auto;
    padding: 16px 0 24px;
}
...
.cz-sidebar-brand {
    ...
    padding: 0 20px 16px; color: #fff;
    display: flex; align-items: center; gap: 8px;
}
.cz-sidebar-brand .cz-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--cz-accent); display: inline-block; }

.cz-nav-group-label {
    font-size: 10px; text-transform: uppercase; letter-spacing: .08em;
    color: #6b7280; padding: 14px 20px 6px;
}

.cz-nav-item {
    display: flex; align-items: center; gap: 10px;
    padding: 9px 20px; font-size: 13.5px; cursor: pointer;
    color: #cbd2e1; border-left: 3px solid transparent;
    transition: background .12s ease, color .12s ease;
}
.cz-nav-item:hover { background: var(--cz-sidebar-hover); color: #fff; }
.cz-nav-item.active { background: var(--cz-sidebar-hover); color: #fff; border-left-color: var(--cz-sidebar-active); font-weight: 600; }
...
.cz-search {
    ...
    border: 1px solid var(--cz-border); border-radius: 8px;
    padding: 7px 12px; font-size: 13px; background: var(--cz-panel-alt);
}
.cz-search:focus { outline: 2px solid var(--cz-accent); outline-offset: 1px; }
.cz-topbar-meta { margin-left: auto; display: flex; gap: 18px; align-items: center; font-size: 12px; color: var(--cz-text-muted); }
.cz-topbar-meta b { color: var(--cz-text); }
.cz-bell { position: relative; cursor: pointer; font-size: 16px; }
.cz-bell .cz-bell-count {
    position: absolute; top: -6px; right: -8px; background: var(--cz-critical); color: #fff;
    font-size: 10px; border-radius: 999px; padding: 1px 5px; line-height: 1.3;
}

/* ---------- Main ---------- */
.cz-main { grid-area: main; overflow-y: auto; padding: 24px 28px 60px; }
.cz-main h1 { font-size: 20px; margin: 0 0 4px; }
.cz-main h2 { font-size: 15px; margin: 24px 0 10px; color: var(--cz-text); }
.cz-main h3 { font-size: 13px; margin: 16px 0 8px; color: var(--cz-text-muted); text-transform: uppercase; letter-spacing: .04em; }
...
table.cz-table tr:hover td { background: var(--cz-panel-alt); }
...
/* v2.0 additions: timeline, health map, dependency tree, progress tracker */
