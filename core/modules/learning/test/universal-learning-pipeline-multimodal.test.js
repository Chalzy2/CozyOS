...
// --- CP14 MERGE FIX: onStop handling + stopVoiceCapture() ---
// Real regression found while reconciling two independent lines of
// work (see docs/checkpoints/CP14-MERGE-KISWAHILI-HEARING-CHECKPOINT.md):
// the LivingHearingSession-based learnFromVoice() above only supplied
// onFinalResult/onError to session.start(), never onStop — reintroducing
// the exact hang-on-manual-Stop bug CP13 (Living Hearing Integration)
// had already fixed at the bare-adapter level, one layer down. Neither
// merged-in test file previously covered this at this level. These
// tests exercise the real fix: a real onStop event (with the session's
// own, already-tested wasExpectedStop flag) must resolve this Promise
// honestly, distinguishing a genuine user Stop from an unexpected one.

function fakeSessionForStopTests({ behavior } = {}) {
    return {
        start: (config, callbacks) => {
            if (behavior === 'user-stop') callbacks.onStop({ wasExpectedStop: true });
            else if (behavior === 'unexpected-stop') callbacks.onStop({ wasExpectedStop: false });
            // 'hang': nothing fires — proves this Promise only resolves via a real callback, never a fabricated timeout.
            return Promise.resolve({ success: true, state: 'LISTENING' });
        },
        stop: () => ({ success: true, state: 'STOPPED' }),
    };
}

test('learnFromVoice() resolves "Stopped by user." when the real session reports wasExpectedStop:true (a genuine user-initiated Stop) — this is the merge-fix regression test', async () => {
    const pipeline = freshWindow();
    global.window.CozyOS.Living = fakeLiving({ isReal: () => true });
    global.window.CozyOS.LivingHearingSession = fakeSessionForStopTests({ behavior: 'user-stop' });
...
test('learnFromVoice() resolves "No speech detected." when the real session reports wasExpectedStop:false (an unexpected disconnect, not a user Stop)', async () => {
    const pipeline = freshWindow();
    global.window.CozyOS.Living = fakeLiving({ isReal: () => true });
    global.window.CozyOS.LivingHearingSession = fakeSessionForStopTests({ behavior: 'unexpected-stop' });

    const result = await pipeline.learnFromVoice({});
    assert.equal(result.success, false);
    assert.equal(result.reason, 'No speech detected.');
});

test('stopVoiceCapture() calls the real, same LivingHearingSession.stop() — no second microphone/speech engine, and it is the path that actually settles a pending manual Stop end-to-end', async () => {
    const pipeline = freshWindow();
    const listeners = {};
    const session = {
        start: (config, callbacks) => { Object.assign(listeners, callbacks); return Promise.resolve({ success: true, state: 'LISTENING' }); },
        stop: () => { listeners.onStop({ wasExpectedStop: true }); return { success: true, state: 'STOPPED' }; },
    };
    global.window.CozyOS.Living = fakeLiving({ isReal: () => true });
    global.window.CozyOS.LivingHearingSession = session;

    const pending = pipeline.learnFromVoice({});
    const stopResult = pipeline.stopVoiceCapture();
...
test('stopVoiceCapture() reports honestly when LivingHearingSession is not loaded', () => {
    const pipeline = freshWindow();
    const result = pipeline.stopVoiceCapture();
    assert.equal(result.success, false);
    assert.match(result.reason, /LivingHearingSession is not loaded/);
});

// --- CP15: duplicate-final-result protection ---

test('CP15: learnFromVoice() resolves only once even if the real session fires onFinalResult twice for the same call — a duplicate final event never produces a second/different result', async () => {
    const pipeline = freshWindow();
    let capturedCallbacks = null;
    const session = {
        start: (config, callbacks) => { capturedCallbacks = callbacks; return Promise.resolve({ success: true, state: 'LISTENING' }); },
        stop: () => ({ success: true, state: 'STOPPED' }),
    };
    global.window.CozyOS.Living = fakeLiving({ isReal: () => true });
    global.window.CozyOS.LivingHearingSession = session;
    ...
    const pending = pipeline.learnFromVoice({});
    // Simulate a duplicate real onFinalResult firing (e.g. a genuine
    // provider-level double-fire, or a session/adapter-layer race) —
    // the SAME real event, twice, with different transcripts to prove
    // the second is genuinely ignored rather than silently winning.
    capturedCallbacks.onFinalResult({ transcript: 'Habari za asubuhi', confidence: 0.9, sessionId: 's1' });
    capturedCallbacks.onFinalResult({ transcript: 'a completely different second transcript', confidence: 0.1, sessionId: 's1' });

    const result = await pending;
    assert.equal(result.transcript, 'Habari za asubuhi', 'only the FIRST real final result may settle the call — a duplicate must never overwrite it');
});
