...
test('buildReviewCardText includes only the fields the observation actually has, never fabricating placeholders', () => {
    ...
    const observation = {
        ...
        audio: { transcript: 'buenos dias', language: 'es-ES' },
    ...
    assert.deepEqual(labels, ['Text', 'Spoken', 'Language', 'Meaning', 'Confidence']);
    assert.equal(card.lines.find((l) => l.label === 'Language').value, 'es-ES');
    assert.equal(card.confidencePercent, 95);
    assert.equal(card.recommendedAction, 'REVIEW_REQUIRED');
});

test('buildReviewCardText omits Language when audio.language was never provided (e.g. no language was configured/known)', () => {
    const core = load();
    const observation = { audio: { transcript: 'buenos dias', language: null } };
    const card = core.buildReviewCardText(observation, {});
    assert.ok(!card.lines.some((l) => l.label === 'Language'), 'must never show a Language line with no real value');
});
