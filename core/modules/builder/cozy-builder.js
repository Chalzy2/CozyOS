...
(function () {
    ...
    class CozyOSBuilderEngine {
         ...
         */
        buildCoordinator(spec, { mode = "coordinator" } = {}) {
        ...
        /**
         * buildCoordinatorWithAI(spec, { mode })
         ...
         *   this produces IDENTICAL output to buildCoordinator().
         */
        async buildCoordinatorWithAI(spec, { mode = "coordinator" } = {}) {
            if (!BUILD_MODES.includes(mode)) throw new TypeError(`[CozyBuilder] Unknown build mode "${mode}". Valid modes: ${BUILD_MODES.join(", ")}.`);
        ...
        #buildFromPlan(plan, mode) {
            ...
            files[`${folder}/cozy-${baseName}.js`] = templates.generateCoordinator(plan);
            ...
            const certificationPreview = this.#previewCertification(files[`${folder}/cozy-${baseName}.js`], plan.exportName);
            ...
            this.#logAudit("BUILD_COMPLETED", `${plan.exportName} built in "${mode}" mode — ${Object.keys(files).length} file(s).`);
            this.#logTimeline(`Build completed: ${plan.exportName} (${mode})`);
            this.emit("build:completed", { exportName: plan.exportName, mode, fileCount: Object.keys(files).length });

            return { files, plan, applicationManifest, certificationPreview, buildId: buildRecord.buildId };
        }
