 ...
 * ═══════════════════════════════════════════════════════════════════════
 *   Canonical Owner: trusted-device Administrator login, real Biometric
 *   Sign-In (Milestone 352 — composes the trusted-device AND
 *   security-key providers, never re-implementing either), and admin
 *   session listing / forced sign-out for sessions that originate from
 *   either path. Exactly the scope the prior stub declared plus the
 *   biometric login this milestone adds — this does not expand
 *   ownership beyond "trusted device / biometric administrator login."
 *
...
(function () {
    ...
    const ADMIN_RECOVERY_POLICY_VERSION = "1.1.0-ENTERPRISE"; // Milestone 352: attemptBiometricLogin()
    ...
    class CozyAdminRecoveryPolicy {
        ...
        /**
         * attemptBiometricLogin({ userId, deviceId })
         *   Milestone 352 — real "Biometric Sign-In" login path. Composes
         *   two already-real, already-registered AuthFactorRegistry
         *   providers, never re-implementing either:
         *     1. "trusted-device" (TrustedDeviceManager-backed) — the
         *        device must genuinely be owned by this administrator,
         *        currently trusted (30-day window), and not idle-locked.
         *     2. "security-key" (WebAuthnProvider-backed) — a real
         *        navigator.credentials.get() browser prompt, verified
         *        exactly as verify() already does for the Administrator
         *        Recovery Wizard's own Platform Authenticator method.
         *   Additionally requires TrustedDeviceManager's own
         *   `biometricEnabled` flag for this specific device — biometric
         *   sign-in is only ever offered after real enrollment on a
         *   trusted device (per spec), never as a bare WebAuthn check
         *   floating free of device trust.
         */
        async attemptBiometricLogin({ userId, deviceId } = {}) {
            if (!userId || !deviceId) return { granted: false, reason: "userId and deviceId are both required." };
            ...
            const tdm = this.#trustedDeviceManager();
            if (!tdm || typeof tdm.listDevicesForUser !== "function") {
                return { granted: false, reason: "TrustedDeviceManager is not loaded — cannot verify device enrollment. Failing closed." };
            }
            const device = tdm.listDevicesForUser(userId).find(d => d.deviceId === deviceId);
            if (!device) return { granted: false, reason: "This device is not registered to this administrator." };
            if (!device.biometricEnabled) return { granted: false, reason: "Biometric sign-in has not been enrolled on this device yet." };

            const registry = this.#factorRegistry();
            if (!registry || typeof registry.getProvider !== "function") {
                return { granted: false, reason: "AuthFactorRegistry is not loaded — cannot verify. Failing closed." };
            }
            const deviceProvider = registry.getProvider("trusted-device");
            if (!deviceProvider || typeof deviceProvider.verify !== "function") {
                return { granted: false, reason: "trusted-device factor provider is not registered. Failing closed." };
            }
            let deviceResult;
            try { deviceResult = await deviceProvider.verify({ userId, deviceId }); }
            catch (err) { return { granted: false, reason: `trusted-device verification threw: ${err.message}` }; }
            if (!deviceResult || !deviceResult.verified) {
                return { granted: false, reason: (deviceResult && deviceResult.reason) || "trusted-device verification failed." };
            }

            const biometricProvider = registry.getProvider("security-key");
            if (!biometricProvider || typeof biometricProvider.verify !== "function") {
                return { granted: false, reason: "security-key (biometric) factor provider is not registered. Failing closed." };
            }
            let biometricResult;
            try { biometricResult = await biometricProvider.verify({ userId }); }
            catch (err) { return { granted: false, reason: `Biometric verification threw: ${err.message}` }; }
            if (!biometricResult || !biometricResult.verified) {
                return { granted: false, reason: (biometricResult && biometricResult.reason) || "Biometric verification failed." };
            }

            if (typeof tdm.touchDevice === "function") {
                try { tdm.touchDevice(deviceId); } catch (_err) { /* non-fatal — verification already succeeded */ }
            }

            const sessionId = `adminrec_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
            const session = {
                id: sessionId, userId, deviceId, authMode: "biometric",
                revoked: false, revokedReason: null, since: new Date().toISOString()
            };
            this.#sessions.set(sessionId, session);
            this.#logHistory("login-granted", { sessionId, userId, deviceId, mode: "biometric" });
            this.#emit("login-granted", { sessionId, userId, deviceId, mode: "biometric" });

            return { granted: true, mode: "biometric", session: { id: sessionId } };
        }
    ...
    if (window.CozyOS.ServiceRegistry && typeof window.CozyOS.ServiceRegistry.registerCoordinator === "function") {
        try {
            window.CozyOS.ServiceRegistry.registerCoordinator({ sourcePath: "core/modules/identity/admin-recovery-policy.js",
