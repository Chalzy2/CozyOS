 ...
 *   - M389 correction: this comment previously claimed real browser
 *     Picture-in-Picture was already wired up. Verified false before
 *     this pass (grep found zero calls to requestPictureInPicture/
 *     documentPictureInPicture anywhere in the repository) - it is now
 *     actually implemented (see #wirePip/#togglePip below), using the
 *     real, standard Video Picture-in-Picture API, feature-detected via
 *     document.pictureInPictureEnabled, honestly disabled with a
 *     tooltip when unsupported. A genuine, smaller-than-normal
 *     in-CozyOS "mini" floating mode (drag/reposition/tap-to-restore,
 *     composing WindowManager's real setBounds()/getBounds() - M389)
 *     was also added for the case where the user wants to keep
 *     watching while working elsewhere inside CozyOS but hasn't left
 *     the browser tab. The player's own always-mounted, never-torn-down
 *     structure (see above) is what actually guarantees uninterrupted
 *     playback across ChurchOS navigation.
...
(function () {
    ...
    class LivingWorshipPlayer {
        ...
        #mountWindow() {
            ...
            const initialMode = prefs.mode || (prefs.theaterMode ? "theater" : "docked");
            const pipSupported = typeof document !== "undefined" && "pictureInPictureEnabled" in document && document.pictureInPictureEnabled;
            this.#root = document.createElement("div");
            this.#root.id = "cozy-worship-player-content";
            this.#root.innerHTML = `
                <div id="cozy-worship-player" data-mode="${initialMode}">
        ...
        #wireControls() {
            ...
            // it always intercepts pointer events before they reach the
            // <video> underneath - a separate click listener on the
            // video itself was dead code (unreachable via mouse/touch,
            // confirmed by an actual failing Playwright click). The
            // delegated [data-player-action] handler above already
            // covers the overlay button (data-player-action="restore-mini"
            // is set on it in the markup), so "tap the floating video to
            // restore" is handled by that single, real, reachable
            // control - not a second, unreachable one.
        }

        /**
         * #miniBounds()
         *   Real, responsive default for the floating mini window: 40%
         *   of viewport width, clamped between 180 and 320px, height
         *   derived from the actual 16:9 video aspect ratio (matches
         *   #cozy-worship-player-video's own CSS aspect-ratio - not a
         *   second, independently-guessed ratio). Falls back to a
         *   previously-dragged position if one was saved; otherwise
         *   defaults to the top-right corner, deliberately away from
         *   the LiveViewController chip's bottom-right default position
         *   (core/living/cozy-living.css #cozy-liveview-controller) so
         *   the two floating elements don't start out overlapping.
         */
        #miniBounds() {
            const saved = loadPrefs().miniBounds;
            const width = Math.min(320, Math.max(180, Math.round(window.innerWidth * 0.4)));
            const h = Math.round(width * (9 / 16)); // matches #cozy-worship-player-video's own 16:9 CSS aspect-ratio
            if (saved && saved.width) return { x: saved.x, y: saved.y, width: saved.width, height: saved.height, minWidth: 160, minHeight: 90 };
            return { x: window.innerWidth - width - 16, y: 72, width, height: h, minWidth: 160, minHeight: 90 };
        }
        ...
        #handleAction(action) {
            ...
            if (action === "expand") {
                ...
                savePrefs({ ...loadPrefs(), theaterMode: !isTheater, mode: el.dataset.mode });
        ...
        /**
         * #wirePip(pipSupported)
         *   M389 — the real "outside CozyOS" bridge this file's own
         *   header comment already claimed but never actually called
         *   (confirmed by grep before writing this: zero prior calls to
         *   requestPictureInPicture/documentPictureInPicture anywhere
         *   in the repository). Uses the standard, genuine browser
         *   Video Picture-in-Picture API - real feature detection via
         *   document.pictureInPictureEnabled, real button
         *   enabled/disabled state, never a fabricated "native Android"
         *   overlay (no Android project exists in this repository,
         *   confirmed absent). This is a true OS-level floating window
         *   on platforms that support it (desktop Chrome/Edge, Android
         *   Chrome); it is the strongest real capability this web-only
         *   codebase can offer without a native app shell.
         */
        #wirePip(pipSupported) {
            if (!pipSupported) return;
            this.#videoEl.addEventListener("leavepictureinpicture", () => {
                // Real, honest sync: if the user closed the native PiP
                // window while this player had been left in "mini"
                // mode, bring it back to a visible in-app state rather
                // than leaving a mini shell with no way to tell it
                // still has a live stream attached.
                const el = this.#root && this.#root.querySelector("#cozy-worship-player");
                if (el && el.dataset.mode === "mini") this.#handleAction("restore-mini");
            });
        }

        async #togglePip() {
            if (typeof document === "undefined" || !("pictureInPictureEnabled" in document) || !document.pictureInPictureEnabled) {
                return { success: false, reason: "Picture-in-Picture is not supported in this browser." };
            }
            try {
                ...
                // Honest failure surface (e.g. headless/automation
                // contexts, or a browser that reports the feature as
                // enabled but rejects this specific video) - never
                // silently pretends it worked.
                const statusEl = this.#root.querySelector("#cozy-worship-player-status");
                if (statusEl) statusEl.textContent = "Picture-in-Picture unavailable: " + err.message;
    ...
    class LiveViewController {
        ...
        mount() {
            ...
            // Same self-healing pattern already proven by
            // cozy-living-assistant.js's #panel (isConnected check +
            // re-append) — if #root exists in memory but was detached
            // from the page (e.g. a body-replacement flow that doesn't
            // know about this specific element), reuse the same node
            // rather than rebuilding it, so listeners/state aren't lost.
    ...
    window.CozyOS.Modules["living-worship-player"] = Object.freeze({
        version: VERSION,
        description: "ChurchOS Living Live Worship Player (C004) — floating/theater/fullscreen live video player composing LiveCaptureEngine/LiveHotspotEngine real streams, plus Live Translation/Scripture/Timeline panels composing real ChurchOS data. Worship Lyrics/Notes/Prayer Requests/Chat panels honestly report unavailable - no backend exists for them. Mounted outside #cozy-workspace-root (same pattern as the Living Assistant) so navigation never interrupts playback. M367: entry point is a real 3-state LiveViewController (minimized/expanded/hidden) instead of an unconditional auto-mounted window. M389: added a real 'mini' floating mode (drag/reposition/tap-to-restore, composing WindowManager's setBounds()/getBounds()) for floating inside CozyOS, plus a real, previously-unwired browser Picture-in-Picture bridge (feature-detected, honestly disabled when unsupported) for floating outside the CozyOS tab — no fabricated native Android overlay, since no native shell exists in this repository."
    });
