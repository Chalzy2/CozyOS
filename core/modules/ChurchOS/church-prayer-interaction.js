...
(function () {
    ...
    class ChurchPrayerInteraction {
        ...
        #isAuthorizedModerator(ldce, identity, requesterUserId, sessionId, hostUserId) {
            ...
            // restriction, applied here with the "moderate-prayer-
            // requests" scope for this file's own capability).
            if (orgId && typeof identity.isPlatformAdmin === "function" && identity.isPlatformAdmin(requesterUserId)) {
                ...
                if (support && typeof support.isSupportActive === "function") {
                    const active = support.isSupportActive(orgId, requesterUserId, { requiredScope: "moderate-prayer-requests" });
                    if (active.active) {
                        if (typeof support.recordSupportAction === "function") {
                            try { support.recordSupportAction(active.grantId, "prayer-moderation-authorized", { sessionId }); } catch (_err) { /* non-fatal */ }
                        }
