    ...
    class CozyOSDeploymentManager {
        ...
        async runDeployment(id, { providerName = this.#activeProviderName } = {}) {
            ...
            if (!provider.isConfigured()) {
                ...
                this.#recordHistory(deployment, providerName, "FAILED", notConfigured.reason || "Provider not configured.");
            ...
            if (validation && validation.success === false) {
                deployment.status = "VERIFICATION_FAILED";
                this.#recordHistory(deployment, providerName, "FAILED", validation.reason || "Validation failed.");
            ...
            try {
                ...
                this.#recordHistory(deployment, providerName, "SUCCESS", null);
                this.#logAudit("DEPLOYMENT_COMPLETED", `${id} -> ${providerName} release ${result.releaseId}`);
                this.emit("deployment:completed", { id, releaseId: result.releaseId, provider: providerName });
            } catch (err) {
                if (deployment.attempts < 3) {
                    ...
                    this.#recordHistory(deployment, providerName, "FAILED", err.message);
        ...
        #recordHistory(deployment, providerName, result, failureReason) {
            this.#history.push({
                id: deployment.id, moduleIds: deployment.moduleIds, version: deployment.name, provider: providerName,
