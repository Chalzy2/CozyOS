...
(function () {
    ...
    /**
     * ensureGeminiProviderRegistered()
     *   Domain 4C dependency #2 — the exact same idempotent, real, lazy
     *   registration pattern as ensureNllbProviderRegistered() above,
     *   for the real, existing, already-tested
     *   SpeechTranslationGeminiProvider.register(). Closes the real gap
     *   found in the Domain 4C review: nothing in the real application
     *   ever called this registration function, so
     *   preferredProviderName:'gemini-translate' could never actually
     *   find a registered provider even with real credentials and
     *   network — not an environment limitation, a wiring gap. Registers
     *   nothing new: this composes the same
     *   window.CozyOS.SpeechTranslationProviders registry NLLB already
     *   uses, via the pre-existing register() method on
     *   SpeechTranslationGeminiProvider (unmodified, no defect found
     *   there). Registering does not mean live Gemini calls will
     *   succeed — that still honestly depends on a real
     *   GEMINI_API_KEY/network at the server, exactly as already
     *   documented; this only makes the provider discoverable in the
     *   registry so translate() can attempt it and report the real
     *   outcome, never a fabricated one.
     */
    function ensureGeminiProviderRegistered() {
        const providers = window.CozyOS.SpeechTranslationProviders;
        const gemini = window.CozyOS.SpeechTranslationGeminiProvider;
        if (!providers || !gemini) return { success: false, reason: 'SpeechTranslationProviders/SpeechTranslationGeminiProvider not loaded.' };
        if (providers.hasAny() && providers.get('gemini-translate')) return { success: true, alreadyRegistered: true };
        try {
            gemini.register();
            return { success: true, alreadyRegistered: false };
        } catch (err) {
     ...
     *
     *   Domain 4C dependency #2 — when preferredProviderName is
     *   genuinely "gemini-translate", ensureGeminiProviderRegistered()
     *   is now also called (mirroring ensureNllbProviderRegistered()
     *   exactly), so the requested provider is actually discoverable.
     *   Every other existing caller — anyone not requesting Gemini —
    ...
    async function translateSegment({ segmentId, sourceLanguage, targetLanguage, sourceText, context = null, sourceTiming = null, pauseMetadata = null, preferredProviderName = null } = {}) {
        ...
        ensureNllbProviderRegistered();
        if (preferredProviderName === 'gemini-translate') {
            ensureGeminiProviderRegistered();
        }
