...
(function () {
    ...
    class CozyThemeController {
        ...
        registerTheme(themeName, aliases = [], options = {}) {
            ...
            // M346 — real, additive notification only. Lets a real listener
            // (LivingThemeEngine) auto-refresh its registry the moment a new
            // theme becomes real here, instead of only discovering it on next
            // page load. Non-fatal if the bus isn't loaded yet (e.g. this
            // file, by design, loads before platform-event-bus.js on some
            // pages) — this controller's own behavior never depends on it.
            if (window.CozyOS.PlatformEventBus && typeof window.CozyOS.PlatformEventBus.emit === "function") {
                try { window.CozyOS.PlatformEventBus.emit("theme:registered", { name: canonical }); } catch (_err) { /* non-fatal */ }
            }
