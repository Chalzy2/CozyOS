...
(function () {
    ...
    class CozyLivingBackground {
        constructor() {
            ...
            this._rotationTimerId = null; // set by startVideoRotation(), cleared by stopVideoRotation()
            this.ctx = null;
        ...
        /**
         * startVideoRotation(categoryList, {intervalMs})
         *   Real, single extension point for cycling the live background
         *   through multiple already-registered scenes, composing the
         *   existing playVideoFromLibrary() (same cross-fade, same
         *   setVideoSource(), same videoEl) rather than a second video
         *   engine or a rewritten play path.
         *
         *   WHY A TIMER, NOT THE VIDEO'S OWN "ended" EVENT: the product
         *   requirement is for each scene to keep looping seamlessly
         *   (videoEl.loop stays true throughout, unchanged from before
         *   this addition) rather than playing once and stopping —
         *   which means "ended" never actually fires (a looping
         *   HTMLMediaElement restarts itself instead of emitting
         *   "ended"), so it cannot be the rotation signal. A fixed
         *   dwell-time interval is the smallest real mechanism that can
         *   advance scenes while genuine per-scene looping is preserved.
         *
         *   Only rotates through categories that actually have at least
         *   one real registered video (via listVideosByCategory()) —
         *   never fabricates a scene, never rotates into an empty
         *   category. Idempotent: calling this again (e.g. a second
         *   real page composing revealLiveBackground() twice) always
         *   stops any previous rotation timer first, so repeated calls
         *   never stack multiple intervals advancing the same videoEl.
         */
        startVideoRotation(categoryList = [], { intervalMs = 8000 } = {}) {
            this.stopVideoRotation();
            const available = categoryList
                .map((category) => ({ category, entries: this.listVideosByCategory(category) }))
                .filter((c) => c.entries.length > 0);
            if (available.length === 0) return { success: false, reason: "No real registered videos to rotate through." };
            let index = 0;
            const advance = () => {
                const { category, entries } = available[index % available.length];
                this.playVideoFromLibrary(category, entries[0].id);
                index++;
            };
            advance(); // real, immediate first scene — no idle wait for the first interval tick
            if (available.length > 1) this._rotationTimerId = setInterval(advance, intervalMs);
            return { success: true, rotation: available.map((a) => a.category), intervalMs, willRotate: available.length > 1 };
        }

        /** stopVideoRotation() — real, idempotent; safe to call even if no rotation is active. */
        stopVideoRotation() {
            if (this._rotationTimerId) { clearInterval(this._rotationTimerId); this._rotationTimerId = null; return { success: true, stopped: true }; }
            return { success: true, stopped: false };
        }

        /** isRotating() — real, reflects whether a rotation timer is genuinely active right now. */
        isRotating() { return !!this._rotationTimerId; }
