...
function updateDashboardTelemetry() {
    ...
    // Milestone 200E: PluginMetadata is a real plain object (pluginManager.js's
    // own documented contract), never a Map - fixed to use Object.values().
    const plugins = window.CozyOS?.PluginMetadata || {};
    const enabledCount = Object.values(plugins).filter(m => m.status === 'enabled').length;
