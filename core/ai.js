 ...
 * FILE: core/ai.js
 * VERSION: 1.4.2 (Production Routing Engine with Capability Introspection)
 *
 ...
 *   [FIX-13] Scripture delegation (Milestone 342) — same additive pattern as FIX-12,
 *            run immediately after it. Detects a Bible reference via the real
 *            Living.scripture.detectReference() gateway (Milestone 340) — no local
 *            regex — and delegates the lookup to Living.scripture.lookup(), which
 *            composes BibleEngine. Never invents or modifies verse text; an
 *            unavailable/uninstalled verse is reported honestly. Fails closed and
 *            falls through unchanged if Living.scripture isn't registered.
 */
...
class CozyAIEngine {
    ...
    /**
     * [M342] Detects a real Bible reference in a free-text query by
     * composing the existing Living.scripture.detectReference() gateway
     * (M340) directly - no local regex is duplicated here, unlike
     * FIX-12's developer-identity matcher above (which has to be local
     * because DeveloperIdentity has no such detection method of its
     * own). Returns the first real detected reference, or null.
     * @returns {{book:string, chapter:number, verseStart:number}|null}
     */
    _matchScriptureReference(normalizedQuery) {
        const living = window.CozyOS?.Living;
        if (!living || !living.scripture || typeof living.scripture.detectReference !== "function") return null;
        const refs = living.scripture.detectReference(String(normalizedQuery || ""));
        return refs.length > 0 ? refs[0] : null;
    }

    /**
     * [M342] Delegates a detected scripture reference to the single
     * canonical owner, Living.scripture (M340) -> BibleEngine
     * (M334-M338). Never invents or modifies verse text - an
     * unavailable/uninstalled verse is reported honestly, not
     * fabricated. Governance: this NEVER routes verse text through any
     * translation/generation path - only real, licensed, installed
     * translations are ever returned.
     * @returns {{responseText:string, pipelineState:string, source:string}|null}
     */
    answerScriptureQuery(ref) {
        const living = window.CozyOS?.Living;
        if (!living || !living.scripture || typeof living.scripture.lookup !== "function") return null;
        const result = living.scripture.lookup(ref.book, ref.chapter, ref.verseStart);
        if (!result.available) {
            return { responseText: result.reason || `${ref.rawMatch} is not available - no licensed translation is installed.`, pipelineState: "unavailable", source: "Living.scripture" };
        }
        const firstTranslation = Object.values(result.translations)[0];
        return { responseText: `${result.reference}: "${firstTranslation.text}"`, pipelineState: "completed", source: "Living.scripture" };
    }
    ...
    async executeRoutingPhase(session, normalizedQuery) {
        ...
        try {
            ...
            if (typeof normalizedQuery === "string" && normalizedQuery.trim()) {
                ...
                // [M342] Scripture references are also meta - not a tenant/
                // industry plugin query - resolved via the single canonical
                // Living.scripture gateway (M340). Additive only: if no
                // reference is detected, or the gateway isn't registered,
                // falls straight through to the unchanged industry-routing
                // path below, same fail-closed pattern as FIX-12 above.
                const scriptureRef = this._matchScriptureReference(normalizedQuery.trim());
                if (scriptureRef) {
                    const scriptureAnswer = this.answerScriptureQuery(scriptureRef);
                    if (scriptureAnswer) {
                        inferredIntent  = "scripture_reference_query";
                        confidenceScore = 1.0;
                        executionStatus = "scripture_delegated";
                        return scriptureAnswer;
                    }
                }
            }
