 ...
 * Truncated identically across every available source (Layer2, Layer3,
 * milestones M173–M373). Unlike compression.js, no other file in the
 * repository ever instantiates or references this class, so class name is
 * taken from this file's own internal log tag ("[BandwidthShaper]") and the
 * constructor shape/export style follows the sibling compression.js module
 * (same author, same header format, same subsystem).
 *
 * `Profiles` values and `_immutableHeaderKeys` are UNRECOVERABLE — nothing
 * elsewhere in the repo defines them. Profile keys are set to their own
 * names as placeholder strings (functionally distinct, so branching logic
 * below is exercisable), and `_immutableHeaderKeys` is left empty, meaning
 * NO keys are currently protected from CRITICAL_LOW shedding. That is a real
 * behavioral gap, not a cosmetic one — flagged for engineering review below.
 * ─────────────────────────────────────────────────────────────────────────
 */
...
export class BandwidthShaper {
    constructor() {
        this.Profiles = {                    // INFERRED placeholders — real wire values unknown
            UNMETERED: "UNMETERED",
            METERED_CELLULAR: "METERED_CELLULAR",
            CRITICAL_LOW: "CRITICAL_LOW",
        };
        this._immutableHeaderKeys = [];      // INFERRED — EMPTY. Real protected-key list is unrecoverable; see notice above.
        this._metrics = {                    // INFERRED — minimal shape required by getShaperMetrics()/methods below
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // § 1. ADAPTIVE PAYLOAD SHAPING
    // ─────────────────────────────────────────────────────────────────────────

    /**
     * [RECONSTRUCTED SIGNATURE] Public entry point — name inferred from the
     * "Public API: applyDataQuotaCaps(payload)" line in the file's own header
     * above, since the original signature line was in the truncated portion.
     */
    applyDataQuotaCaps(payload) {
