 ...
 * This file was truncated identically across every available source (Layer2,
 * Layer3, milestones M173–M373). The class declaration, constructor, and the
 * opening of generateDeltaPayload() (including its Branch 1 condition, which
 * is inferred only from the surviving `basePayload.data.targetField` /
 * `numericDelta` reference below) were reconstructed to restore valid syntax.
 * `codecIdentifier` cross-referenced against the rest of the repo (including
 * sync.js, the only caller of this class) — no definition or value exists
 * anywhere, so it is initialized to null (INFERRED). ESTIMATED_SAVINGS_RATIO
 * is likewise unrecoverable and set to a neutral 0 rather than a guessed
 * business value. No algorithmic logic beyond the minimal `if` needed to
 * reach the surviving code was invented.
 * ─────────────────────────────────────────────────────────────────────────
 */

"use strict";

const ESTIMATED_SAVINGS_RATIO = 0; // INFERRED — real ratio unknown; neutral value so telemetry does not fabricate savings

export class BinaryCompressor {
    constructor() {
        this.codecIdentifier = null; // INFERRED — no definition/value found anywhere in the repo (sync.js instantiates with no args)
        this._metrics = {             // INFERRED — minimal shape required by getCompressionMetrics()/methods below
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // § 1. DELTA PAYLOAD GENERATION
    // ─────────────────────────────────────────────────────────────────────────

    /**
     * [RECONSTRUCTED SIGNATURE] Original JSDoc and the precise Branch 1
     * condition were not recoverable. This guard is the minimal condition
     * that makes the surviving Branch-1 body (targetField/numericDelta)
     * reachable, inferred directly from the fields it reads.
     */
    async generateDeltaPayload(taskItem) {
        const basePayload = taskItem;
        if (basePayload?.data?.targetField !== undefined || basePayload?.data?.numericDelta !== undefined) {
