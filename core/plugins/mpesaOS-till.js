 ...
 *   OWNERSHIP AUDIT: this file (window.CozyOS.MpesaTill) is the
 *   confirmed, canonical, actually-loaded live owner of MpesaOS till
 *   management — referenced by <script src="core/plugins/mpesaOS-till.js">
 ...
 *   docs/builder/knowledge/module-inventory.json. core/plugins/
 *   mpesaOS-engine.js contains a near-duplicate Till-Payment code path
 *   but is confirmed dead (no <script> reference anywhere, absent
 *   from discovery-manifest.json) — left untouched, not a second live
 *   owner.
 *
 *   Feature IDs reuse the exact convention established by receipts
 *   ("mpesa"/"mpesa.receipts"), reports ("mpesa"/"mpesa.reports"), and
 *   float ("mpesa"/"mpesa.float") — "mpesa.till" is added here,
 *   verbatim style. APPLICATION-level ("mpesa") is checked before
 *   FUNCTION-level ("mpesa.till"), the same cascade the other three
 *   integrations use.
 *
 *   Scope of THIS integration pass: registerTill() and setTillStatus()
 *   only — the two purely administrative Till actions. Confirmed by
 *   repository-wide call-site audit (grep across every live plugin) to
 *   have no internal caller anywhere; both are only ever invoked from
 *   UI/admin flows outside this file.
 *
 *   recordPayment() and withdrawFromTill() are deliberately left
 *   UNGUARDED here, and for two different real reasons:
 *     - recordPayment() is called directly by mpesaOS.js's engine for
 *       every committed "Till Payment" transaction (see
 *       core/plugins/mpesaOS.js, the transactionType === "Till
 *       Payment" branch) — it is the real money-movement hook, the
 *       exact Till-side analog of mpesaOS-float.js's
 *       recordTransactionImpact(). Gating it would let a real customer
 *       payment complete while the till's tracked balance silently
 *       failed to reflect it.
 *     - withdrawFromTill() is ALSO called directly by mpesaOS.js's
 *       engine — not only as a discretionary merchant action, but as
 *       the real best-effort ROLLBACK for a failed Till Payment
 *       transaction (see the `tillApplied` reversal branch in
 *       mpesaOS.js's transaction pipeline). Gating it would risk
 *       blocking that reversal and leaving a till balance
 ...
 *       "merchant withdrawing float externally" from "engine
 ...
 *   throws) when till management is not entitled for any reason —
 ...
 *   the same honest, fail-closed convention mpesaOS-float.js and
 *   mpesaOS-reporting.js already use. On success, the existing return
 *   shape (the deep-cloned till/status record) is unchanged.
 */
(function () {
    "use strict";
    ...
    const MPESA_FEATURE_TILL = "mpesa.till";
    ...
    class MpesaTillCoordinator {
        ...
        /**
         * #checkTillEntitlement(organizationId)
         *   The real service-boundary gate for the two purely
         *   administrative Till actions below (see file header for the
         *   full scope discussion — recordPayment()/withdrawFromTill()
         *   are deliberately never routed through this gate). Never
         *   throws; each guarded method reacts to the returned
         *   {allowed, state, reason} the same honest way
         *   mpesaOS-float.js's float gate already does. Fails closed —
         *   an absent EntitlementEngine or a missing organizationId is
         *   a denial, never a silent bypass.
         */
        #checkTillEntitlement(organizationId) {
            const entitlement = getEntitlementEngine();
            if (!entitlement || typeof entitlement.guard !== "function") {
                return { allowed: false, state: "UNAVAILABLE", reason: "Entitlement engine is not available — till management fails closed." };
            }
            if (typeof organizationId !== "string" || !organizationId.trim()) {
                return { allowed: false, state: "UNAVAILABLE", reason: "organizationId is required to evaluate till entitlement." };
            }
            try {
                entitlement.guard(organizationId, MPESA_FEATURE_APP);
                entitlement.guard(organizationId, MPESA_FEATURE_TILL);
                return { allowed: true };
            } catch (err) {
                const state = (err && err.decision && err.decision.state) || "DENIED";
                return { allowed: false, state, reason: `Till management is not available for this organization (${state}).` };
            }
        }
        ...
        registerTill(rawInput = {}) {
            const { companyId, branchId, tillNumber, merchantName, userId, organizationId } = sanitizeObject(rawInput);
            const gate = this.#checkTillEntitlement(organizationId);
            if (!gate.allowed) {
                this.#logAudit("TILL_ACTION_BLOCKED", `registerTill blocked (${gate.state}): ${companyId}/${branchId}`);
        ...
        /** setTillStatus(tillNumber, status, organizationId) — real, validated status change. Honestly refuses an unknown till or invalid status rather than silently no-op. */
        setTillStatus(tillNumber, status, organizationId) {
            const gate = this.#checkTillEntitlement(organizationId);
            if (!gate.allowed) {
                this.#logAudit("TILL_ACTION_BLOCKED", `setTillStatus blocked (${gate.state}): ${tillNumber}`);
                return { available: false, reason: gate.reason, state: gate.state };
            }
    ...
    function initRegistration() {
        ...
        if (window.CozyOS && window.CozyOS.PluginManager) {
            window.CozyOS.PluginManager.register(
                ...
                window.CozyOS.PluginManager.createMinimalIntentHandler(engineInstance, "MpesaOS Till Management")
            );
        } else {
