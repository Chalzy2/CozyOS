 ...
 *
 * OWNERSHIP: this is the real, single startup orchestrator for
 * index.html. It does NOT duplicate dashboard.html's script list as a
 * second, hand-maintained source of truth - it fetches dashboard.html's
 ...
 * inside index.html. dashboard.html remains completely unchanged and
 * fully functional as a standalone entry point (Cloudflare
    ...
    class CozyBootstrap {
        ...
        async start() {
            ...
            try {
                ...
                if (!response.ok) throw new Error(`Real fetch of dashboard.html returned HTTP ${response.status}.`);
                html = await response.text();
            } catch (err) {
                this.#state = "failed";
                this.#emitBootstrapEvent("bootstrap:failed", { reason: err.message });
                return { success: false, reason: `Could not real-fetch dashboard.html: ${err.message}` };
            }
            ...
            const persistedIds = ["cozy-live-bg-canvas", "cozy-live-bg-video"];
