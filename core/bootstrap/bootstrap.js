...
(function () {
    ...
    class CozyBootstrap {
        ...
        // REAL RETRY FIX (reported live failure: "Administrator workspace
        // failed to load: Real script load failed: <path>" — confirmed,
        // by direct manual re-fetch of the exact failing file, to be a
        // genuinely transient failure: the file, its path/case, and the
        // real server serving it were all correct moments later). Out of
        // the 270+ real scripts admin-workspace.html loads strictly
        // sequentially, a single dropped/timed-out request on a mobile
        // connection previously aborted the ENTIRE sequence with no
        // retry at all. Bounded to 3 real attempts total (not infinite —
        // a genuinely, persistently broken script must still fail
        // loudly, never be silently swallowed) with a short real
        // backoff between attempts, since an immediate retry is often no
        // more likely to succeed than the original attempt on a flaky
        // connection.
        async loadExternalScript(item, attempt = 1) {
            ...
            try {
                ...
                return this.loadExternalScript(item, attempt + 1);
            }
        }
