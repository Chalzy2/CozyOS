 ...
 *     owned by the real, existing CameraEngine/AudioManager — this file
...
(function () {
    ...
    class LiveCaptureEngine {
         ...
         *   unmodified CameraEngine/AudioManager, using their own
        ...
        registerProviders() {
            ...
            // AA-004 resolution (M387.5b): this file calls registerProvider(),
            // which only exists on the bridge's Audio Manager — renamed to
            // window.CozyOS.AudioManager to stop colliding with the separate,
            // already-real window.CozyOS.AudioEngine (cozy-audio-engine.js,
            // the Listening Engine cozy-hearing.js depends on instead).
            const audioEngine = window.CozyOS.AudioManager;
            ...
            if (!this.#audioProviderRegistered && audioEngine && typeof audioEngine.registerProvider === "function") {
                ...
                results.audio = this.#audioProviderRegistered ? { success: true, alreadyRegistered: true } : { success: false, reason: "AudioManager is not loaded." };
            }
    ...
    window.CozyOS.Modules["live-capture-engine"] = Object.freeze({
        version: VERSION,
        description: "Live Capture Engine (ChurchOS C003) — genuine new feature: real getUserMedia/MediaRecorder-based camera/microphone capture, start/pause/resume/stop, live preview. Registers real providers with the existing CameraEngine/AudioManager (their own documented provider interface, not a new device registry). Streaming and full RecordExportSessionManager frame-array integration are explicitly out of scope, disclosed, not fabricated."
    });

    // Auto-register providers once CameraEngine/AudioManager exist. They
