...
class CozyPluginManager {
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // 1.5 MINIMAL INTENT-HANDLER ADAPTER (Real bug fix — M387.5 Finding 6)
    // ──────────────────────────────────────────────────────────────────────────

    /**
     * register()'s second argument must be a callable
     * `(query, kernelContext) => result` intent handler (see `execute()`
     * below, which calls it exactly that way) — it is NOT the plugin's real
     * engine instance. `pharmacyOS.js`, `hospitalOS.js`, and the two
     * `mpesaOS*.js` files already define their own small, named
     * `xxxExecutionCore(query, kernelContext)` function for this and pass
     * *that*. 15 other plugin files (ShopOS ×10, MpesaOS ×4, one more)
     * mistakenly passed their real engine class instance directly instead,
     * which is why `register()`'s own `[R-4]` type guard rejected them.
     *
     * This adapter is the smallest fix that satisfies the real contract for
     * those 15 without inventing conversational command-routing logic they
     * never had: it wraps the engine instance in a function that honestly
     * discloses it is a minimal, generic intent handler (not a business-logic
     * router), and surfaces the engine's own real, already-existing
     * `getDiagnosticsReport()` — the near-universal diagnostics method every
     * one of these engine classes already implements — rather than
     * fabricating query-specific responses these engines were never given.
     *
     * @param {object} engineInstance — the real, already-constructed engine.
     * @param {string} pluginLabel — human-readable name for the response text.
     * @returns {function(string, object): object} a valid `executionHandler`.
     */
    createMinimalIntentHandler(engineInstance, pluginLabel) {
        return function (query, kernelContext) {
            if (kernelContext && typeof kernelContext.auditLogging === "function") {
                kernelContext.auditLogging("IntentReceived", { query });
            }
            const diagnostics = (engineInstance && typeof engineInstance.getDiagnosticsReport === "function")
                ? engineInstance.getDiagnosticsReport()
                : { note: `[${pluginLabel}] No getDiagnosticsReport() available on this engine.` };
            return {
                responseText: `[${pluginLabel}] Intent received via PluginManager. This plugin's ` +
                    "PluginManager-routed handler is honestly minimal — no conversational " +
                    `command routing is implemented for "${pluginLabel}" yet. Use its real ` +
                    "engine API directly (the same instance registered under window.CozyOS) " +
    ...
    _compareVersions(v1, v2) {
        // Strip any semver pre-release suffix (e.g. "1.0.0-ENTERPRISE" -> "1.0.0")
        // before splitting, so a widened SEMVER_RE can't reintroduce the NaN
        // comparison bug this guard originally existed to prevent (R-2).
