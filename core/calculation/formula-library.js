...
(function () {
    ...
    function register() {
        ...
        registry.register("Business.LineTotal", {
            // The one genuinely missing generic formula identified by the
            // BusinessRecordEngine ownership audit (Universal Business
            // Calculation milestone) — quantity × unit price. Reused for
            // both cost (quantity × buyingPrice) and sales
            // (quantity × sellingPrice) by supplying different unitPrice
            // inputs; no separate "cost formula" / "sales formula" pair is
            // needed. Added here (not inside BusinessRecordEngine) so any
            // application/pack can reuse it and there is exactly one
            // authority for this arithmetic.
            fn: ({ quantity, unitPrice }) => {
                if (quantity < 0 || unitPrice < 0) throw new Error("Business.LineTotal: quantity and unitPrice must both be non-negative.");
                return quantity * unitPrice;
            },
            requiredInputs: ["quantity", "unitPrice"], version: "1.0.0", pack: "Business",
            description: "Line total for a quantity at a given unit price (e.g. cost = quantity × buying price, sales = quantity × selling price). Rejects negative inputs.",
            sampleInputs: { quantity: 10, unitPrice: 100 }, nonNegativeKeys: ["quantity", "unitPrice"]
        });
