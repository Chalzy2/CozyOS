...
(function () {
    ...
    const COORDINATOR_DEFAULTS = Object.freeze({
        ...
        icon: null,
        description: null,
        sourcePath: null // real file path, needed for automatic certification - null means "not yet certifiable"
    });
    ...
    // Register itself, same as every other coordinator — otherwise any
    // consumer counting "discovered coordinators" on window.CozyOS (which
    // legitimately includes ServiceRegistry, a real coordinator) will never
    // match this registry's own "registered coordinators" count, which
    // would otherwise never include the registry itself.
    try {
        registry.registerCoordinator({
            name: "ServiceRegistry", category: "Foundation", icon: "service-registry.svg",
            description: "CozyOS Service Registry — single metadata catalog of every application and coordinator on this install."
        });
