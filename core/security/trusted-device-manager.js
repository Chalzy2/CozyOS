...
(function () {
    ...
    const TRUSTED_DEVICE_VERSION = "1.1.0-ENTERPRISE"; // 1.1.0: Milestone 360 — added renameDevice() (additive only, Rule 3 / Principle 23 approved)
    ...
    class CozyTrustedDeviceManager {
        ...
        constructor() {
            this.ready = this.#restorePersistedDevices();
        }

        /**
         * #restorePersistedDevices()
         *   Real fix — devices were never persisted at all; every
         *   refresh wiped biometricEnabled/biometricPromptShown,
         *   causing the "Enable biometric unlock" prompt to reappear
         *   every visit even after the user enabled it. Restores from
         *   the existing IdentityStorage "trustedDevices" store (already
         *   a valid store name — confirmed before writing this).
         */
        async #restorePersistedDevices() {
            const storage = window.CozyOS.IdentityStorage;
            if (!storage || typeof storage.loadAll !== "function") return;
            try {
                const result = await storage.loadAll("trustedDevices");
                if (result.success) for (const device of result.records) this.#devices.set(device.deviceId, device);
            } catch (_err) { /* honestly non-fatal - in-memory state remains authoritative for this session */ }
        }

        #persist(device) {
            const storage = window.CozyOS.IdentityStorage;
            if (storage && typeof storage.save === "function") {
                storage.save("trustedDevices", { id: device.deviceId, ...device }).catch(() => { /* honestly non-fatal */ });
            }
        }
        ...
        registerDevice(userId, { nickname, fingerprint } = {}) {
            ...
            const device = {
                ...
                // Milestone 205 — real biometric tracking, extending this
                // existing device record rather than creating separate
                // storage. biometricPromptShown ensures the "Enable
                // Fingerprint Unlock?" onboarding appears only once per
                // real device.
                biometricEnabled: false, biometricPromptShown: false
            };
        ...
        /**
         * findDeviceForUser(userId, fingerprint) — real (Milestone 205).
         * Needed to determine whether a trusted device record already
         * exists for this specific user+browser combination before
         * deciding whether the one-time biometric prompt should appear.
         * Never returns a revoked device.
         */
        findDeviceForUser(userId, fingerprint) {
            for (const device of this.#devices.values()) {
                if (device.userId === userId && device.fingerprint === fingerprint && !device.revoked) {
                    return this.#deepClone(device);
                }
            }
            return null;
        }
        ...
        /**
         * markBiometricPromptShown(deviceId) — real (Milestone 205, Gate
         * 2/3). Ensures the "Enable Fingerprint/Face Unlock?" onboarding
         * appears only once per real device, regardless of how many
         * times that device logs in afterward.
         */
        markBiometricPromptShown(deviceId) {
            const device = this.#devices.get(deviceId);
            if (!device) return { success: false, reason: "No real, registered device with that id." };
            device.biometricPromptShown = true;
            this.#persist(device);
            return { success: true };
        }

        /**
         * setBiometricEnabled(deviceId, enabled) — real (Milestone 205,
         * Gate 4/9). Records whether this specific real device has
         * biometric authentication enabled — reuses this existing device
         * record, never a second storage location.
         */
        setBiometricEnabled(deviceId, enabled) {
            const device = this.#devices.get(deviceId);
            if (!device) return { success: false, reason: "No real, registered device with that id." };
            device.biometricEnabled = !!enabled;
            this.#persist(device);
            this.#logHistory(enabled ? "biometric-enabled" : "biometric-disabled", { deviceId, userId: device.userId });
            return { success: true };
        }
        ...
        /**
         * renameDevice(deviceId, nickname) — real, additive (Milestone
         * 360; approved under Rule 3 / Principle 23 after the Stage 1
         * provider audit confirmed no rename path existed anywhere in
         * this codebase). Updates only the device's own nickname field
         * — trust state, activity clock, and every other field are
         * untouched. Does not alter registerDevice() or any other
         * existing method.
         */
        renameDevice(deviceId, nickname) {
            ...
            if (!nickname || typeof nickname !== "string" || !nickname.trim()) {
                return { success: false, reason: "A real, non-empty nickname is required." };
            }
            const previous = device.nickname;
            device.nickname = nickname.trim();
            this.#persist(device);
            this.#logHistory("device-renamed", { deviceId, userId: device.userId, previous, nickname: device.nickname });
            this.#emit("device-renamed", { deviceId, userId: device.userId, previous, nickname: device.nickname });
            return { success: true, device: this.#deepClone(device) };
        }
    ...
    if (window.CozyOS.ServiceRegistry && typeof window.CozyOS.ServiceRegistry.registerCoordinator === "function") {
        try {
            window.CozyOS.ServiceRegistry.registerCoordinator({ sourcePath: "core/security/trusted-device-manager.js",
