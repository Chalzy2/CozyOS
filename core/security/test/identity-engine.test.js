...
// ---------------------------------------------------------------
// assignApplication() — Level 2B repair: the real, canonical
// ServiceRegistry application ids already in use elsewhere in this
// repository (pharmacyos_core_001, wholesaleos_core_001,
// churchos_core_001, quarry_manager_001) contain underscores; this
// method previously rejected every one of them, so no real user could
// ever be assigned any of those real applications through this
// authoritative path. Minimum fix: underscore added to the allowed
// character set. These tests confirm the fix and that the original
// validation intent (reject anything unsafe) is unchanged.
// ---------------------------------------------------------------

test('assignApplication(): accepts the real, canonical underscore-containing ServiceRegistry ids', async () => {
    const IE = freshIdentityEngine();
    const { userId } = await registerRealUser(IE);
    for (const appId of ['pharmacyos_core_001', 'wholesaleos_core_001', 'churchos_core_001', 'quarry_manager_001']) {
        assert.doesNotThrow(() => IE.assignApplication(userId, appId));
    }
    assert.deepEqual(
        [...IE.listAssignedApplications(userId)].sort(),
        ['churchos_core_001', 'pharmacyos_core_001', 'quarry_manager_001', 'wholesaleos_core_001']
    );
});

test('assignApplication(): still accepts plain hyphenated ids (e.g. "shopos") — no regression for the existing convention', async () => {
    const IE = freshIdentityEngine();
    const { userId } = await registerRealUser(IE);
    assert.doesNotThrow(() => IE.assignApplication(userId, 'shopos'));
    assert.doesNotThrow(() => IE.assignApplication(userId, 'developer-hub'));
});

test('assignApplication(): still rejects unsafe/malformed identifiers — the validation intent is preserved, not weakened', async () => {
    const IE = freshIdentityEngine();
    const { userId } = await registerRealUser(IE);
    for (const bad of ['../../etc/passwd', '<script>alert(1)</script>', 'app id with spaces', 'app/with/slashes', 'app.with.dots', '']) {
        assert.throws(() => IE.assignApplication(userId, bad), TypeError);
    }
});

test('assignApplication(): still throws for an unknown userId — authorization boundary unchanged', async () => {
    const IE = freshIdentityEngine();
    assert.throws(() => IE.assignApplication('no-such-user', 'shopos'), /unknown userId/);
});
