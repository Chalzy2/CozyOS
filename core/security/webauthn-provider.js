...
(function () {
    ...
    const WEBAUTHN_VERSION = "1.1.0-ENTERPRISE"; // 1.1.0: Milestone 360 — added renameCredential()/getCredentialInfo() (additive only, Rule 3 / Principle 23 approved)
    ...
    class CozyWebAuthnProvider {
        ...
        /**
         * renameCredential(userId, nickname) — real, additive (Milestone
         * 360; approved under Rule 3 / Principle 23 after the Stage 1
         * provider audit confirmed no rename/nickname path existed for
         * this factor). Sets a display nickname on the existing
         * credential record only — never touches the public key,
         * credentialId, signCount, or verify() path.
         */
        renameCredential(userId, nickname) {
            const record = this.#credentialsByUser.get(userId);
            if (!record) return { success: false, reason: "No real Platform Authenticator credential is registered for this user." };
            if (!nickname || typeof nickname !== "string" || !nickname.trim()) {
                return { success: false, reason: "A real, non-empty nickname is required." };
            }
            record.nickname = nickname.trim();
            this.#emit("renamed", { userId, nickname: record.nickname });
            return { success: true, nickname: record.nickname };
        }

        /**
         * getCredentialInfo(userId) — real, read-only (Milestone 360,
         * additive). Exposes only non-secret metadata for a "view
         * enrollment details" UI — never the public key or raw
         * credentialId bytes.
         */
        getCredentialInfo(userId) {
            const record = this.#credentialsByUser.get(userId);
            if (!record) return null;
            return { nickname: record.nickname || null, rpId: record.rpId, createdAt: record.createdAt, signCount: record.signCount };
        }
    ...
    if (window.CozyOS.ServiceRegistry && typeof window.CozyOS.ServiceRegistry.registerCoordinator === "function") {
        try {
            window.CozyOS.ServiceRegistry.registerCoordinator({ sourcePath: "core/security/webauthn-provider.js",
