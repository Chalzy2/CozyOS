...
(function () {
    ...
    class LivingLearnPanel {
        ...
        /** open({ userId, container }) — renders the panel. Requests no hardware access by itself. */
        open({ userId, container }) {
        ...
        async #runScan() {
            ...
            videoHost.querySelector('#cozy-learn-capture').addEventListener('click', async () => {
                ...
                const result = await camera.captureForLearning(this.#videoEl, { context: null });
                camera.stopCapture();
                videoHost.innerHTML = '';
        ...
        async #runListenAndFinish(visualFromScan) {
            ...
            // KISWAHILI FLUENCY REQUIREMENT (CP13): Kiswahili is this
            // project's first-priority recognition language — "sw" is the
            // real, already-registered CozySpeech language code (confirmed
            // via CozySpeech.listLanguages(), not invented here; the
            // registry has no region-specific bcp47Tag field yet, e.g.
            // "sw-KE" vs "sw-TZ", so plain "sw" is the honest value
            // actually available, not a fabricated refinement of it).
            // continuous+interimResults are real SpeechRecognitionAdapter/
            // Web Speech API config fields — turning them on here is what
            // "fast, continuous hearing, not waiting for every sentence to
            // finish" concretely means at this layer. This affects
            // RECOGNITION speed only; it has no bearing on speech OUTPUT
            // (voice/TTS), which remains untouched and out of scope for
            // this checkpoint.
            const voice = await pipeline.captureVoiceForLearning({ languageCode: 'sw', continuous: true, interimResults: true });
        ...
        async #finishObservation(visual, audio) {
            ...
            const result = pipeline.learnFromMultimodalObservation({ userId: this.#userId, visual, audio, context: null });
