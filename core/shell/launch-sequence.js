 ...
 */
        (function () {
            // NO-REPLAY-AFTER-LOGIN FIX: this file is the SAME shared
            // sequence every host page (index.html, login.html,
            // dashboard.html, admin-workspace.html standalone) loads via
            // a real <script src> tag, and for every one of those real
            // page loads this guard is a no-op — window.CozyOS.Bootstrap
            // does not exist yet at that point, or exists but is not
            // "starting". The ONE real exception: core/bootstrap/
            // bootstrap.js's Bootstrap.start() re-injects
            // admin-workspace.html's own <script> tags (including this
            // one) into chalzydashboard.html's already-live document
            // AFTER a real admin has already sat through this exact
            // sequence once on login.html and already submitted the
            // login form — Bootstrap.start() sets its internal state to
            // "starting" (isStarting() === true) for the ENTIRE duration
            // of that composition, synchronously before any of these
            // re-injected scripts run. Detecting exactly that one
            // condition here — rather than in admin-workspace.html —
            // means the fix lives in the one file that actually knows
            // whether it is about to run its animation/voice/sound a
            // second time, and every other caller is completely
            // unaffected: zero change to timing, animation, voice, sound,
            // or text for the real, single, intended first playthrough.
            // Emits the exact same completion event the full sequence
            // would have emitted at its true end, immediately — so
            // admin-workspace.html's own existing
            // "cozy:launch-sequence-complete" listener (unchanged) still
            // proceeds straight to mounting the workspace, with no
            // added delay and no second 30-second wait.
            const bootstrapInProgress = window.CozyOS && window.CozyOS.Bootstrap && typeof window.CozyOS.Bootstrap.isStarting === "function" && window.CozyOS.Bootstrap.isStarting();
            if (bootstrapInProgress) {
                const busForSkip = window.CozyOS && window.CozyOS.PlatformEventBus;
                if (busForSkip && typeof busForSkip.emit === "function") busForSkip.emit("cozy:launch-sequence-complete", { skipped: "post-login-bootstrap-composition" });
                return;
            }
            ...
            const STARTUP_TIMING = Object.freeze({
                // AUTHORIZED REBUILD (20s cinematic target): TOTAL_DURATION_MS
                // is no longer used to compute an artificial "wait until this
                // many ms have elapsed" hold — see BREATHING_MS below, which is
                // what actually governs the final pause now. This value is kept
                // only as the documented design target for logging/telemetry
                // (see the console.log at sequence completion), so nothing that
                // reads it for reporting purposes breaks.
                ...
                // AUTHORIZED REBUILD — real, fixed "breathing" pause once the
                // logo/motto have settled into their upper position and before
                // the completion event fires (spec section 13: "approximately
                // 2-5 seconds ... preferably around 2 seconds"). This REPLACES
                // the old "however much time remains to reach TOTAL_DURATION_MS"
                // hold, which is exactly the artificial floor the spec calls out
                // (a sequence that finished its real audio+visuals early used to
                // sit frozen just to burn time up to 30s). The total run length
                // now falls out honestly from real audio duration + real visual
                // choreography + this fixed breathing period — never from a
                // stopwatch target.
                BREATHING_MS: 2200,
            ...
            // Login Sequence video integration — real, official CozyOS
            // live-background video library, registered the same
            // idempotent way as the sound pack immediately above.
            // Actual playback is started later by revealLiveBackground()
            // (see startup-orchestrator.js), not here — this call only
            // registers what's available, matching loadOfficialSoundPack's
            // own register-now/play-on-demand split.
            if (orchestrator && typeof orchestrator.loadOfficialVideoPack === "function") orchestrator.loadOfficialVideoPack();
            ...
            async function playMottoVoice() {
                ...
                // Cozy AI voice choice): synthesis here is the user's actual
                // selected provider speaking its own line, not an impersonation
                // of the Owner's voice, so TTS is the correct real behavior —
                // same provider as the other two phrases, one speaker throughout.
                        ...
                        setTimeout(() => {
                            ...
                            fallBounceSplitColorText(slogan, SLOGAN, SLOGAN_SPLIT_INDEX, () => {
                                ...
                                playMottoVoice().then(() => {
                                    ...
                                    // AUTHORIZED REBUILD (removes the artificial
                                    // 30s-floor hold): the pause before the
                                    // completion event fires is now the fixed,
                                    // intentional BREATHING_MS pause (spec
                                    // section 13), full stop — never a
                                    // "top up to TOTAL_DURATION_MS" calculation.
                                    // The real total run length is whatever the
                                    // real, variable-length Owner audio plus the
                                    // real visual choreography plus this fixed
                                    // breathing period actually add up to; it is
                                    // reported (not enforced) below via
                                    // actualTotalMs against the ~20s design
                                    // target.
                                    const holdDurationMs = STARTUP_TIMING.BREATHING_MS;
