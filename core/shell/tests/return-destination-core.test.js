...
test('rejects /dashboard (routing reversion: dashboard-as-admin-entry undone, /dashboard is ordinary now)', () => {
    const core = loadCore();
    assert.equal(core.resolveReturnDestination('/dashboard'), null);
});

test('rejects /dashboard.html (routing reversion: dashboard-as-admin-entry undone, ordinary now)', () => {
    const core = loadCore();
    assert.equal(core.resolveReturnDestination('/dashboard.html'), null);
});
...
test('ALLOWED_DESTINATIONS is frozen and contains exactly the two expected entries (routing reversion)', () => {
    const core = loadCore();
    assert.deepEqual(Array.from(core.ALLOWED_DESTINATIONS), ['/chalzydashboard', '/chalzydashboard.html']);
