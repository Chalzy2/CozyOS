...
(function () {
    ...
    // and /dashboard.html are REMOVED from this allowlist. They are no
    // longer administrator-entry routes at all (see
    // static-boundary-server.js's ADMIN_CANONICAL_ROUTES, reverted in
    // the same change) — they are the ordinary, ungated User Dashboard.
    // An unauthenticated visitor to /dashboard is not requesting a
    // protected route, so there is nothing here for this allowlist to
    // preserve; login.html's ordinary default (index.html) is correct
    // for that case, exactly like any other public page. Only the two
    // real, still-protected administrator routes remain allowlisted.
