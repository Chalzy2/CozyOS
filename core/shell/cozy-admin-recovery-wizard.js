 ...
 *   Milestone 352: revoke old device(s) + old biometric credential →
 *   register the new device → offer new biometric enrollment). Nothing
 *   else.
 *
...
(function () {
    ...
    const WIZARD_VERSION = "1.1.0-ENTERPRISE-M352";
    ...
    class CozyAdminRecoveryWizard {
        ...
        #renderNewPasswordStep() {
            ...
            this.#panel().querySelector("#cz-recovery-set-password").addEventListener("click", async () => {
                ...
                if (typeof identity.logSecurityEvent === "function") {
                    identity.logSecurityEvent("ADMIN_RECOVERY_PASSWORD_RESET", this.#username);
                }
                this.#renderDeviceReplacementStep();
            });
        }

        /**
         * #renderDeviceReplacementStep()
         *   Milestone 352 — mandatory lost-device-recovery completion.
         *   Real, composes only existing engines:
         *     - TrustedDeviceManager.listDevicesForUser()/removeDevice()
         *       for revoking every old trusted device this administrator
         *       had (lost, stolen, reset, or being replaced) — never a
         *       second device-trust system.
         *     - WebAuthnProvider.removeCredential()/registerCredential()
         *       for revoking the old biometric credential and enrolling
         *       a real new one on this (the new) device — never a
         *       second WebAuthn implementation.
         *     - TrustedDeviceManager.generateFingerprint()/registerDevice()
         *       to register THIS browser (the one completing recovery
         *       right now) as the new trusted device.
         *     - IdentityEngine.logSecurityEvent() for every action, into
         *       the one real, existing audit log — no new audit store.
         *   Skippable only if this administrator genuinely has no
         *   real trusted devices on record (nothing to revoke) — never
         *   skippable if a real device exists, since an old device that
         *   is never revoked could still authenticate, which the spec
         *   states must never happen.
         */
        #renderDeviceReplacementStep() {
            const tdm = window.CozyOS.TrustedDeviceManager;
            const oldDevices = tdm && typeof tdm.listDevicesForUser === "function"
                ? tdm.listDevicesForUser(this.#userId).filter(d => !d.revoked) : [];

            if (oldDevices.length === 0) {
                this.#panel().innerHTML = `
                    <h2 style="margin:0 0 4px 0;font-size:18px;">Password Reset Complete</h2>
                    <p style="margin:0 0 14px 0;color:#666;font-size:13px;">No trusted devices were on record for "${escapeHtml(this.#username)}" — nothing to revoke. You can sign in now.</p>
                    <button id="cz-recovery-done" style="width:100%;padding:9px;border:none;border-radius:6px;background:#2563eb;color:#fff;font-weight:600;cursor:pointer;">Done</button>`;
                this.#panel().querySelector("#cz-recovery-done").addEventListener("click", () => this.close());
                return;
            }

            const rows = oldDevices.map(d => `<li style="font-size:12px;color:#444;margin-bottom:4px;">${escapeHtml(d.nickname || "Unnamed Device")} <span style="color:#999;">(${escapeHtml(d.deviceId)})</span></li>`).join("");
            this.#panel().innerHTML = `
                <h2 style="margin:0 0 4px 0;font-size:18px;">Replace Lost Device</h2>
                <p style="margin:0 0 10px 0;color:#666;font-size:13px;">Password reset. Now revoke every old device and register the one you're using right now as the new trusted device — required so a lost, stolen, reset, or replaced device can never sign in again.</p>
                <p style="margin:0 0 4px 0;font-size:12px;font-weight:600;color:#444;">Devices to revoke:</p>
                <ul style="margin:0 0 14px 0;padding-left:18px;">${rows}</ul>
                <label style="font-size:12px;color:#444;display:block;margin-bottom:10px;">Nickname for this new device
                    <input id="cz-new-device-nickname" type="text" placeholder="e.g. My New Phone" style="display:block;width:100%;box-sizing:border-box;padding:8px;margin-top:4px;border:1px solid #ccc;border-radius:6px;">
                </label>
                <div id="cz-method-error" style="display:none;margin-bottom:10px;padding:8px;border-radius:6px;background:#fef2f2;color:#b91c1c;font-size:12px;"></div>
                <div id="cz-recovery-success" style="display:none;margin-bottom:10px;padding:8px;border-radius:6px;background:#f0fdf4;color:#15803d;font-size:12px;"></div>
                <button id="cz-recovery-replace-device" style="width:100%;padding:9px;border:none;border-radius:6px;background:#2563eb;color:#fff;font-weight:600;cursor:pointer;">Revoke Old Device(s) &amp; Register This One</button>`;

            this.#panel().querySelector("#cz-recovery-replace-device").addEventListener("click", async () => {
                const identity = window.CozyOS.IdentityEngine;
                const webauthn = window.CozyOS.WebAuthnProvider;
                const nickname = this.#panel().querySelector("#cz-new-device-nickname").value.trim() || "Recovered Device";

                // 1. Revoke every old device — real, existing removeDevice(),
                //    which already logs to TrustedDeviceManager's own audit trail.
                for (const device of oldDevices) {
                    tdm.removeDevice(device.deviceId, "Revoked during Administrator Recovery — device reported lost, stolen, reset, or replaced.");
                    if (identity && typeof identity.logSecurityEvent === "function") {
                        identity.logSecurityEvent("DEVICE_REVOKED_ON_RECOVERY", `${this.#username} :: ${device.deviceId}`);
                    }
                }

                // 2. Revoke the old biometric credential. WebAuthnProvider
                //    stores exactly one real credential per userId (not per
                //    device), so this is a complete revocation of whatever
                //    credential the old device had — never a partial one.
                if (webauthn && typeof webauthn.removeCredential === "function") {
                    const hadOne = typeof webauthn.hasCredential === "function" ? webauthn.hasCredential(this.#userId) : true;
                    webauthn.removeCredential(this.#userId);
                    if (hadOne && identity && typeof identity.logSecurityEvent === "function") {
                        identity.logSecurityEvent("BIOMETRIC_REVOKED_ON_RECOVERY", this.#username);
                    }
                }

                // 3. Register THIS browser (the one completing recovery) as
                //    the new trusted device.
                let newDevice;
                try {
                    const fingerprint = await tdm.generateFingerprint();
                    const registerResult = tdm.registerDevice(this.#userId, { nickname, fingerprint });
                    if (!registerResult.success) { this.#showError(registerResult.reason || "Could not register this device."); return; }
                    newDevice = registerResult.device;
                    if (identity && typeof identity.logSecurityEvent === "function") {
                        identity.logSecurityEvent("DEVICE_REGISTERED_ON_RECOVERY", `${this.#username} :: ${newDevice.deviceId}`);
                    }
                } catch (err) { this.#showError(`Could not register this device: ${err.message}`); return; }

                this.#renderNewBiometricEnrollmentStep(newDevice);
            });
        }
        ...
        #renderNewBiometricEnrollmentStep(newDevice) {
            const webauthn = window.CozyOS.WebAuthnProvider;
            const supported = webauthn && typeof webauthn.isSupported === "function" && webauthn.isSupported();
            this.#panel().innerHTML = `
                <h2 style="margin:0 0 4px 0;font-size:18px;">Enroll Biometrics on This Device</h2>
                <p style="margin:0 0 14px 0;color:#666;font-size:13px;">Old device(s) revoked and this device is now trusted. ${supported ? "Enroll biometric sign-in on this new device now, or skip and enroll later." : "This browser does not support WebAuthn, so biometric enrollment is not available here — you can still sign in with your password."}</p>
                <div id="cz-method-error" style="display:none;margin:10px 0;padding:8px;border-radius:6px;background:#fef2f2;color:#b91c1c;font-size:12px;"></div>
                <div style="display:flex;gap:8px;">
                    ${supported ? `<button id="cz-recovery-enroll-biometric" style="flex:1;padding:9px;border:none;border-radius:6px;background:#2563eb;color:#fff;font-weight:600;cursor:pointer;">Enroll Biometrics</button>` : ""}
                    <button id="cz-recovery-finish" style="${supported ? "padding:9px 14px;border:1px solid #ccc;background:#fff;" : "flex:1;padding:9px;border:none;background:#2563eb;color:#fff;font-weight:600;"}border-radius:6px;cursor:pointer;">${supported ? "Skip" : "Done"}</button>
                </div>`;
            this.#panel().querySelector("#cz-recovery-finish").addEventListener("click", () => this.close());
            const enrollBtn = this.#panel().querySelector("#cz-recovery-enroll-biometric");
            if (enrollBtn) {
                enrollBtn.addEventListener("click", async () => {
                    ...
                    successEl.style.cssText = "margin:10px 0 0 0;padding:8px;border-radius:6px;background:#f0fdf4;color:#15803d;font-size:12px;";
                    successEl.textContent = "Biometric sign-in enrolled on this device.";
