...
(function () {
    ...
    class CozyOSWorkspaceShell {
        ...
        getApplicationCenterData() {
            ...
            const allowlist = this.constructor.ENTERPRISE_DASHBOARD_APPS;
            const filtered = applications.filter(app => allowlist.includes(app.id));
            // Milestone 353 — real fix: this list was previously unfiltered
            // by role (every signed-in user, of any role, saw every
            // enterprise application). Reuses IdentityEngine's own real
            // canAccessApplication(userId, appName) — never a second
            // permission system. Administrator/Developer behavior is
            // unchanged (Admin passes everything; Developer already only
            // has developer-hub, which isn't in this enterprise-app list
            // to begin with). Fails closed: no resolved user or no
            // IdentityEngine → nothing is shown, never "show everything".
            ...
            const roleFiltered = (identity && typeof identity.canAccessApplication === "function" && this.#currentUserId)
                ? filtered.filter(app => identity.canAccessApplication(this.#currentUserId, app.id))
                : (this.#currentUserRole === "admin" ? filtered : []);
            ...
            if (!this.#documentClickDismissBound) {
                this.#domRoot.addEventListener("click", (evt) => {
                    ...
                    if (openBtn && !openBtn.disabled) {
                        ...
                        // M365.0: consolidated to the single, shared
                        // ApplicationVisibility.getRealLaunchPath() -
                        // previously an inline copy of this same lookup
                        // lived only here; removed to avoid two sources
                        // of truth for the same real data.
                    ...
                    if (evt.target.dataset && evt.target.dataset.appAction === "open") {
                        // M345 real fix: previously called this.launch(), which only
                        // consults this.#launchers — populated exclusively by
                        // registerLauncher(), which nothing in the entire codebase
                        // ever calls. Every Open button was therefore permanently
                        // dead. Composes the real, already-built, already-loaded
                        // window.CozyOS.ApplicationLauncher (core/shell/application-
                        // launcher.js, verified present on dashboard.html) instead of
                        // inventing a second launch system. registerLauncher()/
                        // launch() are left in place (removing working code/specs is
                        // forbidden) but are no longer on the Open button's path.
                        const appId = evt.target.dataset.appId;
                        const launcher = window.CozyOS && window.CozyOS.ApplicationLauncher;
                        if (!launcher || typeof launcher.open !== "function") {
                            console.warn(`[WorkspaceShell] ApplicationLauncher is not loaded — cannot open "${appId}".`);
                            return;
                        }
                        launcher.open(appId).then(result => {
                            if (result && result.success) {
                                ...
                                this.#diagnostics.errorsHidden++;
                                console.warn("[WorkspaceShell]", (result && result.reason) || `Unknown failure opening "${appId}".`);
                            }
                            this.#render();
                        }).catch(err => {
                            this.#diagnostics.errorsHidden++;
                            console.warn(`[WorkspaceShell] ApplicationLauncher.open("${appId}") threw:`, err && err.message);
                        });
                        return;
                    }
