...
(function () {
    ...
    class CozyWindowManager {
        ...
        /**
         * setBounds(id, { x, y, width, height, minWidth, minHeight })
         *   M389 — real, generic missing dependency identified by audit:
         *   until now nothing could programmatically move/resize a
         *   window (only interactive drag/#wireDrag and interactive
         *   resize/#wireResize existed). Added specifically so content
         *   that needs a smaller-than-normal floating size (e.g. a
         *   compact live-video preview) can ask for it without a
         *   second window/positioning system.
         *
         *   Unmaximizes first (bounds are meaningless while maximized).
         *   Reuses the exact same #clampToViewport() every drag already
         *   uses - no second clamping rule. minWidth/minHeight let a
         *   caller opt into a smaller floor than the interactive-resize
         *   MIN_WIDTH/MIN_HEIGHT (320x240, sized for a full window's
         *   chrome) - interactive corner/edge dragging is completely
         *   unaffected and still enforces the original constants.
         */
        setBounds(id, { x, y, width, height, minWidth = MIN_WIDTH, minHeight = MIN_HEIGHT } = {}) {
            const entry = this.#windows.get(id);
            if (!entry) return { success: false, reason: `No real window "${id}".` };
            entry.state.maximized = false;
            const w = Math.max(minWidth, width != null ? width : entry.state.width);
            const h = Math.max(minHeight, height != null ? height : entry.state.height);
            const targetX = x != null ? x : entry.state.x;
            const targetY = y != null ? y : entry.state.y;
            const clamped = this.#clampToViewport(targetX, targetY, w, h);
            entry.state.x = clamped.x; entry.state.y = clamped.y;
            entry.state.width = w; entry.state.height = h;
            this.#applyState(id);
            this.#persist(id);
            return { success: true, bounds: { x: entry.state.x, y: entry.state.y, width: w, height: h } };
        }

        /** getBounds(id) — real, paired getter for setBounds(); also exposes minimized/maximized so a caller can decide whether it's safe to reposition right now. */
        getBounds(id) {
            const entry = this.#windows.get(id);
            if (!entry) return { success: false, reason: `No real window "${id}".` };
            const { x, y, width, height, minimized, maximized } = entry.state;
            return { success: true, x, y, width, height, minimized, maximized };
        }
        ...
        #getHandle(id) {
            return {
                ...
                setBounds: (bounds) => this.setBounds(id, bounds),
                getBounds: () => this.getBounds(id)
            };
        }
